...
public class SmlTestAgent implements TestAgent, PrintEventInterface, UpdateEventInterface {
  ...
  /**
   * creates the agent, sets up update event, RHS functions assumes kernel has already been created
   *
   * @param test
   */
  private void commonInitialize(Test test) {
    ...
    this.updateHandlerId =
        kernel.RegisterForUpdateEvent(
            sml.smlUpdateEventId.smlEVENT_AFTER_ALL_OUTPUT_PHASES, this, null);
  ...
  private void loadTestCode(Test test) throws SoarException {
    try {
      executeCommandLine(
          String.format(
              "pushd \"%s\"",
              FileTools.getParent(UrlTools.toFile(test.getTestCase().getUrl())).replace('\\', '/')),
          true);
  ...
  private String prepSoarCodeForSml(String code) {
    return code.trim()
        .replace("(pass)", "(exec pass)") // when there are no args
