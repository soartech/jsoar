...
public class JSoarTestAgent implements TestAgent {
    ...
    try {
      agent
          ...
          .eval(
              String.format(
                  "pushd \"%s\"",
                  UrlTools.getParent(test.getTestCase().getUrl()).toString().replace('\\', '/')));
  ...
  public void debug(Test test, boolean exitOnClose) throws SoarException, InterruptedException {
    ...
    debugProps.put(
        DebuggerProvider.CLOSE_ACTION, exitOnClose ? CloseAction.EXIT : CloseAction.DISPOSE);
