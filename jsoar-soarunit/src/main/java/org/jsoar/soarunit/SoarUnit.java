...
@Command(
    name = "soarunit",
    description = "SoarUnit - Soar unit test framework",
    subcommands = {HelpCommand.class})
public class SoarUnit implements Callable<Integer> {
  @Option(
      names = {"-R", "--recursive"},
      description = "Recursively search directories for tests.")
  boolean recursive;

  @Option(
      names = {"-d", "--debug"},
      paramLabel = "TEST_NAME",
      description = "Open the given test in a debugger.")
  String debugTestName = "";

  @Option(
      names = {"-u", "--ui"},
      description = "Show graphical user interface.")
  boolean ui;

  @Option(
      names = {"-s", "--sml"},
      description =
          "Use CSoar/SML instead of JSoar. CSoar's bin directory must be on the system path or in java.library.path.")
  boolean sml;

  @Option(
      names = {"-t", "--threads"},
      converter = ThreadPoolSizeConverter.class,
  ...
  public static void main(String[] args) throws Exception {
    ...
    int result =
        new CommandLine(new SoarUnit(writerProxy, () -> Paths.get("."), true)).execute(args);
    writer.flush();
    if (result != DONT_EXIT) {
      System.exit(result);
    }
  ...
  private ExecutorService getExecutor() {
    if (this.threads == null) {
      ...
      return this.threads.fixed == 1
          ? Executors.newSingleThreadExecutor()
          : Executors.newFixedThreadPool(this.threads.fixed);
    }
  }
  ...
  public Integer call() throws SoarException, InterruptedException, IOException {
    if (this.sml) {
      out.println(
          "Using CSoar/SML, make sure that SOAR_HOME is set to the location of your Soar binaries.");
          ...
          new Runnable() {
            ...
            public void run() {
              ...
              mf.setDefaultCloseOperation(
                  fromCommandLine ? JFrame.EXIT_ON_CLOSE : JFrame.DISPOSE_ON_CLOSE);
              mf.setVisible(true);
              mf.runTests();
            }
          });
  ...
  private void debugTest(List<TestCase> all, String name)
  ...
  private int printAllTestCaseResults(final List<TestCaseResult> results, FiringCounts coverage) {
    ...
    for (TestCaseResult result : results) {
      ...
      for (TestResult testResult : result.getTestResults()) {
        ...
        if (testResult.isPassed()) {
          out.printf(
              "PASSED: %s, %s (%.3f s)%n",
              test.getName(), testResult.getMessage(), testResult.getRunTimeInSeconds());
          totalPassed++;
        } else {
          out.printf(
              "FAILED: %s, %s (%.3f s)%n",
    ...
    out.println("-------------------------------------------------------------");
    out.printf(
        "%d/%d tests run. %d passed, %d failed, %d%% coverage%n",
  ...
  private static class ThreadPoolSizeConverter
      implements CommandLine.ITypeConverter<ThreadPoolSize> {
      ...
      try {
        ...
        if (result.fixed < 1) {
          throw new CommandLine.TypeConversionException(
              "Invalid value " + value + ": must be 1 or more.");
        }
      } catch (NumberFormatException nan) {
        try {
          result.dynamic = ThreadPoolSize.Dynamic.valueOf(value.toLowerCase());
  ...
  /**
   * Provides a lazy way to get a print writer -- helps with SoarUnitCommand, for which we want
   * whatever printer is on top of the stack
   *
   * @author bob.marinier
   */
  @FunctionalInterface
  ...
  /**
   * Provides a lazy way to get the current working directory -- helps with SoarUnitCommand, for
   * which we want to use the agent's cwd
   *
   * @author bob.marinier
   */
