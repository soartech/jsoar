...
/**
 * So this is a very unique class. What this does is it tries to find jsoar-core-*.jar and then load
 * Agent from it. If it can it'll fall back on the internal jsoar version.
 *
 * @author ALT
 */
public class JSoarAgentFactory {
  ...
  JSoarAgentFactory(Path jsoarCorePath) throws MalformedURLException, ClassNotFoundException {
    ...
    child = new URLClassLoader(new URL[] {url}, this.getClass().getClassLoader());
  ...
  /**
   * @return the label for a test. For JSoar this will be 'JSoar' and for JSoar this will be 'JSoar
   *     X' where X is the label in the configuration file.
   */
  Path getSoarPath() {
  ...
  Object newAgent(String agentLabel) {
    if (agent != null) {
      try {
        return agent.getDeclaredConstructor(String.class).newInstance(agentLabel);
          ...
          | SecurityException e) {
        System.err.println(
            "Failed to create new JSoar agent instance: " + e + ".  Returning default agent.");
        return new Agent(agentLabel);
      }
    } else return new Agent(agentLabel);
  }
}
