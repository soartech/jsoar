...
public class JSoarTest implements Test {
  ...
  /**
   * Sets all the values used in the test to be impossible values so we know if something failed
   * horribly.
   *
  ...
  public boolean run(int runCount) throws SoarException {
    ...
    agent =
        (Agent)
            agentFactory.newAgent(
                "JSoar Performance Testing Agent - " + testName + " - " + runCount);
    ...
    if (settings.getDecisionCycles().size() == 0 || settings.getDecisionCycles().get(0) == 0)
      agent.runForever();
    else agent.runFor(settings.getDecisionCycles().get(0), RunType.DECISIONS);
    ...
    memoryForRun =
        Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory() - memoryForRun;
