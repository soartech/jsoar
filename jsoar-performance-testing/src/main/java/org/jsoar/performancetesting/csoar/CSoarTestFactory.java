...
public class CSoarTestFactory implements TestFactory {
  ...
  /**
   * This creates a new and initialized CSoar test. It also, as by product of creating the CSoar
   * test, will load the CSoar sml classes for the first time if necessary.
   *
   * @param testName
   * @param testFile
   * @return A new and initialized CSoar Test (but may be only assertion errors if it didn't load
   *     properly.)
   ...
   */
  @Override
  public Test createTest(String testName, Path testFile, TestSettings settings)
