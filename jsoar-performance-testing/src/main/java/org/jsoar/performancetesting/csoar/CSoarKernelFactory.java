...
/**
 * So this is a very unique class. What this does is it tries to find sml.jar and then load
 * sml.kernel from it. If it can it'll create CSoarKernelWrappers with the appropriate kernel
 * constructors otherwise it'll return a default version which is entire asserts.
 *
 * @author ALT
 */
public class CSoarKernelFactory {
  ...
  CSoarKernelFactory(Path csoarDirectory)
    ...
    else libraries = csoarDirectory.toString();
  ...
  CSoarKernelWrapper CreateKernelInNewThread() {
    ...
    } else return new DefaultCSoarKernelWrapper();
  }
  ...
  CSoarKernelWrapper CreateKernelInCurrentThread(boolean optimized) {
    if (initialized) {
      ...
      try {
        Method method =
            kernel.getDeclaredMethod("CreateKernelInCurrentThread", String.class, boolean.class);
  ...
  /**
   * @return the label for a test. For JSoar this will be 'JSoar X' and for CSoar this will be
   *     'CSoar X' where X is the label in the configuration file.
   */
  Path getSoarPath() {
