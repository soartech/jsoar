...
public abstract class PicocliSoarCommand implements SoarCommand {
  ...
  /**
   * Creates a PicocliSoarCommand with an agent. A command with an agent may print results to the
   * agent's printer and/or return a result directly.
   */
  public PicocliSoarCommand(Agent agent, Object picocliCommand) {
    this.agent = agent;
    this.picocliCommand = picocliCommand;
    this.commandLine =
        new CommandLine(picocliCommand).setExecutionExceptionHandler(new ExceptionHandler());
  }

  /**
   * Creates a PicocliSoarCommand with no agent. A command with no agent is expected to return any
   * result directly.
   */
