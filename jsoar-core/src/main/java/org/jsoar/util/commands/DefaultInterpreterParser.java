...
public class DefaultInterpreterParser {
  ...
  public ParsedCommand parseCommand(ParserBuffer reader) throws IOException, SoarException {
    ...
    final SourceLocation loc =
        DefaultSourceLocation.newBuilder()
            .file(reader.getFile())
    ...
    while ((c = read(reader)) != -1) {
      switch (c) {
        ...
        case '#':
          unread(reader, c);
          return true;
          // Consume in the case of a semicolon: it's lexically a part of the command we just
          // processed.
          // (E.g., consider the case of the empty command ";": we would expect that parsing this
          // would
  ...
  public String parseWord(ParserBuffer reader) throws SoarException, IOException {
    ...
    if (c == '"') {
      ...
      if (braces > 0) {
        throw new SoarParserException(
            "Unexpected end of input. Unmatched opening brace", startOfCommand);
