...
/**
 * Interface for an object that knows how to execute Soar commands and source Soar files. This
 * interface only implements the command parser and interpreter. The actual implementation of the
 * commands is left to instances of {@link SoarCommand}.
 *
 * @author ray
 * @see SoarCommands
 */
public interface SoarCommandInterpreter {
  ...
  /**
   * Dispose of this interpreter, detaching it from the agent. This method is called when a new
   * interpreter is added to the agent. It is also called when {@link Agent#dispose()} is called.
   */
  void dispose();
   ...
   *
   * <p>This is used by the Soar IDE.
   *
   ...
   *
   * <p>If the file refers to a directory that is different from the current directory, then the
   * current directory is first changed and then the file is sourced. If the file is relative it is
   * assumed to be relative to the interpreter's current directory.
   *
   ...
   *
   * <p>While the contents of the URL are being sourced, the interpreter's current directory will be
   * changed to that URL. If files are sourced recursively, the interpreter will do its best to
   * source them relative to this URL, but this may not always make sense.
   *
   * <p>Note that a URL for a Java resource may be obtained through {@link
   ...
   * assumed to be relative to the interpreter's current directory.
   *
   * <p>Note that the working directory is not changed while loading the rete file because
   * additional files cannot be loaded during a rete deserialization.
   *
  ...
  /**
   * Returns a sorted list of the names of all the registered SoarCommands This is not intended to
   * report aliases or interpreter-specific commands (e.g., Tcl commands)
   */
  public List<String> getCommandStrings() throws SoarException;

  /**
   * Returns the interpreter's SoarTclExceptionsManager This is intended to be used by the language
   * server to provide context/info for "soft" exceptions that are caught and logged but not
   * "reported" to IDEs
   */
