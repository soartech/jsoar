...
public class SoarEventManager {
  ...
  private Map<Class<? extends SoarEvent>, List<SoarEventListener>> listeners =
      Collections.synchronizedMap(
          new HashMap<Class<? extends SoarEvent>, List<SoarEventListener>>());

  public SoarEventManager() {}

  /**
   * Add a listener for a particular type of event. A listener may be registered for more than one
   * type of event.
   *
   ...
   * @param klass The class of the event type to remove the listener from, or <code>null</code> to
   *     completely remove the listener from the manager.
   ...
   *
   * <p>It is safe to call this method from any thread, but may negatively affect listeners that
   * expect to be called on a particular thread.
   *
  ...
  /**
   * Fire the given event to all listeners registered for the given event type. This extended
   * version of {@link #fireEvent(SoarEvent)} allows a particular event class to be specified so
   * that sub-classes of an existing event class can be properly routed to listeners of that parent
   * class.
   *
   * @param <T> Event type
   * @param event The event object. Must be non-null.
