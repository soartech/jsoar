...
public class Adaptables {
  ...
  /**
   * Adapt an object to the given class. This is equivalent to o.getAdapter(klass) but o may be null
   * or not adaptable and casting is handled automatically with generics.
   *
   * @param <T> The desired type
   * @param o The object to adapt. If not Adaptable, then a simple instanceof test is performed. May
   *     be null.
  ...
  /**
   * Filter a collection of objects to those that are adaptable to a particular type.
   *
  ...
  /**
   * Search a collection for the first object that is adaptable to a particular type.
   *
   * @param <T> Desired type
   * @param collection Collection to search
   * @param klass Desired type class
   * @return First object in collection that is adaptable to T, or null if none was found.
   */
  ...
  public static <T> T require(Class<?> requiringClass, Object context, Class<T> requiredClass) {
    ...
    if (t == null) {
      throw new IllegalStateException(
          requiringClass.getName() + " requires " + requiredClass.getCanonicalName());
    }
    return t;
  }
}
