...
public class ScriptCommand extends PicocliSoarCommand {
  ...
  @Command(
      name = "script",
      description = "Runs Javascript, Python, or Ruby code",
  ...
  public static class Script implements Callable<String> {
    ...
    @Option(
        names = {"-d", "--dispose"},
        description = "Disposes the given script engine")
    boolean dispose = false;

    @Option(
        names = {"-r", "--reset"},
        description = "Re-initializes the given script engine")
    boolean reset = false;
    ...
    public String call() {
      ...
      try {
        ...
        List<String> trailing =
            new ArrayList<>(Arrays.asList(engineNameAndCode).subList(1, engineNameAndCode.length));
