...
/**
 * A wrapper around a raw {@link Agent} which gives the agent its own thread and provides methods
 * for safely interacting with the agent from other threads. The wrapper includes a number of
 * convenience wrapper methods that forward to the methods with the same name in the underlying
 * agent.
 *
 * <p>See also: <a href="http://code.google.com/p/jsoar/wiki/JSoarUsersGuide">JSoar User Guide</a>
 *
 * <p>Generally, <b>all</b> access to agent data structures, i.e. the {@link Agent} instance should
 * be marshaled through the {@link #executeInternal(Runnable)} methods. Note however, that many
 * public Soar interfaces, or at least parts of them, are immutable, so it is safe to access them
 * from other threads.
 *
 * <p>{@code ThreadedAgent} is an {@link Adaptable} where {@link #getAdapter(Class)} has the same
 * behavior as in {@link Agent}.
 *
 * <p>This object sets the {@link SoarProperties#IS_RUNNING} property appropriately and fires events
 * when its state changes.
 *
 * <p>This object installs {@code run} and {@code stop-soar} commands as well as the {@code wait}
 * RHS function.
 *
 * @author ray
...
public class ThreadedAgent extends AbstractAdaptable implements AgentRunController {
  ...
  private final PropertyProvider<Boolean> agentRunningProvider =
      new PropertyProvider<Boolean>() {
        ...
        public Boolean set(Boolean value) {
          throw new UnsupportedOperationException(
              SoarProperties.IS_RUNNING.getName() + " property is read-only");
        }
      };
  ...
  /**
   * Create a new {@link Agent} and automatically wrap it with a ThreadedAgent. This method also
   * initializes the agent and starts its thread.
   *
   ...
   * }</pre>
   *
   * <p>Note that, unlike a normal call to {@link #initialize()} this method will not return until
   * initialization has completed. Otherwise, strange situations can arise if the agent is returned
   * before initialization is complete.
   *
   * <p>A {@link ThreadedAgentAttachedEvent} will be fired before the method returns.
   *
  ...
  /**
   * If there is a ThreadedAgent already attached to the given agent, return it.
   *
   * @param agent the agent
   * @return the ThreadedAgent proxy, or <code>null</code> if none is currently attached.
   */
  ...
  /**
   * Attach a threaded wrapper to the given agent. The returned object must be initialized.
   *
  ...
  /**
   * Returns the event manager used for global ThreadedAgent events such as {@link
   * ThreadedAgentAttachedEvent}.
   *
  ...
  /**
   * Initialize this object and the underlying agent. This returns immediately. If want to wait, use
   * a completion handler like the one in {@link ThreadedAgentManager#create}
   *
   * @return this
   */
   ...
   * a completion handler like the one in {@link ThreadedAgentManager#create}
   *
   * @param done if not <code>null</code> this handler is called after the agent is initialized.
   * @return this
  ...
  /**
   * Detach this object from the agent. This method will stop the agent thread, and wait for it to
   * exit before proceeding. After being detached, this object may not be used again. The underyling
   * agent remains available for use.
   *
   * <p>A {@link ThreadedAgentDetachedEvent} will be fired before the method returns.
   *
   * @see #dispose()
   */
  public void detach() {
  ...
  /**
   * Run the agent in a separate thread. This method returns immediately. If the agent is already
   * running, the command is ignored. When the agent stops a {@link StopEvent} event will be fired.
   *
  ...
  /**
   * Start the agent running. The agent will run until {@link #stop()} is called or it halts for
   * some reason. When the agent starts running (at some point in the future), a {@link StartEvent}
   * event will be fired. When the agent stops a {@link StopEvent} event will be fired.
   */
  ...
  /**
   * Stop the agent running at some point in the future. When the agent finally stops, a {@link
   * StopEvent} event will be fired.
   */
  public void stop() {
    executeInternal(
        new Runnable() {
  ...
  /**
   * Schedule loading a rete in the agent thread and return immediately. This will not deadlock.
   *
   * <p>If you don't need a completion handler, it is simpler to use {@code loadProductions(Object
   * rete)}.
   *
   * @param source the source to load
   * @param finish called after the callable is executed. Ignored if <code>null</code>.
   * @throws RuntimeException if an exception is thrown while waiting for the result.
   */
  ...
  /**
   * Schedule loading productions in the agent thread and return immediately. This will not
   * deadlock.
   *
   * @param source the source to load
   * @throws RuntimeException if an exception is thrown while waiting for the result.
   ...
   *
   * <p>Note that in almost all cases, {@link #loadProductions(Object, CompletionHandler)} or {@link
   * #loadProductions(Object)} is what you want. This method is very prone to deadlocks if the
   * thread that is calling it (e.g. the Swing UI thread) handles events from the agent.
   */
  @Deprecated
   ...
   * Schedule loading a rete in the agent thread and return immediately. This will not deadlock.
   *
   * <p>If you don't need a completion handler, it is simpler to use {@code loadRete(Object rete)}.
   *
   * @param rete the rete to load
   * @param finish called after the callable is executed. Ignored if <code>null</code>.
   * @throws RuntimeException if an exception is thrown while waiting for the result.
   ...
   *
   * <p>Note that in almost all cases, {@link #loadRete(Object, CompletionHandler)} or {@link
   * #loadRete(Object)} is what you want. This method is very prone to deadlocks if the thread that
   * is calling it (e.g. the Swing UI thread) handles events from the agent.
   */
  @Deprecated
  ...
  /**
   * Schedule the given runnable for execution in the agent thread and return immediately. This is
   * the correct way to access the agent in a thread-safe manner without deadlocks.
   *
   * <p>If you need to handle a return type, see {@code execute(final Callable<V> callable, final
   * CompletionHandler<V> finish)}.
   *
   * <p>Note that your runnable will need to handle checked exceptions when using this. Using the
   * callable version handles that for you. This is a limitation of the Runnable interface.
   *
   * @param runnable the runnable to run
   * @throws RuntimeException if an exception is thrown while waiting for the result.
   */
  ...
  /**
   * Schedule the given callable for execution in the agent thread and return immediately. This is
   * the correct way to access the agent in a thread-safe manner without deadlocks.
   *
   * <p>If you don't need a return type with a completion handler, it is simpler to use {@code
   ...
   *
   * <p>Note that in almost all cases, {@link #execute(Callable, CompletionHandler)} is what you
   * want. This method is very prone to deadlocks if the thread that is calling it (e.g. the Swing
   * UI thread) handles events from the agent.
   *
   ...
   */
  public <V> V executeAndWait(final Callable<V> callable, long timeout, TimeUnit timeUnit)
   ...
   */
  @Deprecated
  public <V> V executeAndWait(final Callable<V> callable)
   ...
   *
   * <p>Note that in almost all cases, {@link #execute(Runnable)} is what you want. This method is
   * very prone to deadlocks if the thread that is calling it (e.g. the Swing UI thread) handles
   ...
   */
  @Deprecated
  public void executeAndWait(final Runnable runnable, long timeout, TimeUnit timeUnit)
      throws InterruptedException, ExecutionException, TimeoutException {
    executeAndWait(
        () -> {
          runnable.run();
  ...
  private void processUncaughtException(Exception e) {
    try {
      ...
      agent
          .getPrinter()
          .error(
              "Agent thread caught unhandled exception: "
