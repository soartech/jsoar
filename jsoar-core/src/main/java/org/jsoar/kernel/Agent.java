...
/**
 * This is a the base agent object in JSoar. It is a "raw" agent which roughly corresponds to the
 * {@code agent_struct} in CSoar (see agent.h).
 *
 * <p>Note that this is a low-level class and in most cases, you probably want {@link
 * ThreadedAgent}.
 *
 * <p>See also: <a href="http://code.google.com/p/jsoar/wiki/JSoarUsersGuide">JSoar User Guide</a>
 *
 * <p>The {@code Agent} object attempts to provide a simple interface for typical Soar integration
 * cases (load productions, run, do I/O, etc). However, for more involved development, it may be
 * necessary to access data structures that are not exposed through the public interface. In this
 * case, {@code Agent} makes use of the {@code Adaptables} framework. Client code can ask for
 * internal interfaces using the {@link Adaptables#adapt(Object, Class)} on the agent object. For
 ...
 * }</pre>
 *
 * Note that this access pattern should generally be assumed to be unstable. It is useful for
 * research programming, but for general development should be avoided. If a particular value in a
 * module need to be exposed, consider exposing it through the property system.
 *
...
public class Agent extends AbstractAdaptable implements AgentRunController {
  ...
  @NonNull @Getter private final Explain explain = new Explain(this);
  ...
  private final ExecutionTimer totalCpuTimer =
      DefaultExecutionTimer.newInstance().setName("Total CPU time");
  /** agent.h:482:total_kernel_time */
  private final ExecutionTimer totalKernelTimer =
      DefaultExecutionTimer.newInstance().setName("Total kernel time");

  private boolean initialized = false;

  /**
   * The objects in this list are retrievable by requesting them, by class, using the adaptables
   * framework, i.e. {@link #getAdapter(Class)}
   *
   * <p>For example:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * This allows these fields to be private (not cluttering up the public interface) while still
   * making them accessible to the spaghetti that is the kernel at the moment. Hopefully, it will
   * become less necessary as the system is cleaned up.
   */
  ...
  /**
   * Construct a new agent with a generated name. Also initializes the agent.
   *
  ...
  /**
   * Construct a new agent with a generated name. Also lets you explicitly state whether to
  ...
  /**
   * Construct a new agent with the given name. Also initializes the agent.
   *
   * @see #Agent(String, boolean)
   */
  ...
  /**
   * Construct a new agent with the given name. Also lets you explicitly state whether to initialize
   * the agent
   *
   * @param name the name. If {@code null}, a new name is generated.
   * @param initializeAgent lets you explicitly choose whether to initialize the agent.
   */
  ...
  /**
   * Set the name of the agent. This can also be set with property {@link SoarProperties#NAME}.
   *
  ...
  /**
   * Must be called before the agent is run. This is separate from the constructor to give client
   * code the change to register callbacks, modify the trace level or printer, etc before the agent
   * is initialized, which may initiate these actions.
   *
   ...
   *
   * <p>If an interpreter has not been set yet, a default one will be created the first time this
   * method is called.
   *
   * <p>This method may be called from any thread, but care should be taken when calling the methods
   * of the returned interpreter.
   *
  ...
  public SoarCommandInterpreter getInterpreter() {
    synchronized (this) {
      if (interp == null) {
        ...
        logger.info(
            "Current command interpreter is '{}' : '{}'", interp.getName(), interp.getClass());
        final var DEFAULT_ALIASES = "/org/jsoar/kernel/commands/aliases";
        try {
          interp.source(Agent.class.getResource(DEFAULT_ALIASES));
        } catch (SoarException e) {
          logger.error(
              "Failed to load default aliases from '" + DEFAULT_ALIASES + "': " + e.getMessage(),
              e);
        }
      }
      return interp;
  ...
  public void setInterpreter(SoarCommandInterpreter interp) {
    synchronized (this) {
      ...
      this.interp = interp;
      logger.info(
          "Current command interpreter is '{}'", interp != null ? interp.getClass() : "none");
    }
  }

  /**
   * Returns the property manager for this agent. All agent configuration (waitsnc, learn, etc)
   * should be performed programmatically through the property manager
   *
  ...
  /**
   * Returns the agents RHS function manager. Use this interface to register new RHS functions.
   *
  ...
  /**
   * Returns the agent's event manager. Use this interface to register for events generated by the
   * agent.
   *
  ...
  /**
   * Returns the agent's production manager. Use this interface to access loaded productions, load
   * new productions, or excise existing productions.
   *
  ...
  /**
   * Returns this agent's symbol factory. The symbol factory is the interface to use to create new
   * symbols or find existing ones.
   *
  ...
  /**
   * utilities.cpp:132:get_context_var_info
   *
  ...
  /**
   * @return the current goal stack as a list of goal identifiers. The caller may modify the list
  ...
  /**
   * @return the agent's random number generator. It is safe to call setSeed() on the returned
   *     generator.
   */
  ...
  public void runFor(long n, RunType runType) {
    ...
    if ((runType == RunType.DECISIONS)
        && (this.decisionCycle.current_phase.get() != getStopPhase() && (n > 0))) {
      n--;
    }
  ...
  /**
   * Run this agent forever, i.e. until an interrupt or halt. The agent is run in the current
   * thread.
   */
  ...
  /**
   * Checks that the agent has been initialized and throws an exception if not.
   *
  ...
  /**
   * Request that the agent stop, i.e. exit the active call to {@link #runFor(long, RunType)} of
   * {@link #runForever()}.
   *
   * <p>This method is not thread safe. It must be called from the same thread that the agent is
   * running in.
   */
  public void stop() {
  ...
  /** @return the reason the agent stopped, or <code>null</code> if the agent has not stopped. */
  ...
  /**
   * Return {@code true} if the given wme is currently in the rete, i.e. is in working memory.
   *
   * <p><em>This method should only be called from the agent thread</em>
   *
  ...
  private void installDefaultTraceFormats() {
    ...
    traceFormats.add_trace_format(
        false, TraceFormatRestriction.FOR_ANYTHING_TF, null, "%id %ifdef[(%v[name])]");
    traceFormats.add_trace_format(
        false,
        TraceFormatRestriction.FOR_STATES_TF,
        null,
    ...
    traceFormats.add_trace_format(
        true, TraceFormatRestriction.FOR_STATES_TF, null, "%right[6,%dc]: %rsd[   ]==>S: %cs");
    traceFormats.add_trace_format(
        true, TraceFormatRestriction.FOR_OPERATORS_TF, null, "%right[6,%dc]: %rsd[   ]   O: %co");
  }
