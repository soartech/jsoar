...
public class SoarSettingsCommand extends PicocliSoarCommand
{
    ...
    @Command(name = "soar", description = "Commands and settings related to running Soar", subcommands = { HelpCommand.class,
    ...
    @Command(name = "max-elaborations", description = "Maximum elaboration in a decision cycle", subcommands = { HelpCommand.class })
    ...
    static public class StopPhase implements Runnable
    {
        ...
        public void run()
        {
            if(phase == null)
            {
                CommandPhase currentPhase = CommandPhase.values()[parent.agent.getProperties().get(SoarProperties.STOP_PHASE).ordinal()];
    ...
    static public class Timers implements Runnable
    {
        ...
        @Option(names = { "on", "-e", "--on", "--enable" }, defaultValue = "false", description = "Enables timers")
        boolean enable;
        
        @Option(names = { "off", "-d", "--off", "--disable" }, defaultValue = "false", description = "Disables timers")
        boolean disable;
    ...
    @Command(name = "wait-snc", description = "Wait instead of impasse after state-no-change", subcommands = { HelpCommand.class })
    static public class WaitSNC implements Runnable
    {
        @ParentCommand
        Soar parent; // injected by picocli
        
        @Option(names = { "on", "-e", "--on", "--enable" }, defaultValue = "false", description = "Enables wait-snc")
        boolean enable;
        
        @Option(names = { "off", "-d", "--off", "--disable" }, defaultValue = "false", description = "Disables wait-snc")
        boolean disable;
