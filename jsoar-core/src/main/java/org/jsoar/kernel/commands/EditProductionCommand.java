...
public class EditProductionCommand extends PicocliSoarCommand {
  ...
  @Command(
      ...
      description = "Opens the given production in a text editor",
      subcommands = {HelpCommand.class})
  public static class EditProduction implements Runnable {
    private Agent agent;
    ...
    public void run() {
      ...
      if (file == null || file.length() == 0) {
        agent
            .getPrinter()
            .startNewLine()
            .print("Don't know source " + "location of production '" + prodName + "'");
        return;
      }
      if (FileTools.asUrl(file) != null) {
        agent
            .getPrinter()
            .startNewLine()
            .print("Don't know how to " + "edit productions loaded from URLs: " + file);
        return;
      }
