...
public class SaveCommand extends PicocliSoarCommand {
  ...
  @Command(
      name = "save",
      description = "Saves a rete-net",
      subcommands = {HelpCommand.class, ReteNet.class})
  ...
  @Command(
      name = "rete-net",
      description = "Serializes an agent's productions to a binary file",
      subcommands = {HelpCommand.class})
  public static class ReteNet implements Runnable {
    @ParentCommand Save parent; // injected by picocli

    @Option(
        names = {"-s", "--save"},
        arity = "1",
        description = "File name to save rete-net to")
    String fileName;
      ...
      try {
        ...
        if (os != null) {
          try {
            ...
            parent.agent.getPrinter().startNewLine().print("IO error while closing the file.");
          }
        }
      }
