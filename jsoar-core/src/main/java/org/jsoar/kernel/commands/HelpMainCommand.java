...
/**
 * This is the implementation of the "help" command. The class name contains the word "main" so it
 * doesn't interfere with picocli's HelpCommand class.
 *
...
public class HelpMainCommand extends PicocliSoarCommand {
  ...
  public static class Help implements Runnable {
    ...
    @Unmatched private String[] remainder;
    ...
    public void run() {
      if (command == null) {
        agent
            .getPrinter()
            .print("JSoar " + JSoarVersion.getInstance().getVersion() + " Command List:\n\r");
        ...
        try {
          // we're using a ParsedCommand in case they passed in arguments -- this allows us to get
          // the base command so we can ignore args, like csoar
          ParsedCommand parsedCommand =
              agent
                  .getInterpreter()
                  .getParsedCommand(command, DefaultSourceLocation.newBuilder().build());
          // we're using getCommand, because it resolves aliases
          SoarCommand soarCommand =
              agent
                  .getInterpreter()
                  .getCommand(parsedCommand.getArgs().get(0), parsedCommand.getLocation());
          String result =
              soarCommand.execute(
                  new DefaultSoarCommandContext(null),
                  new String[] {parsedCommand.getArgs().get(0), "help"});
