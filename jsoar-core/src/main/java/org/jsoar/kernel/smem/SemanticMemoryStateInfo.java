...
/**
 * SMem info associated with a state. This structure isn't accessed in a tight loop so we can just
 * maintain a map in {@link SemanticMemory}
 *
 * <p>semantic_memory.h:266:smem_data_struct decide.cpp:2273:create_new_context (initialization)
 *
 * @author ray
 */
public class SemanticMemoryStateInfo {
  ...
  public SemanticMemoryStateInfo(DefaultSemanticMemory smem, WorkingMemory wm, IdentifierImpl id) {
    ...
    smem_header = smem.symbols.createIdentifier('S', id.getLevel());
    SoarModule.add_module_wme(wm, id, smem.predefinedSyms.smem_sym, smem_header);
    smem_cmd_header = smem.symbols.createIdentifier('C', id.getLevel());
    SoarModule.add_module_wme(wm, smem_header, smem.predefinedSyms.smem_sym_cmd, smem_cmd_header);
    smem_result_header = smem.symbols.createIdentifier('R', id.getLevel());
