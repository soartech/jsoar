...
public class ChunkConditionSet {
  ...
  final List<ListHead<ChunkCondition>> table =
      new ArrayList<ListHead<ChunkCondition>>(CHUNK_COND_HASH_TABLE_SIZE);
  ...
  /** chunk.cpp:356:add_to_chunk_cond_set */
  boolean add_to_chunk_cond_set(ChunkCondition new_cc) {
    ...
    for (old = bucket.first; old != null; old = old.next)
      if (old.item.hash_value == new_cc.hash_value)
        if (Condition.conditions_are_equal(old.item.cond, new_cc.cond)) break;
  ...
  /**
   * chunk.cpp:376:remove_from_chunk_cond_set
   *
   * @param cc
   */
