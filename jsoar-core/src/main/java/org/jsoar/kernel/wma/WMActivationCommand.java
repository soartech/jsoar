...
public class WMActivationCommand extends PicocliSoarCommand {
  ...
  public WMActivationCommand(Agent agent) {
    super(
        ...
        new WMA(
            Adaptables.require(
                WMActivationCommand.class, agent, DefaultWorkingMemoryActivation.class),
  ...
  @Command(
      name = "wm",
      description = "Commands and settings related to working memory",
      subcommands = {HelpCommand.class, WMActivationCommand.Activation.class})
  public static class WMA implements Runnable {
    ...
    public void run() {
      agent
          .getPrinter()
          .startNewLine()
          .print("The 'wm' commands interact " + "with working memory contents and settings.");
    }
  }
  ...
  @Command(
      name = "activation",
      description =
          "Changes the behavior of and displays " + "information about working memory activation",
      subcommands = {HelpCommand.class})
  public static class Activation implements Runnable {
    @ParentCommand WMA parent; // injected by picocli

    @Option(
        names = {"-g", "--get"},
        description = "Print current parameter setting")
    ...
    @Option(
        names = {"-S", "--stats"},
        description = "Print statistic summary or specific statistic")
    boolean printStats = false;

    @Option(
        names = {"-t", "--timers"},
        description = "Print timer summary or specific timer")
    boolean printTimer = false;

    @Option(
        names = {"-h", "--history"},
        description = "Print reference history of a WME")
    boolean printHistory = false;

    @Parameters(
        arity = "0..1",
        description = "Set value, specific statistic, specific timer, or WME timetag")
    String param = null;
    ...
    private void doGet(String param) {
      ...
      final PropertyKey<?> key =
          DefaultWorkingMemoryActivationParams.getProperty(
      ...
      parent
          ...
          .getPrinter()
          .startNewLine()
          .print(parent.wma.getParams().getProperties().get(key).toString());
    }
    ...
    private void doSet(String param, String value) {
      ...
      try {
        if (value == null) {
          ...
          props.set(
              DefaultWorkingMemoryActivationParams.ACTIVATION, ActivationChoices.valueOf(value));
          ...
          parent
              ...
              .getPrinter()
              .startNewLine()
              .print("This parameter is protected while WMA is on.");
          ...
          props.set(
              DefaultWorkingMemoryActivationParams.PETROV_APPROX,
    ...
    private void doStats(String param) {
      ...
      if (param == null) {
        ...
        final PropertyKey<?> key =
            DefaultWorkingMemoryActivationStats.getProperty(
                parent.wma.getParams().getProperties(), param);
        if (key == null) {
