...
public class EpmemCommand extends PicocliSoarCommand {
  ...
  @Command(
      name = "epmem",
      description = "Controls the behavior of episodic memory",
  ...
  public static class EpmemC implements Runnable {
    ...
    @Option(
        names = {"-p", "--print"},
        description = "Prints episode in a user-readable format")
    String printEpisode = null;

    @Option(
        names = {"-r", "--reinit"},
        description = "Re-initializes episodic memory")
    boolean reinit = false;

    @Option(
        names = {"-b", "--backup"},
        arity = "1..*",
        description = "Creates a backup of the episodic database on disk")
    String[] backupFileName = null;

    @Option(
        names = {"-a", "--add"},
        description = "Adds knowledge to episodic memory")
    String knowledgeToAdd = null;

    @Parameters(
        arity = "0..1",
        description = "The new value of the parameter; " + "or specific statistic to print")
    String param = null;
    ...
    private String doSet(String paramToSet, String value) {
      ...
      try {
        switch (paramToSet) {
          ...
          case "graph-match-ordering":
            props.set(DefaultEpisodicMemoryParams.GM_ORDERING, GmOrderingChoices.valueOf(value));
            return "Set graph-match-ordering to " + GmOrderingChoices.valueOf(value);
          case "balance":
          ...
          case "database":
            if (value.equals("memory")) {
              ...
              throw new ParameterException(
                  spec.commandLine(), "Invalid value for EpMem database parameter");
            }
          default:
    ...
    private String doGet(String paramToGet) {
      final PropertyKey<?> key =
          DefaultEpisodicMemoryParams.getProperty(epmem.getParams().getProperties(), paramToGet);
      if (key == null) {
        if (paramToGet.equals("database")) {
          PropertyKey<?> pathProperty =
              DefaultEpisodicMemoryParams.getProperty(epmem.getParams().getProperties(), "path");
          if (pathProperty == null) {
            agent.getPrinter().startNewLine().print("Path is null.");
            return "";
          }

          var path = epmem.getParams().getProperties().get(pathProperty).toString();
          ...
          return exclusions.stream().map(SymbolImpl::toString).collect(Collectors.joining(","));
        } else if (paramToGet.equals("inclusions")) {
          Set<SymbolImpl> inclusions = epmem.getParams().inclusions;
          return inclusions.stream().map(SymbolImpl::toString).collect(Collectors.joining(","));
        } else {
          throw new ParameterException(
              spec.commandLine(), "Unknown parameter '" + paramToGet + "'");
        }
      }
    ...
    private String doStats(String statToPrint) {
      ...
      if (statToPrint == null) {
        ...
        try {
          String database =
              epmem.getDatabase().getConnection().getMetaData().getDatabaseProductName();
          String version =
              epmem.getDatabase().getConnection().getMetaData().getDatabaseProductVersion();
        ...
        long pageCount;
        long pageSize;
        try (var s = epmem.getDatabase().getConnection().createStatement()) {
        ...
        pw.printf(
            PrintHelper.generateItem(
                "Memory Usage:", (stats.mem_usage.get() / 1024.0) + " KB", 40));
        ...
        pw.printf(
            PrintHelper.generateItem(
                "Last Graph Match Attempts:", stats.last_graph_matches.get(), 40));
        pw.printf(PrintHelper.generateItem("Episodes Considered:", stats.considered.get(), 40));
        pw.printf(
            PrintHelper.generateItem("Last Episodes Considered:", stats.last_considered.get(), 40));
      } else {
        final PropertyKey<?> key =
            DefaultEpisodicMemoryStats.getProperty(epmem.getParams().getProperties(), statToPrint);
        if (key == null) {
    ...
    private String doBackup(String[] fileName) {
      ...
      var dbFile = Arrays.stream(fileName).collect(Collectors.joining(" "));
      dbFile = dbFile.trim();
