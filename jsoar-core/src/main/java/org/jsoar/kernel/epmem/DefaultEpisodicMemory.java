 ...
 *
 * <p>The smem_data_struct that was added to every identifier in CSoar is instead maintained in a
 * map from id to {@link EpisodicMemoryStateInfo} in this class.
 *
...
public class DefaultEpisodicMemory implements EpisodicMemory {
  ...
  /** episodic_memory.h:55:EPMEM_NODE_POS int, so it can be switched on */
  public static final int EPMEM_NODE_POS = 0;

  /** episodic_memory.h:56:EPMEM_NODE_NEG int, so it can be switched on */
  ...
  private Map<Long, Map<Long, LinkedList<EpisodicMemoryIdPair>>> /* epmem_parent_id_pool */
      epmem_id_repository;

  /** agent.h:epmem_id_replacement */
  private Map<Long, LinkedList<EpisodicMemoryIdPair>> /* epmem_return_id_pool */
      epmem_id_replacement;

  /** agent.h:epmem_id_ref_counts */
  private Map<Long, Set<WmeImpl>> /* epmem_id_ref_counter */ epmem_id_ref_counts;

  /*
   * List of WME Identifiers that should be removed from Episodic Memory
   */
  ...
  private final epmem_rit_state[] epmem_rit_state_graph =
      new epmem_rit_state[] {new epmem_rit_state(), new epmem_rit_state()};
  ...
  /**
   * This is called when the agent is initialized. This code here in CSoar is usually run at agent
   * creation.
   */
  public void initialize() {
    ...
    recognitionMemory =
        Adaptables.require(DefaultEpisodicMemory.class, context, RecognitionMemory.class);
    ...
    final PropertyManager properties =
        Adaptables.require(DefaultEpisodicMemory.class, context, PropertyManager.class);
    ...
    epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].offset.var_key =
        epmem_variable_key.var_rit_offset_1;
    // epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].leftroot.stat = rit_left_root_1;
    epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].leftroot.var_key =
        epmem_variable_key.var_rit_leftroot_1;
    // epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].rightroot.stat = rit_right_root_1;
    epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].rightroot.var_key =
        epmem_variable_key.var_rit_rightroot_1;
    // epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].minstep.stat = rit_min_step_1;
    epmem_rit_state_graph[EPMEM_RIT_STATE_NODE].minstep.var_key =
        epmem_variable_key.var_rit_minstep_1;

    // epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].offset.stat = rit_offset_2;
    epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].offset.var_key =
        epmem_variable_key.var_rit_offset_2;
    // epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].leftroot.stat = rit_left_root_2;
    epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].leftroot.var_key =
        epmem_variable_key.var_rit_leftroot_2;
    // epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].rightroot.stat = rit_right_root_2;
    epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].rightroot.var_key =
        epmem_variable_key.var_rit_rightroot_2;
    // epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].minstep.stat = rit_min_step_2;
    epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE].minstep.var_key =
        epmem_variable_key.var_rit_minstep_2;
   ...
   * Opens the SQLite database and performs all initialization required for the current mode
   *
   * <p>The readonly param should only be used in experimentation where you don't want to alter
  ...
  /**
   * Extracted from epmem_init_db(). Take performance settings and apply then to the current
   * database.
   *
  ...
  private void applyDatabasePerformanceOptions() throws SQLException, SoarException, IOException {
    ...
    if (params.driver.get().equals("org.sqlite.JDBC")) {
      ...
      try (var s = db.getConnection().createStatement()) {
        s.execute("PRAGMA main.cache_size = " + cacheSize);
      }
    }
    ...
    if (params.optimization.get() == Optimization.performance) {
      ...
      try (InputStream perfStream = getClass().getResourceAsStream(perfResource)) {
        final String fullPath =
            "/" + getClass().getCanonicalName().replace('.', '/') + "/" + perfResource;
        if (perfStream != null) {
          log.info("Applying performance settings from '{}'.", fullPath);
          JdbcTools.executeSql(db.getConnection(), perfStream, null /* no filter */);
        } else {
          log.warn(
              "Could not find performance resource at '{}'. No performance settings applied.",
              fullPath);
        }
      }
    }
    ...
    if (params.driver.get().equals("org.sqlite.JDBC")) {
      ...
      switch (pageSize) {
        case page_16k:
          pageSizeLong = 16 * KILOBYTE;
          break;
        case page_1k:
          pageSizeLong = KILOBYTE;
          break;
        case page_2k:
          pageSizeLong = 2 * KILOBYTE;
          break;
        case page_32k:
          pageSizeLong = 32 * KILOBYTE;
          break;
        case page_4k:
          pageSizeLong = 4 * KILOBYTE;
          break;
        case page_64k:
          pageSizeLong = 64 * KILOBYTE;
          break;
        case page_8k:
          pageSizeLong = 8 * KILOBYTE;
          break;
        default:
          break;
      }
  ...
  private void initMinMax(
      long time_max,
      PreparedStatement minmax_select,
      List<Boolean> minmax_max,
      List<Long> minmax_min)
      throws SQLException {
      ...
      while (rs.next()) {
        ...
        */
        if (rs.getInt(1) != 0) {
          ...
          // temp_q->column_int( 0 );
          var num_ids = rs.getInt(1);

          // Note that substituting resize for an add loop only works if the incoming lists are
          // empty. -ACN
  ...
  /**
   * Private method for epmem_init_db that throws SQLException, IOException so it can wrap in
   * SoarException and throw.
   *
   * <p>episodic_memory.cpp:1458:epmem_init_db
   */
  private void epmem_init_db_ex(boolean readonly /* = false */)
      throws SQLException, IOException, SoarException {
    ...
    final String jdbcUrl =
        URLDecoder.decode(params.protocol.get() + ":" + params.path.get(), StandardCharsets.UTF_8);
    final var connection = JdbcTools.connect(params.driver.get(), jdbcUrl);
    final DatabaseMetaData meta = connection.getMetaData();

    log.info(
        "Opened database '{}' with {}:{}", jdbcUrl, meta.getDriverName(), meta.getDriverVersion());
    ...
    if (!EpisodicMemoryDatabase.IN_MEMORY_PATH.equals(params.path.get())) {
      try (ResultSet result = db.get_schema_version.executeQuery()) {
        if (result.next()) {
          var schemaVersion = result.getString(1);
          if (!EpisodicMemoryDatabase.EPMEM_SCHEMA_VERSION.equals(schemaVersion)) {
            log.error(
                "Incorrect database version, switching to memory.  Found version: {}",
                schemaVersion);
    ...
    epmem_id_repository.put(EPMEM_NODEID_ROOT, new LinkedHashMap<>());
    {
    ...
    // get/set RIT variables
    {
      ...
      for (int i = EPMEM_RIT_STATE_NODE; i <= EPMEM_RIT_STATE_EDGE; i++) {
        ...
        if (epmem_get_variable(epmem_rit_state_graph[i].offset.var_key, var_val)) {
          ...
          epmem_set_variable(
              epmem_rit_state_graph[i].offset.var_key, epmem_rit_state_graph[i].offset.stat);
        }
        ...
        if (epmem_get_variable(epmem_rit_state_graph[i].leftroot.var_key, var_val)) {
          ...
          epmem_set_variable(
              epmem_rit_state_graph[i].leftroot.var_key, epmem_rit_state_graph[i].leftroot.stat);
        }
        ...
        if (epmem_get_variable(epmem_rit_state_graph[i].rightroot.var_key, var_val)) {
          ...
          epmem_set_variable(
              epmem_rit_state_graph[i].rightroot.var_key, epmem_rit_state_graph[i].rightroot.stat);
        }
        // minstep
        if (epmem_get_variable(epmem_rit_state_graph[i].minstep.var_key, var_val)) {
          ...
          epmem_set_variable(
              epmem_rit_state_graph[i].minstep.var_key, epmem_rit_state_graph[i].minstep.stat);
        }
      }
    }
    ...
    if (!readonly) {
      ...
      final var now_select = new PreparedStatement[] {db.now_select_node, db.now_select_edge};
      final var now_add =
          new PreparedStatement[] {
            db.add_epmem_wmes_constant_point, db.add_epmem_wmes_identifier_point
          };
      final var now_delete = new PreparedStatement[] {db.now_delete_node, db.now_delete_edge};
      ...
      for (int i = EPMEM_RIT_STATE_NODE; i <= EPMEM_RIT_STATE_EDGE; i++) {
        ...
        try (ResultSet rs = temp_q2.executeQuery()) {
          ...
          while (rs.next()) {
            ...
            var range_start = rs.getLong(1 + 1);

            // point
            if (range_start == time_last) {
              temp_q.setLong(1, rs.getLong(1));
              temp_q.executeUpdate(/* soar_module::op_reinit */ );
            } else {
              epmem_rit_insert_interval(
                  range_start, time_last, rs.getLong(1), epmem_rit_state_graph[i]);
            }
          }
        }
    ...
    // get id pools
    {
      ...
      try (ResultSet rs = temp_q.executeQuery()) {
        ...
        while (rs.next()) {
          ...
          hp = epmem_id_repository.computeIfAbsent(parent_n_id, k -> Maps.newLinkedHashMap());

          ip = hp.computeIfAbsent(attributes_s_id, k -> Lists.newLinkedList());

          ip.addFirst(new EpisodicMemoryIdPair(child_n_id, wi_id));

          hp = epmem_id_repository.computeIfAbsent(child_n_id, k -> Maps.newLinkedHashMap());
        }
      }
    }
  ...
  private void epmem_rit_insert_interval(long lower, long upper, long id, epmem_rit_state rit_state)
      throws SQLException {
    // initialize offset
  ...
  /**
   * episodic_memory.cpp: 1068: int64_t epmem_rit_fork_node( int64_t lower, int64_t upper, bool /
   * *bounds_offset* /, int64_t *step_return, epmem_rit_state *rit_state )
   *
   * @param upper The C has an *int64_t here, but were just going to return it in the java
   * @return EpmemRitForkNodeResult In the C, there is a pass by reference int, so Java will
   *     simulate that with a return class.
   */
  private EpmemRitForkNodeResult epmem_rit_fork_node(
      long lower, long upper, epmem_rit_state rit_state) {
   ...
   */
  boolean epmem_get_variable(epmem_variable_key variable_id, ByRef<Long> variable_value)
      throws SQLException {
  ...
  private boolean epmem_expand_inclusions(SymbolImpl sym, Marker tc, Set<WmeImpl> wmesToKeep) {
    ...
    if (id != null && id.epmem_id != EPMEM_NODEID_BAD) {
      List<WmeImpl> wmes = getAllWmesOfId(id, tc);
    ...
    // perform storage
    {
        ...
        for (IdentifierImpl id_p : epmem_wme_adds) {
          ...
          if (id_p.epmem_id != EPMEM_NODEID_BAD) {
            ...
            while (!parent_syms.isEmpty()) {
              ...
              wmes = getAllWmesOfId(parent_sym, tc);
              if (!params.inclusions.isEmpty()) {
                ListIterator<WmeImpl> it = wmes.listIterator();
                while (it.hasNext()) {
                  if (!inclusion_wmes.contains(it.next())) it.remove();
                }
              }
              if (!wmes.isEmpty()) {
                _epmem_store_level(
      ...
      // all inserts
      {
        ...
        while (!epmem_node.isEmpty()) {
          ...
          // add NOW entry
          // id = ?, start_episode_id = ?
          db.add_epmem_wmes_constant_now.setLong(
              1, temp_node); // my_agent->epmem_stmts_graph->add_node_now->bind_int(
          // 1, (*temp_node) );
          db.add_epmem_wmes_constant_now.setLong(
              2, time_counter); // my_agent->epmem_stmts_graph->add_node_now->bind_int(
          // 2, time_counter
          // );
          db.add_epmem_wmes_constant_now.executeUpdate(
              /* soar_module::op_reinit */ ); // my_agent->epmem_stmts_graph->add_node_now->execute(
        ...
        while (!epmem_edge.isEmpty()) {
          ...
          // add NOW entry
          // id = ?, start_episode_id  = ?
          db.add_epmem_wmes_identifier_now.setLong(
              1, temp_node); // my_agent->epmem_stmts_graph->add_edge_now->bind_int(
          // 1, (*temp_node) );
          db.add_epmem_wmes_identifier_now.setLong(
              2, time_counter); // my_agent->epmem_stmts_graph->add_edge_now->bind_int(
          // 2, time_counter
          // );
          db.add_epmem_wmes_identifier_now.executeUpdate(
              /* soar_module::op_reinit */ ); // my_agent->epmem_stmts_graph->add_edge_now->execute(
          // soar_module::op_reinit
          // );
          ...
          db.update_epmem_wmes_identifier_last_episode_id.setLong(
              1, Long.MAX_VALUE); // my_agent->epmem_stmts_graph->update_edge_unique_last->bind_int(
          // 1,
          // LLONG_MAX
          // );
          db.update_epmem_wmes_identifier_last_episode_id.setLong(
              2, temp_node); // my_agent->epmem_stmts_graph->update_edge_unique_last->bind_int(
          // 2,
          // *temp_node
          // );
        ...
        for (Map.Entry<Long, Boolean> r : epmem_node_removals.entrySet()) {
          if (r.getValue()) {
            ...
            // node
              ...
              epmem_rit_insert_interval(
                  range_start, range_end, r.getKey(), epmem_rit_state_graph[EPMEM_RIT_STATE_NODE]);
            }
        ...
        for (Map.Entry<Long, Boolean> r : epmem_edge_removals.entrySet()) {
          if (r.getValue()) {
            ...
            // node
              ...
              epmem_rit_insert_interval(
                  range_start, range_end, r.getKey(), epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE]);
            }
      ...
      // update time wme on all states
      {
        ...
        while (state != null) {
          ...
          stateInfo.epmem_time_wme =
              soarModule.add_module_wme(
                  stateInfo.epmem_header, predefinedSyms.epmem_sym_present_id, my_time_sym);
  ...
  /**
   * This routine gets all WMEs rooted at an id. WMEs are also often called augmentations because
   * they “augment” the object, providing more detail about it.
   */
  private List<WmeImpl> getAllWmesOfId(final SymbolImpl sym, final Marker tc) {
    List<WmeImpl> wmesOfId = Lists.newLinkedList();

    // Working Memory Elements only exist for identifiers
    final IdentifierImpl id = sym.asIdentifier();
    if (id != null && id.tc_number != tc) {
      id.tc_number = tc;

      // add all impasse WMEs
      if (id.goalInfo != null) {
        wmesOfId.addAll(id.goalInfo.getImpasseWmes());
      }

      // add all input WMEs
      wmesOfId.addAll(id.getInputWmes());

      // add all regular WMEs
      for (Slot s = id.slots; s != null; s = s.next) {
        wmesOfId.addAll(s.getWmes());
        wmesOfId.addAll(s.getAcceptablePreferenceWmes());
      }
    }
   ...
   * *************************************************************************
   *
   * <p>_epmem_store_level
   *
   * <p>This function process the addition of "one level" of wme's rooted at the n_id specified by
   * parent_id into the working memory graph. It does not process any temporal information.
   *
   * <p>- w_b is the list of wme's it will process (until it hits w_e)
   *
   * <p>- can add syms and ids of augmentations to parent_syms and parent_ids queues so that they
   * can be processed during the next "level"
   *
   * <p>- The identifier symbol of each wme isn't set and is ignored. parent_id is used to specify
   * the root.
   *
   * <p>three cases for sharing ids amongst identifiers in two passes: 1. value known in phase one
   * (try reservation) 2. value unknown in phase one, but known at phase two (try assignment
   * adhering to constraint) 3. value unknown in phase one/two (if anything is left, unconstrained
   * assignment)
   *
   * <p>*****************************************************************************
   * episodic_memory.cpp:2454:{@code inline void _epmem_store_level( agent* my_agent, std::queue<
   * Symbol* >& parent_syms, std::queue< epmem_node_id >& parent_ids, tc_number tc,
   * epmem_wme_list::iterator w_b, epmem_wme_list::iterator w_e, epmem_node_id parent_id,
   * epmem_time_id time_counter, std::map< wme*, epmem_id_reservation* >& id_reservations, std::set<
   * Symbol* >& new_identifiers, std::queue< epmem_node_id >& epmem_node, std::queue< epmem_node_id
   * >& epmem_edge )}
   */
  void _epmem_store_level(
    ...
    // id repository
    LinkedList<EpisodicMemoryIdPair> /*epmem_id_pool*/ my_id_repo;
    ...
    for (WmeImpl wme : w_p) {
      ...
      if (wme.value.asIdentifier() != null
        ...
        if (wme.acceptable) {
          new_id_reservation =
              new EpisodicMemoryIdReservation(EPMEM_NODEID_BAD, EPMEM_HASH_ACCEPTABLE);
        } else {
          new_id_reservation =
              new EpisodicMemoryIdReservation(EPMEM_NODEID_BAD, epmem_temporal_hash(wme.attr));
        }
    ...
    for (WmeImpl wme : w_p) {
      ...
      if (wmeValueId != null) {
        ...
        value_known_apriori =
            (wmeValueId.epmem_id != EPMEM_NODEID_BAD && wmeValueId.epmem_valid == epmem_validation);
        ...
        if (wmeValueId.isLongTermIdentifier()) {
          ...
          if (!value_known_apriori) {
            ...
            if (wmeValueId.epmem_id == EPMEM_NODEID_BAD) {
              ...
              log.debug(
                  "   Adding new n_id and setting wme id to {} for VALUE which is lti {} {}\n",
                  wmeValueId.epmem_id,
                  wmeValueId.getNameLetter(),
                  wmeValueId.getNameNumber());

              // add repository
              Map<Long, LinkedList<EpisodicMemoryIdPair>> epmem_hashed_id_pool =
          ...
          if (value_known_apriori) {
            ...
            // case we need an exact match (case 2)
              ...
              // fprintf(stderr, "   No reservation found.  Looking for shared identifier at same
              // level.\n");
          ...
          // case 3
            ...
            if (my_id_repo != null) {
              ...
              if (!my_id_repo.isEmpty()) {
                ...
                while (it.hasNext()) {
                  final EpisodicMemoryIdPair pool_p = it.next();
                  // the ref set for this epmem_id may not be there if the pools were regenerated
                  // from a previous DB
                  // a non-existant ref set is the equivalent of a ref count of 0 (ie. an empty ref
                  // set)
        ...
        if (wme.epmem_id == EPMEM_NODEID_BAD) {
          ...
          // can't use value_known_apriori, since value may have been assigned (lti, id repository
          // via case 3)
          if (wmeValueId.epmem_id == EPMEM_NODEID_BAD
              || wmeValueId.epmem_valid != epmem_validation) {
            // update next id
            ...
            log.debug(
                "   Adding new n_id and setting wme id for VALUE to {} \n", wmeValueId.epmem_id);
        ...
        if (wme.epmem_id == EPMEM_NODEID_BAD || wme.epmem_valid != epmem_validation) {
          ...
          if (wme.epmem_id == EPMEM_NODEID_BAD) {
            ...
            log.debug(
                "   No duplicate wme found in epmem_wmes_constant.  Adding wme to table!!!!\n");
  ...
  /** episodic_memory.cpp:2373:void epmem_schedule_promotion( agent* my_agent, Symbol* id ) */
  ...
  /**
   * episodic_memory.cpp:2031:inline void _epmem_promote_id( agent* my_agent, Symbol* id,
   * epmem_time_id t )
   */
  ...
  ************************************************************************** */
  ...
  private void epmem_respond_to_cmd(boolean created_new_memory) throws SoarException, SQLException {
    ...
    while (state != null) {
      ...
      if (new_cue && wme_count != 0) {
        _epmem_respond_to_cmd_parse(
            cmds, good_cue, path, retrieve, next, previous, query, neg_query, filter, prohibit,
            before, after, currents, cue_wmes, store);
          ...
          // previous or next
            ...
            if (next.value != null) {
              epmem_install_memory(
                  state, epmem_next_episode(epmem_info.last_memory), meta_wmes, retrieval_wmes);

              // add one to the next stat
              stats.nexts.set(stats.nexts.get() + 1L);
            } else {
              epmem_install_memory(
                  state, epmem_previous_episode(epmem_info.last_memory), meta_wmes, retrieval_wmes);
  ...
  /**
   * episodic_memory.cpp:992:{@code inline void epmem_process_buffered_wmes( agent* my_agent,
   * Symbol* state, soar_module::wme_set& cue_wmes, soar_module::symbol_triple_list& meta_wmes,
  ...
  /**
   * episodic_memory.cpp:912:{@code inline void _epmem_process_buffered_wme_list( agent* my_agent,
   * Symbol* state, soar_module::wme_set& cue_wmes, soar_module::symbol_triple_list& my_list,
  ...
  private void _epmem_process_buffered_wme_list(
    ...
    var inst = SoarModule.make_fake_instantiation(state, cue_wmes, my_list);
    ...
    for (Preference pref = inst.preferences_generated; pref != null; pref = pref.inst_next) {
      ...
      // insert_at_head_of_dll( state->id.preferences_from_goal, pref, all_of_goal_next,
      // all_of_goal_prev );
      ...
      if (my_justification_list.value != null /*NIL*/) {
        ...
        for (Instantiation my_justification = my_justification_list.value;
          ...
          if (my_justification.in_ms) {
            my_justification.prod.instantiations =
                my_justification.insertAtHeadOfProdList(my_justification.prod.instantiations);
          }

          for (just_pref = my_justification.preferences_generated;
              just_pref != null /*NIL*/;
  ...
  private static class EpmemTriple implements Comparable<EpmemTriple> {
    ...
    public boolean equals(Object obj) {
      ...
      if (parent_n_id != other.parent_n_id) return false;
      if (child_n_id != other.child_n_id) return false;
      if (attribute_s_id != other.attribute_s_id) return false;
      return true;
    }
  }
  ...
  /**
   * episodic_memory.cpp:3869:{@code void epmem_process_query( agent *my_agent, Symbol *state,
   * Symbol *pos_query, Symbol *neg_query, epmem_time_list& prohibits, epmem_time_id before,
   * epmem_time_id after, epmem_symbol_set& currents, soar_module::wme_set& cue_wmes,
  ...
  private void epmem_process_query(
    ...
    Map<WmeImpl, EpmemLiteral> literal_cache = new LinkedHashMap<>();
    @SuppressWarnings("unchecked")
    Map<EpmemTriple, EpmemPEdge>[] pedge_caches = new Map[2];
    ...
    #else
    */
    // epmem_triple_uedge_map uedge_caches[2] = {epmem_triple_uedge_map(),
    // epmem_triple_uedge_map()};
    var uedge_caches = new SortedMap[2];
    ...
    var root_literal = new EpmemLiteral();
    ...
    if (level > 1) {
      // build the DNF graph while checking for leaf WMEs
      {
        ...
        */
        ...
        for (int query_type = EPMEM_NODE_POS; query_type <= EPMEM_NODE_NEG; query_type++) {
          ...
          List<WmeImpl> /*epmem_wme_list**/ children =
              getAllWmesOfId(query_root, DefaultMarker.create()); // get_new_tc_number(my_agent));
          // for each first level WME, build up a DNF
          for (WmeImpl wme_iter : children) {
            EpmemLiteral /*epmem_literal**/ child =
                epmem_build_dnf(
                    wme_iter,
                    literal_cache,
                    leaf_literals,
      ...
      */
      ...
      // create dummy edges and intervals
      {
        ...
        var triple = new EpmemTriple(EPMEM_NODEID_BAD, EPMEM_NODEID_BAD, EPMEM_NODEID_ROOT);
        var root_pedge = new EpmemPEdge();
        ...
        root_pedge.sql =
            db.pool_dummy.getCopy(); // my_agent->epmem_stmts_graph->pool_dummy->request();
        ...
        var root_interval = new EpmemInterval();
      ...
      while (pedge_pq.size() != 0 && current_episode > after) {
        ...
        while ((pedge_pq.size() != 0)
            && (pedge_pq.peek().time == next_edge || pedge_pq.peek().time >= current_episode)) {
          final EpmemPEdge pedge = pedge_pq.poll();
          final var triple = pedge.triple.copyEpmemTriple();
          ...
          log.debug(
              "  EDGE {}-{}-{}", triple.parent_n_id, triple.attribute_s_id, triple.child_n_id);

          // create queries for the unique edge children of this partial edge
          if (pedge.value_is_id != 0) {
            @SuppressWarnings("unused")
            var created = false;
            // for (epmem_literal_set::iterator literal_iter = pedge->literals.begin(); literal_iter
            // != pedge->literals.end(); literal_iter++) {
            for (EpmemLiteral literal : pedge.literals) {
              // for (epmem_literal_set::iterator child_iter = literal->children.begin(); child_iter
              // != literal->children.end(); child_iter++) {
              for (EpmemLiteral child_iter : literal.children) {
                created |=
                    epmem_register_pedges(
                        triple.child_n_id, child_iter, pedge_pq, after, pedge_caches, uedge_caches);
              }
            }
          }
          // Left in from C: todo what I want to do here is, if there is no children which leads to
          // a leaf, retract everything
          ...
          Map<EpmemTriple, EpmemUEdge> /*epmem_triple_uedge_map**/ uedge_cache =
              uedge_caches[pedge.value_is_id];
          EpmemUEdge uedge_iter = uedge_cache.get(triple);
          if (uedge_iter == null) {
            // create a uedge for this
            var uedge = new EpmemUEdge();
            ...
            var created = false;
            var /*int64_t*/ edge_id = pedge.sqlResults.getLong(1);
            long /*epmem_time_id*/ promo_time = EPMEM_MEMID_NONE;
            boolean is_lti =
                (pedge.value_is_id != 0
                    && pedge.triple.child_n_id != EPMEM_NODEID_BAD
                    && pedge.triple.child_n_id != EPMEM_NODEID_ROOT);
            if (is_lti) {
            ...
            for (var interval_type = EPMEM_RANGE_EP;
                interval_type <= EPMEM_RANGE_POINT;
                interval_type++) {
              for (var point_type = EPMEM_RANGE_START;
                  point_type <= EPMEM_RANGE_END;
                  point_type++) {
                /*
                ...
                */
                ...
                // try to find an existing query first; if none exist, allocate a new one from the
                // memory pools
                PreparedStatement interval_sql;
                if (is_lti) {
                  interval_sql =
                      db.pool_find_lti_queries[point_type][interval_type].getCopy(/*sql_timer*/ );
                } else {
                  interval_sql =
                      db.pool_find_interval_queries[pedge.value_is_id][point_type][interval_type]
                          .getCopy(/*sql_timer*/ );
                }
                // CK: both the JDBC and C drivers have an index of 1 for the leftmost parameter in
            ...
            if (created) {
              if (is_lti) {
                ...
                var start_interval = new EpmemInterval();
            ...
            uedge_iter.pedges.add(pedge);
            if (uedge_iter.activated && uedge_iter.activation_count == 1) {
          ...
          if (pedge.sql != null) {
            ...
            if (pedge.sqlResults == null) {
              pedge.sqlResults = pedge.sql.executeQuery();
            }
        ...
        while (interval_pq.size() != 0
            && interval_pq.peek().time > next_edge
            && current_episode > after) {
          ...
          while (interval_pq.size() != 0 && interval_pq.peek().time >= current_episode) {
            ...
            var triple = uedge.triple.copyEpmemTriple();

            log.debug(
                "  INTERVAL ({}): {}-{}-{}",
                interval.is_end_point != 0 ? "end" : "start",
                triple.parent_n_id,
                triple.attribute_s_id,
                triple.child_n_id);
            ...
            if (interval.is_end_point != 0) {
              ...
              if (uedge.activation_count == 1) {
                for (EpmemPEdge pedge_iter : uedge.pedges) {
                  for (EpmemLiteral lit_iter : pedge_iter.literals) {
                    ByRef<Double> curScoreRef = new ByRef<>(current_score);
                    ByRef<Long> curCardinalityRef = new ByRef<>(current_cardinality);
                    changed_score |=
                        epmem_satisfy_literal(
          ...
          next_episode = (Math.max(next_edge, next_interval));
          ...
          while (prohibits.size() != 0
              && current_episode > next_episode
              && current_episode == prohibits.get(prohibits.size() - 1)) {
            current_episode--;
            prohibits.remove(prohibits.size() - 1);
          }
          ...
          */
          ...
          if (current_episode > next_episode
              ...
              && (best_episode == EPMEM_MEMID_NONE
                  ...
                  || (do_graph_match && current_score == best_score && !best_graph_matched))) {
            var new_king = false;
            ...
            if (current_cardinality == perfect_cardinality) {
              ...
              if (do_graph_match) {
                if (gm_order == DefaultEpisodicMemoryParams.GmOrderingChoices.undefined) {
                  ...
                  gm_ordering.sort((a, b) -> (a.matches.size() < b.matches.size() ? -1 : 1));
                }
                ...
                @SuppressWarnings("unchecked")
                Map<Long /*epmem_node_id*/, SymbolImpl>[] /*epmem_node_symbol_map*/ bound_nodes =
                    (Map<Long, SymbolImpl>[]) new Map[2];
                ...
                graph_matched =
                    epmem_graph_match(
                        gm_ordering, gm_ordering.listIterator(), best_bindings, bound_nodes, 2);
      ...
      if (best_episode == EPMEM_MEMID_NONE) {
        ...
        SymbolImpl temp_sym;
        Map<Long /*epmem_node_id*/, SymbolImpl> /*epmem_id_mapping*/ node_map_map =
            new LinkedHashMap<>();
        Map<Long /*epmem_node_id*/, SymbolImpl> /*epmem_id_mapping*/ node_mem_map =
            new LinkedHashMap<>();
        // cue size
        ...
        epmem_buffer_add_wme(
            meta_wmes,
            epmem_info(state).epmem_result_header,
            predefinedSyms.epmem_sym_cue_size,
            temp_sym);
        ...
        epmem_buffer_add_wme(
            meta_wmes,
            epmem_info(state).epmem_result_header,
            predefinedSyms.epmem_sym_match_cardinality,
            temp_sym);
        // symbol_remove_ref(my_agent, temp_sym);
        // match score
        ...
        epmem_buffer_add_wme(
            meta_wmes,
            epmem_info(state).epmem_result_header,
            predefinedSyms.epmem_sym_match_score,
            temp_sym);
        // symbol_remove_ref(my_agent, temp_sym);
        ...
        epmem_buffer_add_wme(
            meta_wmes,
            epmem_info(state).epmem_result_header,
            predefinedSyms.epmem_sym_normalized_match_score,
            temp_sym);
        // symbol_remove_ref(my_agent, temp_sym);
        // status
        epmem_buffer_add_wme(
        ...
        if (do_graph_match) {
          ...
          epmem_buffer_add_wme(
              meta_wmes,
              epmem_info(state).epmem_result_header,
              predefinedSyms.epmem_sym_graph_match,
              temp_sym);
          // symbol_remove_ref(my_agent, temp_sym);
          ...
          if (best_graph_matched) {
            ...
            epmem_buffer_add_wme(
                meta_wmes,
                epmem_info(state).epmem_result_header,
                predefinedSyms.epmem_sym_graph_match_mapping,
                mapping);
            // symbol_remove_ref(my_agent, mapping);

            // for (epmem_literal_node_pair_map::iterator iter = best_bindings.begin(); iter !=
            // best_bindings.end(); iter++) {
            for (Entry<EpmemLiteral, EpmemNodePair> iter : best_bindings.entrySet()) {
              if (iter.getKey().value_is_id != 0) {
                ...
                epmem_buffer_add_wme(
                    meta_wmes,
                    mapping,
                    predefinedSyms.epmem_sym_graph_match_mapping_node,
                    temp_sym);
                // symbol_remove_ref(my_agent, temp_sym);
                ...
                epmem_buffer_add_wme(
                    meta_wmes,
                    temp_sym,
                    predefinedSyms.epmem_sym_graph_match_mapping_cue,
                    iter.getKey().value_sym);
        ...
        if (level > 2) {
          epmem_install_memory(
              state, best_episode, meta_wmes, retrieval_wmes, node_mem_map, filter);
        }
        if (best_graph_matched) {
          // for (epmem_id_mapping::iterator iter = node_mem_map.begin(); iter !=
          // node_mem_map.end(); iter++) {
          for (Entry<Long, SymbolImpl> iter : node_mem_map.entrySet()) {
            ...
            if (map_iter != null && iter.getValue() != null) {
              epmem_buffer_add_wme(
                  meta_wmes, map_iter, predefinedSyms.epmem_sym_retrieved, iter.getValue());
            }
          }
        }
    ...
    for (int type = EPMEM_RIT_STATE_NODE; type <= EPMEM_RIT_STATE_EDGE; type++) {
      // for (epmem_triple_pedge_map::iterator iter = pedge_caches[type].begin(); iter !=
      // pedge_caches[type].end(); iter++) {
   ...
   *
   * <p>{@code bool epmem_graph_match( epmem_literal_deque::iterator& dnf_iter,
   * epmem_literal_deque::iterator& iter_end, epmem_literal_node_pair_map& bindings,
   * epmem_node_symbol_map bound_nodes[], agent* my_agent, int depth = 0 )}
   */
  private boolean epmem_graph_match(
    ...
    // #ifdef USE_MEM_POOL_ALLOCATORS
    //    epmem_node_set failed_parents = epmem_node_set(std::less<epmem_node_id>(),
    // soar_module::soar_memory_pool_allocator<epmem_node_id>(my_agent));
    //    epmem_node_set failed_children = epmem_node_set(std::less<epmem_node_id>(),
    // soar_module::soar_memory_pool_allocator<epmem_node_id>(my_agent));
    // #else
    ...
    for (EpmemNodePair match : literal.matches) {
      ...
      if (log.isTraceEnabled()) {
        var sb = "\t".repeat(Math.max(0, depth)) + "TRYING " + literal + " " + parent_n_id;
        log.trace(sb);
      }
      var relations_okay = true;
      // for all parents
      ...
      if (!relations_okay) {
        if (log.isTraceEnabled()) {
          String sb = "\t".repeat(Math.max(0, depth)) + "PARENT CONSTRAINT FAIL";
          log.trace(sb);
        }
      ...
      if (log.isTraceEnabled()) {
        String sb =
            "\t".repeat(Math.max(0, depth)) + literal + " " + parent_n_id + " " + child_n_id;
        log.trace(sb);
      }
      ...
      boolean list_satisfied =
          epmem_graph_match(dnf_array, next_iter, bindings, bound_nodes, depth + 1);
    ...
    // EPIC FAIL
    if (log.isTraceEnabled()) {
      String sb = "\t".repeat(Math.max(0, depth)) + "EPIC FAIL";
      log.trace(sb);
    }
    return false;
  }
  ...
  /**
   * {@code bool epmem_unsatisfy_literal( epmem_literal* literal, epmem_node_id parent,
   * epmem_node_id child, double& current_score, long int& current_cardinality,
  ...
  private boolean epmem_unsatisfy_literal(
    ...
    // epmem_node_pair_set::iterator lit_match_iter = literal->matches.find(std::make_pair(parent,
    // child));
    var enpair = new EpmemNodePair(parent, child);
    ...
    if (removedMatch) {
      ...
      if (value == 0) {
        ...
        if (literal.is_leaf) {
          ...
          var changed_score = false;
          var match = new EpmemSymbolNodePair(literal.value_sym, child);
          ...
          // if this literal is still satisfied, recurse on children who is matching on descendants
          // of this edge
          if (literal.matches.size() == 0) {
            ...
            var node_pair = new EpmemNodePair(child, EPMEM_NODEID_BAD);
            for (EpmemLiteral child_lit : literal.children) {
  ...
  /**
   * {@code bool epmem_satisfy_literal( epmem_literal* literal, epmem_node_id parent, epmem_node_id
   * child, double& current_score, long int& current_cardinality, epmem_symbol_node_pair_int_map&
   * symbol_node_count, epmem_triple_uedge_map uedge_caches[], epmem_symbol_int_map&
   * symbol_num_incoming)}
   */
  private boolean epmem_satisfy_literal(
    ...
    if (!parents_satisfied) {
      ...
      // since, by definition, if a node satisfies all incoming literals, all incoming literals are
      // satisfied
    ...
    if (parents_satisfied) {
      ...
      if (value == null) {
        ...
        if (literal.is_leaf) {
          ...
          var matchKey = new EpmemSymbolNodePair(literal.value_sym, child);
          symbol_node_count.merge(matchKey, 1, Integer::sum);
          // recurse over child literals
          for (EpmemLiteral child_lit : literal.children) {
            SortedMap<EpmemTriple, EpmemUEdge> uedge_cache =
                uedge_caches[(int) child_lit.value_is_id];
            var child_triple =
                new EpmemTriple(child, child_lit.attribute_s_id, child_lit.child_n_id);
            EpmemUEdge child_uedge;
            if (child_lit.child_n_id == EPMEM_NODEID_BAD) {
              ...
              for (Map.Entry<EpmemTriple, EpmemUEdge> entry : tailMap.entrySet()) {
                ...
                if (child_triple.parent_n_id != child
                    || child_triple.attribute_s_id != child_lit.attribute_s_id) {
                  break;
                }
                if (child_uedge.activated
                    && (!literal.is_current || child_uedge.activation_count == 1)) {
                  changed_score |=
                      epmem_satisfy_literal(
                          child_lit,
                          child_triple.parent_n_id,
                          child_triple.child_n_id,
                          current_score,
                          current_cardinality,
                          symbol_node_count,
                          uedge_caches,
  ...
  /**
   * episodic_memory.cpp:3539 {@code bool epmem_register_pedges( epmem_node_id parent,
   * epmem_literal* literal, epmem_pedge_pq& pedge_pq, epmem_time_id after, epmem_triple_pedge_map
   * pedge_caches[], epmem_triple_uedge_map uedge_caches[], agent* my_agent )}
   */
  private boolean epmem_register_pedges(
      long parent,
      EpmemLiteral literal,
      PriorityQueue<EpmemPEdge> pedge_pq,
      long after,
      ...
      SortedMap<EpmemTriple, EpmemUEdge>[] uedge_caches)
      throws SQLException {
    // we don't need to keep track of visited literals/nodes because the literals are guaranteed to
    // be acyclic
    // that is, the expansion to the literal's children will eventually bottom out
    // select the query
    var triple = new EpmemTriple(parent, literal.attribute_s_id, literal.child_n_id);
    ...
    if (child_pedge == null) {
      ...
      // soar_module::pooled_sqlite_statement* pedge_sql =
      // my_agent->epmem_stmts_graph->pool_find_edge_queries[is_edge][has_value]->request(my_agent->epmem_timers->query_sql_edge);
      ...
      if (!child_pedge.literals.contains(literal)) {
        ...
        // if the literal is an edge with no specified value, add the literal to all potential
        // pedges
        if (!literal.is_leaf && literal.child_n_id == EPMEM_NODEID_BAD) {
          ...
          for (Map.Entry<EpmemTriple, EpmemUEdge> entry : uedge_iter.entrySet()) {
            ...
            if (child_triple.parent_n_id != triple.parent_n_id
                || child_triple.attribute_s_id != triple.attribute_s_id) {
              break;
            }
   ...
   *
   * <p>{@code void epmem_print_retrieval_state(epmem_wme_literal_map& literals,
   * epmem_triple_pedge_map pedge_caches[], epmem_triple_uedge_map uedge_caches[])}
   */
  private String epmem_print_retrieval_state(
      Map<WmeImpl, EpmemLiteral> literals,
      Map<EpmemTriple, EpmemPEdge>[] pedge_caches,
    ...
    for (Map.Entry<WmeImpl, EpmemLiteral> entry : literals.entrySet()) {
      ...
      if (literal.id_sym != null) {
        sb.append("\"").append(literal.value_sym).append("\" [");
        if (literal.child_n_id == EPMEM_NODEID_BAD) {
          sb.append("label=\"").append(literal.value_sym).append("\"");
        } else {
          sb.append("label=\"").append(literal.child_n_id).append("\"");
        }
        ...
        sb.append("];\n");
        sb.append("\"")
            .append(literal.id_sym)
            .append("\" -> \"")
            .append(literal.value_sym)
            .append("\" [label=\"");
      ...
      for (Map.Entry<EpmemTriple, EpmemUEdge> entry : uedge_cache.entrySet()) {
        ...
        if (triple.child_n_id != EPMEM_NODEID_ROOT) {
          if (type == EPMEM_RIT_STATE_NODE) {
            sb.append("\"n").append(triple.child_n_id).append("\" [shape=\"rect\"];\n");
          }
          sb.append("\"e")
              .append(triple.parent_n_id)
              .append("\" -> \"")
              .append(type == EPMEM_RIT_STATE_NODE ? "n" : "e")
              .append(triple.child_n_id)
              .append("\" [label=\"")
              .append(triple.attribute_s_id)
              .append("\"];\n");
        }
      }
    }
      ...
      for (Map.Entry<EpmemTriple, EpmemUEdge> entry : uedge_cache.entrySet()) {
        ...
        if (triple.attribute_s_id != EPMEM_NODEID_BAD) {
          for (EpmemPEdge pedge : uedge.pedges) {
            var pair = new EpmemPEdgeNodePair(pedge, triple.parent_n_id);
            ...
            sb.append("\"")
                .append(pedge)
                .append("\" -> \"")
                .append(pedge.value_is_id != 0 ? "e" : "n")
                .append(triple.child_n_id)
                .append("\" [style=\"dashed\"];\n");
            /*
            ...
            */
            for (EpmemPEdge pedge_iter : parent_pedge_map.get(triple.child_n_id)) {
              sb.append("\"").append(pedge).append("\" -> \"").append(pedge_iter).append("\";\n");
            }
          }
        }
      }
  ...
  private static class EpmemPEdgeNodePair {
    ...
    public boolean equals(Object obj) {
      ...
      if (pedge == null) {
        return other.pedge == null;
      } else return pedge.equals(other.pedge);
    }
  }

  /**
   * episodic_memory.cpp:3431 {@code epmem_literal* epmem_build_dnf( wme* cue_wme,
   * epmem_wme_literal_map& literal_cache, epmem_literal_set& leaf_literals, epmem_symbol_int_map&
   * symbol_num_incoming, epmem_literal_deque& gm_ordering, epmem_symbol_set& currents, int
   * query_type, std::set<Symbol*>& visiting, soar_module::wme_set& cue_wmes, agent* my_agent )}
   */
  private EpmemLiteral epmem_build_dnf(
    ...
    if (identifier == null) { // WME is a value
      ...
      List<WmeImpl> children = getAllWmesOfId(value, DefaultMarker.create());
      literal.value_is_id = EPMEM_RIT_STATE_EDGE;
    ...
    literal.weight =
        (literal.is_neg_q != 0 ? -1 : 1)
            * (params.balance.get() >= 1.0 - 1.0e-8
                ? 1.0
                : cue_wme.wma.wma_get_wme_activation(cue_wme, true));
    // #ifdef USE_MEM_POOL_ALLOCATORS
    //    new(&(literal->matches)) epmem_node_pair_set(std::less<epmem_node_pair>(),
    // soar_module::soar_memory_pool_allocator<epmem_node_pair>(my_agent));
    // #else
   ...
   * soar_module::symbol_triple_list& my_list, Symbol* id, Symbol* attr, Symbol* value )}
   */
  private void epmem_buffer_add_wme(
      List<SymbolTriple> my_list, SymbolImpl id, SymbolImpl attr, SymbolImpl value) {
  ...
  /**
   * episodic_memory.cpp:3199:{@code epmem_time_id epmem_previous_episode( agent *my_agent,
   * epmem_time_id memory_id )}
   *
   * <p>Returns the last valid temporal id. This is really only an issue if you implement episode
   * dynamics like forgetting.
   */
  private long epmem_previous_episode(long memory_id) {
    ...
    if (memory_id != EPMEM_MEMID_NONE) {
      ...
      try {
        ...
        try (var resultSet = myQuery.executeQuery()) {
          if (resultSet.next()) {
  ...
  /**
   * Returns the next valid temporal id. This is really only an issue if you implement episode
   * dynamics like forgetting.
   */
  ...
  /**
   * episodic_memory.cpp: 2847: {@code void epmem_install_memory( agent *my_agent, Symbol *state,
   * epmem_time_id memory_id, soar_module::symbol_triple_list& meta_wmes,
   * soar_module::symbol_triple_list& retrieval_wmes, epmem_id_mapping *id_record = NULL )}
   */
  private void epmem_install_memory(
      IdentifierImpl state,
      long /*epmem_time_id*/ memory_id,
    ...
    // if no memory, say so
    if ((memory_id == EPMEM_MEMID_NONE) || !epmem_valid_episode(memory_id)) {
      epmem_buffer_add_wme(
          meta_wmes,
          result_header,
    ...
    epmem_buffer_add_wme(
        meta_wmes, result_header, predefinedSyms.epmem_sym_retrieved, retrieved_header);
    ...
    // install memory
    {
      ...
      PreparedStatement my_q = db.get_wmes_with_identifier_values;
      {
        ...
        epmem_rit_prep_left_right(
            memory_id, memory_id, epmem_rit_state_graph[EPMEM_RIT_STATE_EDGE]);
        ...
        var resultSet = my_q.executeQuery();
        while (resultSet.next()) {
          ...
          if (id_p != null) {
            ...
            if (dont_abide_by_ids_second || id_p.second) {
              ...
              if (filter == null) should_install = true;
              else {
                ...
                if (passedFilter.contains(memoryId)) {
                  ...
                  {
                    for (SymbolImpl filterSym :
                        filterIdSet) // for each filterWme in filterParents[wme.parent]:
                    {
                      IdentifierImpl filterId = filterSym.asIdentifier();
                      var slot = Slot.find_slot(filterId, attr);
                      if (slot != null) // if filterWme.hasAttribute(wme.attr)
                      {
                        ...
                        while (it.hasNext()) {
                          ...
                          if (childWme.value.asIdentifier() != null) {
                            if (filterSlots == null) filterSlots = new HashSet<>();
                ...
                if (filterSlots != null) {
                  ...
                  for (Slot filterSlot : filterSlots) {
                    ...
                    while (it.hasNext()) // for childWme in filterWme.getChildren()
                    {
                      ...
                      if (childId != null) {
                        if (childId.slots == null) passedFilter.add(childMemoryId);
                        else currentFilterList.add(childSym);
                      }
                    }
                  }
                } else if (filter != null) {
      ...
      my_q = db.get_wmes_with_constant_values;
      {
        ...
        epmem_rit_prep_left_right(
            memory_id, memory_id, epmem_rit_state_graph[EPMEM_RIT_STATE_NODE]);
                      ...
                      filterIdSet) // for each filterWme in filterParents[wme.parent]:
                  {
                    ...
                    if (slot != null) // if filterWme.hasAttribute(wme.attr)
                    {
                      ...
                      while (it.hasNext()) {
                        ...
                        // If these are the same value, or if this matches a dangling identifier on
                        // the filter.
      ...
      if (filter != null) {
        ...
        while (i < goodIndex) {
          ...
          if (wme.value.asIdentifier() == null) passedFilter.add(wme.id.asIdentifier());
          if (passedFilter.contains(wme.id)) Collections.swap(retrieval_wmes, i, --goodIndex);
          else ++i;
        }
  ...
  /**
   * episodic_memory.cpp: 1121: {@code void epmem_rit_clear_left_right( agent *my_agent )}
   *
   * <p>Clears the left/right relations populated during prep
   */
  ...
  /**
   * episodic_memory.cpp: 2779: {@code inline void _epmem_install_id_wme( agent* my_agent, Symbol*
   * parent, Symbol* attr, std::map< epmem_node_id, std::pair< Symbol*, bool > >* ids, epmem_node_id
   * child_n_id, bool val_is_short_term, char val_letter, uint64_t val_num, epmem_id_mapping*
   * id_record, soar_module::symbol_triple_list& retrieval_wmes )}
   */
  private void _epmem_install_id_wme(
    ...
    if (val_is_short_term) {
      ...
      if (existing_identifier) {
        ...
        // SymbolImpl value = smem_lti_soar_make( my_agent, smem_lti_get_id( my_agent, val_letter,
        // val_num ), val_letter, val_num, parent->id.level );
        SymbolImpl value =
            smem.smem_lti_soar_make(
        ...
        ids.put(
            ...
            new SymbolBooleanPair(
                value,
                !(((value.asIdentifier().goalInfo != null)
                        && !value.asIdentifier().goalInfo.getImpasseWmes().isEmpty())
                    || !value.asIdentifier().getInputWmes().isEmpty()
  ...
  /**
   * episodic_memory.cpp: 1156: void epmem_rit_prep_left_right( agent *my_agent, int64_t lower,
   * int64_t upper, epmem_rit_state *rit_state )
   *
   * <p>Implements the computational components of the RIT
   */
  ...
  /** episodic_memory.cpp: 1144: void epmem_rit_add_right( agent *my_agent, epmem_time_id id ) */
  ...
  /**
   * episodic_memory.cpp: 1132: void epmem_rit_add_left( agent *my_agent, epmem_time_id min,
   * epmem_time_id max )
   *
   * <p>Adds a range to the left relation
   */
  ...
  private boolean epmem_valid_episode(long /*epmem_time_id*/ memory_id) throws SQLException {
...

    {
      ...
      return_val = (resultSet.getLong(1) > 0);
      resultSet.close();
    }
  ...
  /**
   * episodic_memory.cpp:5063: {@code void inline _epmem_respond_to_cmd_parse( agent* my_agent,
   * epmem_wme_list* cmds, bool& good_cue, int& path, epmem_time_id& retrieve, Symbol*& next,
   * Symbol*& previous, Symbol*& query, Symbol*& neg_query, epmem_time_list& prohibit,
   * epmem_time_id& before, epmem_time_id& after, epmem_symbol_set& currents, soar_module::wme_set&
   * cue_wmes )}
   */
      ...
      if (good_cue.value) {
        ...
        if (w_p.attr == predefinedSyms.epmem_sym_retrieve) {
          ...
          if ((w_p.getValue().asInteger() != null) && ((path.value == 0) || (path.value == 3))) {
            if ((before.value == EPMEM_MEMID_NONE)
                || (w_p.value.asInteger().getValue() < before.value)) {
  ...
  public void removeWme(WmeImpl w) {
    ...
    if (w.value.asIdentifier() != null) {
      ...
      if (!lti
          && (w.value.asIdentifier().epmem_id != EPMEM_NODEID_BAD)
          && (w.value.asIdentifier().epmem_valid == epmem_validation)) {
        Set<WmeImpl> my_refs =
            epmem_id_ref_counts.computeIfAbsent(
                w.value.asIdentifier().epmem_id, k -> new LinkedHashSet<>());
  ...
  public void processIds() {
    while (!epmem_id_removes.isEmpty()) {
      ...
      if ((id.epmem_id != EPMEM_NODEID_BAD) && (id.epmem_valid == epmem_validation)) {
        ...
        // remove all impasse WMEs
        if (id.goalInfo != null) {
          id.goalInfo.getImpasseWmes().forEach(this::removeWme);
        }

        // remove all input WMEs
        id.getInputWmes().forEach(this::removeWme);

        // removal all regular WMEs
        for (Slot s = id.slots; s != null; s = s.next) {
          s.getWmes().forEach(this::removeWme);
          s.getAcceptablePreferenceWmes().forEach(this::removeWme);
        }
      }
    }
  }
  ...
  String epmem_print_episode(long /*epmem_time_id*/ memory_id) {
    try {
      ...
      log.error("Soar Exception: {}", e.getStackTrace());
    } catch (SQLException e) {
      log.error("Database Error: {}", e.getStackTrace());
    }
    return "";
  }

  /**
   * episodic_memory.cpp:4518: epmem_print_episode( agent* my_agent, epmem_time_id memory_id,
   * std::string* buf )
   *
   * <p>This version of the function can throw, but we dont want that to propogate, and I don't want
   * to wrap 2/3 of the function in a try catch, so there is a wrapped version of this with the
   * original C function name that can catch the exceptions.
   */
  String epmem_print_episode_ex(long /*epmem_time_id*/ memory_id)
    ...
    // output
    {
      ...
      for (Map.Entry<Long, Map<String, List<String>>> ep_it : ep.entrySet()) {
        ...
        if (lti_it == null) {
          buf.append(_epmem_print_sti(ep_it.getKey()));
        } else {
          buf.append(lti_it);
        }
  ...
  public void epmem_reinit() {
    try {
      ...
      if (!EpisodicMemoryDatabase.IN_MEMORY_PATH.equalsIgnoreCase(params.path.get())
          && params.append_database.get() == AppendDatabaseChoices.on) {
        log.info(
            "EpMem|   Note: There was no effective change to memory contents because append mode is on and path set to file.");
      }
    } catch (SQLException e) {
  ...
  static boolean epmem_parse_chunk(
      SymbolFactoryImpl symbols,
      Lexer lexer,
      Map<String, IdentifierImpl> ids,
      Set<WmeImpl> wmes,
      IdentifierHolder root,
      IdentifierHolder firstHolder)
      throws IOException {
    var return_val = false;
    var good_at = false;
    ...
    var nextIdentifierIsState = false;
    ...
    if ((lexer.getCurrentLexeme().type == LexemeType.AT)
      ...
      if (good_at) {
        var lexeme = lexer.getCurrentLexeme();
        String idKey;
        IdentifierImpl id;
        ...
        if (firstHolder.getIdentifier() == null) firstHolder.setIdentifier(id);
        ...
        while (lexer.getCurrentLexeme().type == LexemeType.UP_ARROW) {
          ...
          if (chunk_attr != null) {
            ...
            while (lexer.getCurrentLexeme().type == LexemeType.PERIOD) {
              // Anonymous WME: don't need an idKey, don't need to add it to "ids", don't need to
              // check for existing slots.
              IdentifierImpl temp_id = symbols.createIdentifier(chunk_attr.getFirstLetter());
              var newWme = new WmeImpl(intermediate_parent, chunk_attr, temp_id, false, 0);
              wmes.add(newWme);
            ...
            if (chunk_attr != null) {
              ...
              do {
                ...
                if (chunk_value != null) {
                  ...
                  while (it.hasNext()) {
                    var wme = it.next();
                    if (compare_symbol(wme.getAttribute(), chunk_attr)
                        && compare_symbol(wme.getValue(), chunk_value)) {
                      if (wme instanceof WmeImpl) newWme = (WmeImpl) wme;
                      break;
                    }
                  }
  ...
  public static boolean compare_symbol(Symbol a, Symbol b) {
    var idA = a.asIdentifier();
    var idB = b.asIdentifier();

    if ((idA == null) != (idB == null)) return false;
    if (idA != null) return a.equals(b);

    if (!a.getClass().equals(b.getClass())) return false;

    if (a instanceof DoubleSymbol) return a.asDouble().getValue() == b.asDouble().getValue();
    else if (a instanceof IntegerSymbol)
      return a.asInteger().getValue() == b.asInteger().getValue();
    else if (a instanceof StringSymbol) return a.asString().getValue() == b.asString().getValue();
    else if (a instanceof JavaSymbol) return a.asJava().getValue().equals(b.asJava().getValue());
  ...
  boolean epmem_parse_and_add(String chunkString) throws SoarException {
    try {
      ...
      var filename = new File(chunkString);
      if (filename.isFile()) return epmem_parse_and_add_file(filename);
      else
        return epmem_parse_and_add_string(
            "{" + chunkString + "}"); // The interpreter strips braces; put them back.
  ...
  private boolean epmem_parse_and_add_file(File filename) throws SoarException, SQLException {
    try {
      var fileContents = Files.readString(filename.toPath(), Charset.defaultCharset());
  ...
  private boolean epmem_parse_and_add_string(String chunkString)
      throws SoarException, IOException, SQLException {
    ...
    do {
      ...
      if (clause_count == 0)
        throw new SoarException(
            "Unexpected character at the beginning of episode #" + episode_index);
      ...
      if ((possibleRoots.size() == 1 && possibleRoots.iterator().next() != rootId)
          || possibleRoots.size() > 1)
        throw new SoarException("Too many possible top-states in episode #" + episode_index);

      // Clear away anything else marked for addition (we'll re-add it when we re-mark current WM
      // for inclusion in epmem).
      epmem_wme_adds.clear();
      ...
      while (!symbolsToTraverse.isEmpty()) {
        ...
        List<WmeImpl> children = getAllWmesOfId(sym, marker);
        for (WmeImpl wme : children) {
          if (wme.value.asIdentifier() != null) symbolsToTraverse.add(wme.value);
          removeWme(wme);
        }
      }
