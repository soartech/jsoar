...
public class DefaultEpisodicMemory implements EpisodicMemory {
  ...
  private final Set<IdentifierImpl> /* epmem_symbol_set */ epmem_wme_adds =
      new LinkedHashSet<IdentifierImpl>();

  /** agent.h:epmem_promotions */
  private final Set<SymbolImpl> /* epmem_symbol_set */ epmem_promotions =
      new LinkedHashSet<SymbolImpl>();
  ...
  private void applyDatabasePerformanceOptions() throws SQLException, SoarException, IOException {
    ...
    if (params.driver.get().equals("org.sqlite.JDBC")) {
      ...
      final var s = db.getConnection().createStatement();
      try {
  ...
  private void epmem_init_db_ex(boolean readonly /* = false */)
    ...
    final String jdbcUrl =
        URLDecoder.decode(params.protocol.get() + ":" + params.path.get(), "UTF-8");
    ...
    epmem_id_repository.put(
        EPMEM_NODEID_ROOT, new LinkedHashMap<Long, LinkedList<EpisodicMemoryIdPair>>());
    {
          ...
          while (rs.next()) {
            ...
            if (range_start == time_last) {
              ...
              epmem_rit_insert_interval(
                  range_start, time_last, rs.getLong(0 + 1), epmem_rit_state_graph[i]);
            }
          }
        } finally {
          rs.close();
   ...
   */
  private final EpmemRitForkNodeResult epmem_rit_fork_node(
    ...
    // perform storage
    {
      ...
      // walk appropriate levels
      {
        ...
        Map<WmeImpl, EpisodicMemoryIdReservation> id_reservations =
            new LinkedHashMap<WmeImpl, EpisodicMemoryIdReservation>();
  ...
  ************************************************************************** */
  ...
  private void epmem_process_query(
    ...
    Map<WmeImpl, EpmemLiteral> /*epmem_wme_literal_map*/ literal_cache =
        new LinkedHashMap<WmeImpl, EpmemLiteral>();
    @SuppressWarnings("unchecked")
    Map<EpmemTriple, EpmemPEdge> /*epmem_triple_pedge_map*/[] pedge_caches = new Map[2];
    pedge_caches[0] =
        new LinkedHashMap<DefaultEpisodicMemory.EpmemTriple, DefaultEpisodicMemory.EpmemPEdge>();
    pedge_caches[1] =
        new LinkedHashMap<DefaultEpisodicMemory.EpmemTriple, DefaultEpisodicMemory.EpmemPEdge>();
    /*
    ...
    */
    ...
    @SuppressWarnings("unchecked")
    var /*epmem_triple_uedge_map*/ uedge_caches = new SortedMap[2];
    ...
    LinkedList<EpmemLiteral> /*epmem_literal_deque*/ gm_ordering = new LinkedList<EpmemLiteral>();
        ...
        for (int query_type = EPMEM_NODE_POS; query_type <= EPMEM_NODE_NEG; query_type++) {
          ...
          List<WmeImpl> /*epmem_wme_list**/ children =
              epmem_get_augs_of_id(
                  query_root, DefaultMarker.create()); // get_new_tc_number(my_agent));
      ...
      */
      ...
      while (pedge_pq.size() != 0 && current_episode > after) {
        ...
        while ((pedge_pq.size() != 0)
          ...
          if (uedge_iter == null) {
            ...
            var created = false;
            var /*int64_t*/ edge_id = pedge.sqlResults.getLong(0 + 1);
      ...
      if (best_episode == EPMEM_MEMID_NONE) {
        ...
        Map<Long /*epmem_node_id*/, SymbolImpl> /*epmem_id_mapping*/ node_map_map =
            new LinkedHashMap<Long, SymbolImpl>();
  ...
  private static class EpmemPEdgeNodePair {
    ...
    public boolean equals(Object obj) {
      ...
      if (pedge == null) {
        if (other.pedge != null) return false;
      } else if (!pedge.equals(other.pedge)) return false;
      return true;
    }
  }
  ...
  private long epmem_previous_episode(long memory_id) {
    ...
    if (memory_id != EPMEM_MEMID_NONE) {
      ...
      try {
        ...
        final var resultSet = myQuery.executeQuery();
        try {
          if (resultSet.next()) {
  ...
  private void epmem_install_memory(
    ...
    // install memory
    {
      ...
      Map<Long /*epmem_node_id*/, SymbolBooleanPair> ids =
          new LinkedHashMap<Long, SymbolBooleanPair>();
                      ...
                      if (slot != null) // if filterWme.hasAttribute(wme.attr)
                      {
                        ...
                        while (it.hasNext()) {
                          ...
                          if (childWme.value.asIdentifier() != null) {
                            if (filterSlots == null) filterSlots = new HashSet<Slot>();
  ...
  private boolean epmem_parse_and_add_file(File filename) throws SoarException, SQLException {
    try {
      var fileContents =
          new String(Files.readAllBytes(filename.toPath()), Charset.defaultCharset());
