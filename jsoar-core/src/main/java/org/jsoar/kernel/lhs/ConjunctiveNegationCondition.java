...
public class ConjunctiveNegationCondition extends Condition {
  ...
  public boolean cond_is_in_tc(Marker tc) {
    ...
    for (Condition c = top; c != null; c = c.next) c.already_in_tc = false;
    ...
    boolean result = true;
    for (Condition c = top; c != null; c = c.next) if (!c.already_in_tc) result = false;
