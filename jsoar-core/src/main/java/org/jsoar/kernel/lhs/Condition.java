...
public abstract class Condition implements Formattable {
  ...
  public static void addAllVariables(
      Condition header, Marker tc_number, ListHead<Variable> var_list) {
    for (Condition c = header; c != null; c = c.next) {
      c.addAllVariables(tc_number, var_list);
    }
  }

  public static void addBoundVariables(
      Condition header, Marker tc_number, ListHead<Variable> var_list) {
    for (Condition c = header; c != null; c = c.next) {
   ...
   */
  public abstract void add_cond_to_tc(
  ...
  public static int hash_condition(Condition cond) {
    ...
    if (pc != null) {
      ...
      if (pc.test_for_acceptable_preference) result++;
    ...
    if (nc != null) {
      ...
      if (nc.test_for_acceptable_preference) result++;
  ...
  /**
   * Copies the given condition list, returning pointers to the top-most and bottom-most conditions
   * in the new copy.
   *
   * <p>production.cpp:772:copy_condition_list
   *
   ...
   */
  public static void copy_condition_list(
      Condition top_cond, ByRef<Condition> dest_top, ByRef<Condition> dest_bottom) {
    Condition prev = null;
    while (top_cond != null) {
      Condition New = copy_condition(top_cond);

      if (prev != null) prev.next = New;
      else dest_top.value = New;
  ...
  public static Condition explain_find_cond(Condition target, Condition cond_list) {
    for (Condition cond = cond_list; cond != null; cond = cond.next) {
      if (conditions_are_equal(target, cond)) return cond;
    }
    return null;
  }
