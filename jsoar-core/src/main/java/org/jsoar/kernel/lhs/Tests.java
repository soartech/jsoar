 ...
 *
 * <p>For test_is_blank_or_equality_test, just use {@link Test#asEqualityTest()}.
 *
 * @author ray
 */
public class Tests {
  /**
   * Looks through a test, and returns a new copy of the first equality test it finds. Signals an
   * error if there is no equality test in the given test.
   *
   * <p>production.cpp:686:copy_of_equality_test_found_in_test
   *
   * @param t
  ...
  /**
   * Destructively modifies the first test (t) by adding the second one (add_me) to it (usually as a
   * new conjunct). The first test need not be a conjunctive test.
   *
  ...
  public static boolean test_includes_equality_test_for_symbol(Test test, SymbolImpl sym) {
    ...
    var eq = test.asEqualityTest();
    if (eq != null) {
      if (sym != null) return eq.getReferent() == sym;
      return true;
    }

    var ct = test.asConjunctiveTest();
    if (ct != null) {
      for (Test child : ct.conjunct_list) {
  ...
  public static boolean test_includes_goal_or_impasse_id_test(
      Test t, boolean look_for_goal, boolean look_for_impasse) {
    ...
    if (ct != null) {
      for (Test c : ct.conjunct_list) {
        return test_includes_goal_or_impasse_id_test(c, look_for_goal, look_for_impasse);
      }
    }
    return false;
  }
  ...
  /**
   * Same as copy_test(), only it doesn't include goal or impasse tests in the new copy. The caller
   * should initialize the two flags to FALSE before calling this routine; it sets them to TRUE if
   * it finds a goal or impasse test.
   *
   ...
   */
  public static Test copy_test_removing_goal_impasse_tests(
      Test t, ByRef<Boolean> removed_goal, ByRef<Boolean> removed_impasse) {
    ...
    if (ct != null) {
      Test new_t = null;
      for (Test c : ct.conjunct_list) {
        var temp = copy_test_removing_goal_impasse_tests(c, removed_goal, removed_impasse);
        if (!Tests.isBlank(temp)) {
          new_t = add_new_test_to_test(new_t, temp);
        }
      }
      // TODO I don't think reverse is correct here.
      var newct = new_t.asConjunctiveTest();
      if (newct != null) {
  ...
  /**
   * Same as add_new_test_to_test(), only has no effect if the second test is already included in
   * the first one.
   *
  ...
  /**
   * This routine pushes bindings for variables occurring (i.e., being equality-tested) in a given
   * test. It can do this in DENSE fashion (push a new binding for ANY variable) or SPARSE fashion
   * (push a new binding only for previously-unbound variables), depending on the boolean "dense"
   * parameter. Any variables receiving new bindings are also pushed onto the given "varlist".
   *
   ...
   * @param varlist
   */
  public static void bind_variables_in_test(
      Test t, int depth, int field_num, boolean dense, ListHead<Variable> varlist) {
    if (Tests.isBlank(t)) {
      return;
    }
  ...
  /**
   * TODO Make add_test_to_tc polymorphic on Test
   *
   ...
   * @param var_list
   */
  static void add_test_to_tc(
      Test t, Marker tc, ListHead<IdentifierImpl> id_list, ListHead<Variable> var_list) {
    ...
    var ct = t.asConjunctiveTest();
    if (ct != null) {
      for (Test c : ct.conjunct_list) add_test_to_tc(c, tc, id_list, var_list);
    }
  }

  /**
   * production.cpp:1354:test_is_in_tc
   *
   * @param t the test
  ...
  static boolean test_is_in_tc(Test t, Marker tc) {
    if (Tests.isBlank(t)) return false;
    var eq = t.asEqualityTest();
    if (eq != null) {
    ...
    var ct = t.asConjunctiveTest();
    if (ct != null) {
      for (Test c : ct.conjunct_list) if (test_is_in_tc(c, tc)) return true;
      return false;
    }
    return false;
  }
  ...
  public static int hash_test(Test t) {
    ...
    var eq = t.asEqualityTest();
    if (eq != null) return eq.getReferent().getHash();
    ...
    var dt = t.asDisjunctionTest();
    if (dt != null) {
      var result = 7245;
      for (SymbolImpl c : dt.disjunction_list) result = result + c.getHash();
      return result;
    }
    var ct = t.asConjunctiveTest();
    if (ct != null) {
    ...
    var rt = t.asRelationalTest();
    if (rt != null) {
      return (rt.type << 24) + rt.referent.getHash();
    }
  ...
  /**
   * production.cpp::test_is_blank_test
   *
   * @param t the test
