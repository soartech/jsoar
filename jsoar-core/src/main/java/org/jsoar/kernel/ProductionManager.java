...
public interface ProductionManager {
  ...
  /**
   * Set the default parser used by this production manager when {@link #loadProduction(String)} is
   * called.
   *
  ...
  /**
   * Parse a <b>production body</b> and add it to the agent's rete network. Note that the production
   * body does not include the {@code sp} or surrounding braces!
   *
  ...
  /**
   * Convenience version of {@link #loadProduction(String, SourceLocation)} equivalent to {@code
   * loadProduction(body, DefaultSourceLocation.UNKNOWN); }
   *
  ...
  /**
   * Add the given chunk or justification production to the agent. The chunk is reordered and
   * registered, but it is <b>not</b> added to the rete network.
   *
  ...
  /**
   * Returns a list of productions of a particular type, or all productions if type is <code>null
   * </code>
   *
  ...
  /**
   * Add the given production to the agent. If a production with the same name is already loaded, it
   ...
   * @throws IllegalArgumentException if p is a chunk or justification or if p (actual instance, not
  ...
  /**
   * production.cpp:1595:excise_production
   *
   * @param prod
