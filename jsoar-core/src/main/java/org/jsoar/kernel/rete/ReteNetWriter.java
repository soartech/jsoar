...
public class ReteNetWriter {
  ...
  protected final HashSet<PropertyKey<?>> propertiesToInclude =
      new HashSet<PropertyKey<?>>() {
        {
  ...
  /**
   * Write a rete net to the given output stream. All rules and symbols are written.
   *
   ...
   */
  private void writeProperties(DataOutputStream dos, HashSet<PropertyKey<?>> properties)
      throws IOException, SoarException {
  ...
  private void writeChildrenOfNode(DataOutputStream dos, ReteNode node)
    ...
    // For example: @see org.jsoar.kernel.rete.ReteNode.make_new_mem_node(Rete, ReteNode,
    // ReteNodeType, VarLocation)
  ...
  /** Returns, in the order they appear in the node's linked list, a list of the node's children. */
  ...
  private void writeNodeAndChildren(DataOutputStream dos, ReteNode node)
    ...
    // RETECOMPAT: When writing out an enum type, JSoar uses the enum's toString(), but CSoar writes
    // out one byte.
    ...
    switch (node.node_type) {
      ...
      case UNHASHED_MP_BNODE:
        // RETECOMPAT: No effort has been put in writing out CSoar compatible integers. Everything
        // is written
    ...
    if (node.node_type == ReteNodeType.CN_PARTNER_BNODE) node = node.b_cn().partner;
    // --- Write out records for all the node's children. ---
    writeChildrenOfNode(dos, node);
  }

  /**
   * Convenience function for writing out the field_num and levels_up for a {@link ReteNode}.
   *
   ...
   *
   * <p>Format: Writes an integer containing the number of actions to be written, then writes each
   * action.
   *
   ...
   */
  private void writeActionList(DataOutputStream dos, Action firstAction)
      throws IOException, SoarException {
    int numActions = 0;
    Action a;
   ...
   *
   * <p>An integer is written to represent the type of action, followed by a string representation
   * of its preference type {@link PreferenceType} (if any), and the appropriate RHS values {@link
   * RhsValue}.
   *
   ...
   *
   * <p>Record: 1 int: type (0={@link RhsSymbolValue}, 1={@link RhsFunctionCall}, 2={@link
   * ReteLocation}, 3={@link UnboundVariable})
   *
   * <p>for symbols, an int of the symbol index.
   *
   * <p>for function calls, an int of the function name's symbol index, if it's a standalone
   * function, the number of arguments, and a rhs value record for each argument.
   *
   ...
   *
   * <p>The number (int) of nodes in the linked list is saved, followed by each {@link ReteTest}.
   *
   ...
   */
  private void writeTestList(DataOutputStream dos, ReteTest firstRete)
      throws IOException, SoarException {
    int numTests = 0;
    ReteTest rt;
   ...
   *
   * <p>Record syntax: one int for {@link ReteTest#type}, and one int for {@link
   * ReteTest#right_field_num}.
   *
   ...
   *
   * <p>For disjunctions: one int for number of disjunts, and then list of ints, one for the
   * symindex of each disjunct.
   *
  ...
  /**
   * Writes out all instances of {@link StringSymbol}, {@link Variable}, {@link IntegerSymbol}, and
   * {@link DoubleSymbol}.
   *
   ...
   */
  private <T extends Symbol> int writeSymbolList(
      DataOutputStream dos, int nextIndex, List<T> symbols, SymbolWriter<T> writer)
      throws IOException {
  ...
  /**
   * Gets the symbol index for a symbol s. If s is null, returns 0. If s has no existing index,
   * returns 0.
   */
   ...
   *
   * <p>Record format: three ints for the symbol index of each of the id, attr, and values. One
   * boolean for the acceptable preference.
   *
   ...
   * Writes varnames.
   *
   * <p>Record format: type (1 byte): 0=null, 1=one var, 2=list if one var: 4 bytes (symindex) if
   * list: 4 bytes (number of items) + list of symindices
   *
   ...
   */
  private void writeNodeVarNames(DataOutputStream dos, NodeVarNames nvn, ReteNode node)
      throws IOException {
    while (true) {
