...
/**
 * An interface intended to decouple Soar-specific handling of rete matches and unmatches from the
 * general rete algorithm.
 *
 * @author ray
 */
public interface ReteListener {
  ...
  /**
   * Called after {@link #startRefraction(Rete, Production, Instantiation, ReteNode)} and the
   * initial call to {@link Rete#update_node_with_matches_from_above(ReteNode)} on the p-node.
   *
   * <p>rete.cpp:3515:add_production_to_rete
   *
   * @param rete The rete
   ...
   *
   * <p>BUGBUG shouldn't need to pass in both tok and w -- should have the p-node's token get passed
   * in instead, and have it point to the corresponding instantiation structure.
   *
  ...
  /**
   * Called when a production is being excised. This is called after the p-node's instantiations
   * have been retracted (p_node_left_removal) but before the node itself is actually removed from
   * the rete.
   *
   * <p>This method was extracted into this interface to decouple Soar-specific rete stuff from the
   * general rete algorithm. See
   *
   * @param rete The rete
