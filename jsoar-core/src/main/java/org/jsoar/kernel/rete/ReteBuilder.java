 ...
 *
 * <p>Functions and data structures for building the rete network. Extracted from rete.cpp for
 * sanity.
 *
 * @author ray
 */
class ReteBuilder {
  ...
  static {
    ...
    test_type_to_relational_test_type[RelationalTest.NOT_EQUAL_TEST] =
        ReteTest.RELATIONAL_NOT_EQUAL;
    ...
    test_type_to_relational_test_type[RelationalTest.LESS_OR_EQUAL_TEST] =
        ReteTest.RELATIONAL_LESS_OR_EQUAL;
    test_type_to_relational_test_type[RelationalTest.GREATER_OR_EQUAL_TEST] =
        ReteTest.RELATIONAL_GREATER_OR_EQUAL;
    test_type_to_relational_test_type[RelationalTest.SAME_TYPE_TEST] =
        ReteTest.RELATIONAL_SAME_TYPE;

    relational_test_type_to_test_type[ReteTest.RELATIONAL_EQUAL] = EQUAL_TEST_TYPE;
    relational_test_type_to_test_type[ReteTest.RELATIONAL_NOT_EQUAL] =
        RelationalTest.NOT_EQUAL_TEST;
    ...
    relational_test_type_to_test_type[ReteTest.RELATIONAL_LESS_OR_EQUAL] =
        RelationalTest.LESS_OR_EQUAL_TEST;
    relational_test_type_to_test_type[ReteTest.RELATIONAL_GREATER_OR_EQUAL] =
        RelationalTest.GREATER_OR_EQUAL_TEST;
    relational_test_type_to_test_type[ReteTest.RELATIONAL_SAME_TYPE] =
   ...
   * constant-to-be-tested-by-the-alpha-network. It takes all sub-tests from a given test, converts
   * them into the necessary Rete tests (if any -- note that an equality test with a
   * previously-unbound variable can be ignored), and destructively adds the Rete tests to the given
   * "rt" parameter. The "current_depth" and "field_num" params tell where the current test
   * originated.
   *
   * <p>For any field, we can handle one equality-with-a-constant test in the alpha net. If the
   * "*alpha_constant" parameter is initially NIL, this routine may also set *alpha_constant to
   * point to the constant symbol for the alpha net to test (rather than creating the corresponding
   * rete_test).
   *
   * <p>Before calling this routine, variables should be bound densely for parent and higher
  ...
  /**
   * This is used for checking whether an existing Rete node can be shared, instead of building a
   * new one.
   *
   * <p>Single_rete_tests_are_identical() checks whether two (non-conjunctive) Rete tests are the
   * same. (Note that in the case of disjunction tests, the symbols in the disjunction have to be in
   * the same order; this simplifies and speeds up the code here, but unnecessarily reduces
   * sharing.)
   *
  ...
  private static boolean single_rete_tests_are_identical(ReteTest rt1, ReteTest rt2) {
    ...
    throw new IllegalStateException(
        "Error: found unknown rete test type while building rete: " + rt1 + ", " + rt2);
  }

  /**
   * Rete_test_lists_are_identical() checks whether two lists of Rete tests are identical. (Note
  ...
  /**
   * Extracts from a Rete test list the variable equality test to use for hashing. Returns TRUE if
   * successful, or FALSE if there was no such test to use for hashing. The Rete test list ("rt") is
  ...
  /**
   * Finds or creates a node for the given single condition {@code <cond>}, which must be a simple
   * positive condition. The node is made a child of the given {@code <parent>} node. Variables for
   * earlier conditions should be bound densely before this routine is called. The routine returns a
   * pointer to the (newly-created or shared) node.
   *
  ...
  private static ReteNode make_node_for_positive_cond(
      @NonNull Rete rete,
      @NonNull PositiveCondition cond,
      int current_depth,
    ...
    return ReteNode.make_new_mp_node(
        rete, parent, mp_node_type, left_hash_loc.value, am, rt.value, false);
  }

  /**
   * Finds or creates a node for the given single condition {@code <cond>}, which must be a simple
   * negative (not ncc) condition. The node is made a child of the given {@code <parent>} node.
   * Variables for earlier conditions should be bound densely before this routine is called. The
  ...
  private static ReteNode make_node_for_negative_cond(
    ...
    final ReteNodeType node_type =
        hash_this_node ? ReteNodeType.NEGATIVE_BNODE : ReteNodeType.UNHASHED_NEGATIVE_BNODE;
   ...
   * <dest_bottom_node>} is given as non-NIL, this routine fills it in with a pointer to the
   * lowermost node in the resulting network. If {@code <dest_bottom_depth>} is non-NIL, this
   * routine fills it in with the depth of the lowermost node. If {@code <dest_vars_bound>} is
   * non_NIL, this routine fills it in with a list of variables bound in the given {@code
   * <cond_list>}, and does not pop the bindings for those variables, in which case the caller is
   * responsible for popping theose bindings. If {@code <dest_vars_bound>} is given as NIL, then
  ...
  static /*package*/ void build_network_for_condition_list(
      Rete rete,
      Condition cond_list,
      int depth_of_first_cond,
      ReteNode parent,
      ByRef<ReteNode> dest_bottom_node,
      ByRef<Integer> dest_bottom_depth,
    ...
    for (Condition cond = cond_list; cond != null; cond = cond.next) {
      ...
      if (pc != null) {
        ...
        build_network_for_condition_list(
            rete, ncc.top, current_depth, node, subconditions_bottom_node, null, null);
  ...
  /**
   * After we've built the network for a production, we go through its RHS and replace all the
   * variables with reteloc's and unboundvar indices. For each variable {@code <v>} on the RHS, if
   * {@code <v>} is bound on the LHS, then we replace RHS references to it with a specification of
   * where its LHS binding can be found, e.g., "the value field four levels up". Each RHS variable
   * {@code <v>} not bound on the LHS is replaced with an index, e.g., "unbound varible number 6".
   * As we're doing this, we keep track of the names of all the unbound variables.
   *
   * <p>When this routine is called, variables should be bound (densely) for the entire LHS.
   *
   * <p>NOTE: In CSoar, variables are added to the front of the unbound var list and then reversed
   * later by the calling code (because they're using CSoar's built in cons list). Here, we just add
   * to the end of the list and omit the reversal.
   *
  ...
  static RhsValue fixup_rhs_value_variable_references(
      ...
      RhsValue rv,
      int bottom_depth,
      List<Variable> rhs_unbound_vars_for_new_prod,
