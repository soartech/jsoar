...
public class Rete {
  ...
  /**
   * TODO: Although this hashset preserves the insertion order of the WMEs, the order is the reverse
   * of that in CSoar which inserts at the front. It doesn't appear to affect correctness, but it
   * may cause firing order variation from CSoar. See usage in find_or_make_alpha_mem()
   *
   * <p>all_wmes_in_rete
   */
  ...
  /**
   * This is only intended for testing. It allows the SimpleMatcher to create a Rete without needing
   * to fully instantiate epmem.
   */
  ...
  /**
   * @return List of all Wmes currently in the rete. This is the actual list so please don't modify
   *     it.
   */
  ...
  /**
   * Add_production_to_rete() adds a given production, with a given LHS, to the rete. If
   * "refracted_inst" is non-NIL, it should point to an initial instantiation of the production.
   * This routine returns DUPLICATE_PRODUCTION if the production was a duplicate; else
   * NO_REFRACTED_INST if no refracted inst. was given; else either REFRACTED_INST_MATCHED or
   * REFRACTED_INST_DID_NOT_MATCH.
   *
   * <p>The initial refracted instantiation is provided so the initial instantiation of a
   * newly-build chunk doesn't get fired. We handle this as follows. We store the initial
   * instantiation as a "tentative retraction" on the new p-node. Then we inform the p-node of any
   * matches (tokens from above). If any of them is the same as the refracted instantiation, then
   * that instantiation will get removed from "tentative_retractions". When the p-node has been
   * informed of all matches, we just check whether the instantiation is still on
   * tentative_retractions. If not, there was a match (and the p-node's activation routine filled in
   * the token info on the instantiation for us). If so, there was no match for the refracted
   * instantiation.
   *
   * <p>BUGBUG should we check for duplicate justifications?
   *
   * <p>rete.cpp:3522:add_production_to_rete
   *
   * @param p The production to add
  ...
  public ProductionAddResult add_production_to_rete(
    ...
    ReteBuilder.build_network_for_condition_list(
        this, lhs_top, 1, dummy_top_node, bottom_node, bottom_depth, vars_bound);
    ...
    for (ReteNode p_node = bottom_node.value.first_child;
        p_node != null;
      ...
      if (!ignore_rhs
          && !Action.same_rhs(
              p_node.b_p().prod.getFirstAction(),
              p.getFirstAction(),
      ...
      if (warn_on_duplicates) {
        ...
        // << " because it is a duplicate of " << symbol_to_string( thisAgent,
        // p_node->b.p.prod->name, TRUE, 0, 0 )
        // << " ";
    ...
    if (refracted_inst == null) {
      ...
      production_addition_result =
          refactedInstMatched
              ? ProductionAddResult.REFRACTED_INST_MATCHED
              : ProductionAddResult.REFRACTED_INST_DID_NOT_MATCH;
    }
  ...
  /**
   * This removes a given production from the Rete net, and enqueues all its existing instantiations
   * as pending retractions.
   *
   * <p>rete.cpp:3726:excise_production_from_rete
   *
  ...
  /**
   * Stand-in for taking the address of an object in C, where the address is used for hashing, e.g.
   * cn_node_left_addition() in rete.cpp
   *
   ...
   *
   * <p>rete.cpp:1408:add_wme_to_alpha_mem
   *
   * @param w
   * @param am
   */
  ...
  /**
   * Removes a WME (right_mem) from its alpha memory, but doesn't inform any successors
   *
   * <p>rete.cpp:1429:remove_wme_from_alpha_mem
   *
   * @param rm
   */
   ...
   * @param acceptable
   */
  /*package*/ HashTable<AlphaMemory> table_for_tests(
      SymbolImpl id, SymbolImpl attr, SymbolImpl value, boolean acceptable) {
    final int index =
        ((id != null) ? 1 : 0)
            + ((attr != null) ? 2 : 0)
            + ((value != null) ? 4 : 0)
  ...
  AlphaMemory find_alpha_mem(SymbolImpl id, SymbolImpl attr, SymbolImpl value, boolean acceptable) {
    ...
    for (AlphaMemory am = ht.getBucket(hash_value);
        am != null;
  ...
  /**
   * Find and share existing alpha memory, or create new one. Adjusts the reference count on the
   * alpha memory accordingly.
   *
   ...
   * @param acceptable
   */
  AlphaMemory find_or_make_alpha_mem(
      SymbolImpl id, SymbolImpl attr, SymbolImpl value, boolean acceptable) {
  ...
  /**
   * Using the given hash table and hash value, try to find a matching alpha memory in the indicated
   * hash bucket. If we find one, we add the wme to it and inform successor nodes.
   *
  ...
  /**
   * The dummy top node always has one token in it (WME=NIL). This is just there so that (real) root
   * nodes in the beta net can be handled the same as non-root nodes.
   *
  ...
  /**
   * Calls a node's left-addition routine with each match (token) from the node's parent. DO NOT
   * call this routine on (positive, unmerged) join nodes.
   *
  ...
  void update_node_with_matches_from_above(ReteNode child) {
    if (child.node_type.bnode_is_bottom_of_split_mp()) {
      throw new IllegalArgumentException(
          "Internal error: update_node_with_matches_from_above called on split node");
    }
  ...
  /**
   * When a production is fired, we use an array of gensyms to store the bindings for the RHS
   * unbound variables. We have to grow the memory block allocated for this array any time a
   * production comes along with more RHS unbound variables than we've ever seen before. This
   * procedure checks the number of RHS unbound variables for a new production, and grows the array
   * if necessary.
   *
  ...
  /**
   * This routine destructively modifies a given test, adding to it a test for equality with a new
   * gensym variable.
   *
  ...
  /**
   * We're reconstructing the conditions for a production in top-down fashion. Suppose we come to a
   * Rete test checking for equality with the "value" field 3 levels up. In that case, for the
   * current condition, we want to include an equality test for whatever variable got bound in the
   * value field 3 levels up. This function scans up the list of conditions reconstructed so far,
   * and finds the appropriate variable.
   *
   ...
   * @return the variable
   */
  public static SymbolImpl var_bound_in_reconstructed_conds(
      Condition cond, int where_field_num, int where_levels_up) {
  ...
  /**
   * This replaces left_addition_routines in CSoar. A simple switch statement was easier, faster,
   * and simpler than emulating function pointers with Java interfaces.
   *
  ...
  private void executeLeftAddition(ReteNode node, Token tok, WmeImpl w) {
    ...
    switch (node.node_type) {
      case MEMORY_BNODE:
        beta_memory_node_left_addition(node, tok, w);
        break;
      case UNHASHED_MEMORY_BNODE:
        unhashed_beta_memory_node_left_addition(node, tok, w);
        break;
      case MP_BNODE:
        mp_node_left_addition(node, tok, w);
        break;
      case UNHASHED_MP_BNODE:
        unhashed_mp_node_left_addition(node, tok, w);
        break;
      case CN_BNODE:
        cn_node_left_addition(node, tok, w);
        break;
      case CN_PARTNER_BNODE:
        cn_partner_node_left_addition(node, tok, w);
        break;
      case P_BNODE:
        p_node_left_addition(node, tok, w);
        break;
      case NEGATIVE_BNODE:
        negative_node_left_addition(node, tok, w);
        break;
      case UNHASHED_NEGATIVE_BNODE:
        unhashed_negative_node_left_addition(node, tok, w);
        break;
      case DUMMY_MATCHES_BNODE:
        dummy_matches_node_left_addition(node, tok, w);
        break;
      default:
  ...
  /**
   * This replaces right_addition_routines in CSoar. A simple switch statement was easier, faster,
   * and simpler than emulating function pointers with Java interfaces.
   */
  private void executeRightAddition(ReteNode node, WmeImpl w) {
    switch (node.node_type) {
      case POSITIVE_BNODE:
        positive_node_right_addition(node, w);
        break;
      case UNHASHED_POSITIVE_BNODE:
        unhashed_positive_node_right_addition(node, w);
        break;
      case MP_BNODE:
        mp_node_right_addition(node, w);
        break;
      case UNHASHED_MP_BNODE:
        unhashed_mp_node_right_addition(node, w);
        break;
      case NEGATIVE_BNODE:
        negative_node_right_addition(node, w);
        break;
      case UNHASHED_NEGATIVE_BNODE:
        unhashed_negative_node_right_addition(node, w);
        break;
      default:
        throw new IllegalStateException("Unhandled node type " + node.node_type);
    }
  ...
  private void beta_memory_node_left_addition(ReteNode node, Token tok, WmeImpl w) {
    ...
    for (ListItem<ReteNode> child = node.b_mem().first_linked_child.first;
        child != null;
        child = next) {
  ...
  /**
   * This routine does tree-based removal of a token and its descendents. Note that it uses a
   * nonrecursive tree traversal; each iteration, the leaf being deleted is the leftmost leaf in the
   * tree.
   *
   * <p>rete.cpp:6083:remove_token_and_subtree
   *
   * @param root
   */
  /*package*/ void remove_token_and_subtree(Token root) {
    ...
    while (true) {
      ...
      if ((node_type == ReteNodeType.MP_BNODE) || (node_type == ReteNodeType.UNHASHED_MP_BNODE)) {
        ...
        for (ListItem<RightToken> it = ((LeftToken) tok).getFirstNegRightToken();
            it != null;
            it = it.next) {
        ...
        throw new IllegalStateException(
            "Internal error: bad node type " + node.node_type + " in remove_token_and_subtree");
      }
  ...
  /**
   * P_node_to_conditions_and_nots() takes a p_node and (optionally) a token/wme pair, and
   * reconstructs the (optionally instantiated) LHS for the production. If "actions" is non-NIL, it
   * also reconstructs the RHS actions, and fills in actions with the action list. Note: if
   * tok!=NIL, this routine also returns (in nots) the top-level positive {@code "<>"} tests. If
   * tok==NIL, nots is not used.
   *
  ...
  public ConditionsAndNots p_node_to_conditions_and_nots(
    ...
    ReteNodeToConditionsResult rntc =
        rete_node_to_conditions(
            p_node.parent, p_node.b_p().parents_nvn, dummy_top_node, tok, w, null);
  ...
  /**
   * This routine adds (an equality test for) each variable in "vn" to the given test "t",
   * destructively modifying t. This is used for restoring the original variables to test in a
   * hand-coded production when we reconstruct its conditions.
   *
   * <p>rete.cpp:4058:add_varnames_to_test
   *
   * @param vn
   * @param t
   */
  ...
  /**
   * Given the additional Rete tests (besides the hashed equality test) at a certain node, we need
   * to convert them into the equivalent tests in the conditions being reconstructed. This procedure
   * does this -- it destructively modifies the given currently-being-reconstructed-cond by adding
   * any necessary extra tests to its three field tests.
   *
   * <p>rete.cpp:3896:add_rete_test_list_to_tests
   *
   * @param cond
   * @param rtIn
   */
  void add_rete_test_list_to_tests(ThreeFieldCondition cond, final ReteTest rtIn) {
    for (ReteTest rt = rtIn; rt != null; rt = rt.next) {
      ...
      if (rt.type == ReteTest.ID_IS_GOAL) {
        ...
        final int test_type =
            ReteBuilder.relational_test_type_to_test_type[rt.kind_of_relational_test()];
            ...
            if (!Tests.test_includes_equality_test_for_symbol(cond.value_test, null)) {
              cond.value_test =
                  add_gensymmed_equality_test(
                      cond.value_test, Tests.first_letter_from_test(cond.attr_test));
            }
          }
        }
        SymbolImpl referent =
  ...
  /**
   * When we build the instantiated conditions for a production being fired, we also record all the
   * "{@code <>}" tests between pairs of identifiers. (This information is used during chunking.)
   * This procedure looks for any such {@code <>} tests in the given Rete test list (from the "other
   * tests" at a Rete node), and adds records of them to the global variable
   * nots_found_in_production. "Right_wme" is the wme that matched the current condition; "cond" is
   * the currently-being-reconstructed condition.
   *
   * <p>rete.cpp:4000:collect_nots
   *
   * @param rt
   * @param right_wme
   * @param cond
   ...
   */
  NotStruct collect_nots(
      ReteTest rt, WmeImpl right_wme, Condition cond, NotStruct nots_found_in_production) {
    for (; rt != null; rt = rt.next) {

      if (!rt.test_is_not_equal_test()) continue;

      SymbolImpl right_sym = right_wme.getField(rt.right_field_num);

      if (right_sym.asIdentifier() == null) continue;
  ...
  /**
   * This routine adds an equality test to the id field test in a given condition, destructively
   * modifying that id test. The equality test is the one appropriate for the given hash location
   * (field_num/levels_up).
   *
   * <p>rete.cpp:4082:add_hash_info_to_id_test
   *
   * @param cond
   * @param field_num
   * @param levels_up
  ...
  /**
   * This is the main routine for reconstructing the LHS source code, and for building instantiated
   * conditions when a production is fired. It builds the conditions corresponding to the given rete
   * node ("node") and all its ancestors, up to the given "cutoff" node. The given node_varnames
   * structure "nvn", if non-NIL, should be the node_varnames corresponding to "node". {@code
   * <tok,w>} (if they are non-NIL) specifies the token/wme pair that emerged from "node" -- these
   * are used only when firing, not when reconstructing. "conds_for_cutoff_and_up" should be the
   * lowermost cond in the already-constructed chain of conditions for the "cutoff" node and higher.
   * "Dest_top_cond" and "bottom" get filled in with the highest and lowest conditions built by this
   * procedure.
   *
   * <p>Note: Original return by ref parameters in CSoar were replaced by a return structure in
   * Java.
   *
   ...
   */
  ReteNodeToConditionsResult rete_node_to_conditions(
      ReteNode node,
      NodeVarNames nvn,
      ReteNode cutoff,
      Token tok,
    ...
    if (node.node_type == ReteNodeType.CN_BNODE) {
      ...
      if (w != null && cond.asPositiveCondition() != null) {
        ...
        if ((node.node_type == ReteNodeType.MP_BNODE)
            || (node.node_type == ReteNodeType.NEGATIVE_BNODE)) {
          ...
          add_hash_info_to_id_test(
              tfc, node.parent.left_hash_loc_field_num, node.parent.left_hash_loc_levels_up);
        }
        ...
        if (nvn == null) {
          ...
          if (!Tests.test_includes_equality_test_for_symbol(tfc.value_test, null)) {
            tfc.value_test =
                add_gensymmed_equality_test(
                    tfc.value_test, Tests.first_letter_from_test(tfc.attr_test));
          }
        }
      }
    }
  ...
  public static void print_whole_token_wme(Printer printer, WmeImpl w, WmeTraceType wtt) {
    if (w != null) {
      if (wtt == WmeTraceType.TIMETAG) printer.print("%d", w.getTimetag());
      else if (wtt == WmeTraceType.FULL) printer.print("%s", w);
      if (wtt != WmeTraceType.NONE) printer.print(" ");
    }
  }

  /**
   * prints out a given token in the format appropriate for the given wme_trace_type: either a list
   * of timetags, a list of WMEs, or no printout at all.
   *
  ...
  /**
   * rete.cpp:7554:get_all_left_tokens_emerging_from_node
   *
   * @param node
   */
  ...
  /**
   * This is for the "matches" command. Print_partial_match_information() is called from the
   * interface routine; ppmi_aux() is a helper function. We first call
   * p_node_to_conditions_and_nots() to get the condition list for the LHS. We then (conceptually)
   * start at the top of the net, with the first condition; for each condition, we collect the
   * tokens output by the previous node, to find the number of matches here. We print the # of
   * matches here; print this condition. If this is the first cond that didn't have any match, then
   * we also print its matches-for-left and matches-for-right.
   *
   * <p>Of course, we can't actually start at the top of the net and work our way down, since we'd
   * have no way to find our way the the correct p-node. So instead, we use a recursive procedure
   * that basically does the same thing.
   *
   * <p>Print stuff for given node and higher, up to but not including the cutoff node. Return
   * number of matches at the given node/cond.
   *
  ...
  private int ppmi_aux(
    ...
    final int matches_one_level_up =
        ppmi_aux(printer, parent, cutoff, cond.prev, wtt, indent, showNodeIds);
    ...
    if (matches_one_level_up == 0) {
      ...
      if (showNodeIds) {
        match_count_string +=
            String.format(" %10s", "(" + Integer.toHexString(System.identityHashCode(node)) + ")");
      }
    }
    ...
    if (ncc != null) {
      ...
      ppmi_aux(
          printer,
          node.b_cn().partner.real_parent_node(),
          parent,
          ncc.bottom,
          wtt,
          indent + 5,
      ...
      if (matches_one_level_up != 0 && matches_at_this_level == 0) {
        if (wtt != WmeTraceType.NONE) {
          ...
          for (RightMemory rm = node.b_posneg().alpha_mem_.right_mems;
              rm != null;
              rm = rm.next_in_am) {
            if (wtt == WmeTraceType.TIMETAG) printer.print("%d", rm.w.getTimetag());
            else if (wtt == WmeTraceType.FULL) printer.print("%s", rm.w);
            printer.print(" ");
          }
          printer.print("\n");
        }
  ...
  /**
   * Print partial matches for the given p-node. Client code should call {@link
   * Production#printPartialMatches(Printer, WmeTraceType)}
   *
   ...
   * @param wtt
   */
  public void print_partial_match_information(
      Printer printer, ReteNode p_node, WmeTraceType wtt, boolean showNodeIds) {
  ...
  private List<Entry> getPartialMatchesAux(
      List<Entry> entries, ReteNode node, ReteNode cutoff, Condition cond) {
  ...
  public PartialMatches getPartialMatches(ReteNode p_node) {
    ...
    final List<PartialMatches.Entry> entries =
        getPartialMatchesAux(new ArrayList<Entry>(), p_node.parent, dummy_top_node, cans.bottom);
  ...
  public int countTokensProduction(ReteNode p_node) {
    ...
    while (node != dummy_top_node) {
      if ((node.node_type != ReteNodeType.POSITIVE_BNODE)
        ...
        for (Token tok = node.a_np().tokens; tok != null; tok = tok.next_of_node) count++;
      }
