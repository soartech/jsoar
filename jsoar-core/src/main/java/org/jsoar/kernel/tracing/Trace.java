...
/**
 * This class encapsulates the tracing mechanism used throughout the kernel into a single location.
 *
 * @author ray
 */
public class Trace {
  ...
  /**
   * How much information to print about the wmes matching an instantiation. These values can
   * typically be or'd together in an EnumSet.
   *
  ...
  /**
   * Control category tracing. If set to false, no tracing will be performed for the given category.
   * Setting this to true will also change the value of {@link #isEnabled()} to true.
   *
   * @param c The category
   * @param enabled True to enable tracing for the given category, false to disable tracing for the
   *     given category
   */
  ...
  /**
   * Set the trace to the given watch level, an integer in [0, 5] inclusive. All trace categories
   * that are watchable and active at the given watch level will be enabled. All trace categories
   * that are inactive at the given watch will be disabled.
   *
  ...
  public Trace setWatchLevel(int watchLevel) {
    Arguments.check(
        0 <= watchLevel && watchLevel <= 5, "watch level must be in 0, 1, 2, 3, 4, or 5");
    this.enabled = true;
    for (Category c : Category.values()) {
      if (c.isWatchable()) {
  ...
  /**
   * A wrapper object we put around all trace arguments that implement {@link Traceable}. This gives
   * us a way to give the traceable access to the Trace object in the context of the Java
   * formattable framework.
   *
   * @author ray
   */
