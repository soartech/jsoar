...
public class Printer {
  ...
  private final Writer startOfLineDetector =
      new Writer() {
   ...
   *
   * @param writer The writer to write to. If null, then a NullWriter is used and all output will be
   *     dropped.
   */
  ...
  /**
   * Return the current writer. Writing to this is equivalent to calling {@link #print(String)} and
   * friends. That is, persistent writers will still be called.
   *
   * <p>Note that this is not necessarily the same writer as was last passed to {@link
   * #pushWriter(Writer)}.
   *
  ...
  /**
   * Push a new writer to print to. The current writer is flushed and then pushed onto a stack. It
   * can be restored with {@link #popWriter()}
   *
   * @param writer The new writer to write to. If null, the a NullWriter is used and all output will
   *     be dropped.
   */
  ...
  /**
   * Pop the current writer from the writer stack. The current writer is flushed and returned. The
   * writer currently on the top of the stack becomes the new writer.
   *
  ...
  /**
   * Add a writer that will receive <em>all</em> print output regardless of push/pop.
   *
  ...
  /**
   * gsysparam.h:132:PRINT_WARNINGS_SYSPARAM
   *
