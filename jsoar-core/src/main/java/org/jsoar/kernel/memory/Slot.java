 ...
 * Fields in a slot:
 *
 * * `next`, `prev`: used for a doubly-linked list of all slots for a certain identifier.
 * * `id`, `attr`: identifier and attribute of the slot
 * * `wmes`: header of a doubly-linked list of all wmes in the slot
 * * `acceptable_preference_wmes`: header of doubly-linked list of all acceptable preference wmes in the slot.
 * (This is only used for context slots.)
 * * `all_preferences`: header of a doubly-linked list of all preferences currently in the slot
 * * `preferences[NUM_PREFERENCE_TYPES]`: array of headers of doubly-linked lists, one for each possible type of
 * preference.  These store all the preferences, sorted into lists according to their types. Within each list, the
 * preferences are sorted according to their match goal, with the pref. supported by the highest goal at the head
 * of the list.
 * * `CDPS`: a dll of preferences in the context-dependent preference set, which is the set of all preferences that
 * contributed to an operator's selection. This is used to allow Soar to backtrace through evaluation rules in
 * substates. The rules that determine which preferences are in the CPSD are outlined in run_preference_semantics().
 * * `impasse_id`: points to the identifier of the attribute impasse object for this slot.  (NIL if the slot isn't
 * impassed.)
 * * `isa_context_slot`: TRUE if this is a context slot
 * * `impasse_type`: indicates the type of the impasse for this slot. This is one of NONE_IMPASSE_TYPE,
 * CONSTRAINT_FAILURE_IMPASSE_TYPE, etc.
 * * `marked_for_possible_removal`: TRUE if this slot is on the list of slots that might be deallocated at the end of
 * the current top-level
 * phases.
 * * `changed`: indicates whether the preferences for this slot have changed. For non-context slots, this is either NIL
 * or a pointer to the corresponding dl_cons in changed_slots (see decide.c); for context slots, it's just a
 * zero/nonzero flag.
 * * `acceptable_preference_changed`: for context slots only; this is  zero if no acceptable or require preference in
 * this slot has changed; if one has changed, it points to a dl_cons.
 *
 * `gdatastructs.h:288`
 *
 * @author ray
...
public class Slot {
     ...
     *         the id of the slot
     * @param attr
     *         the attribute of the slot
     * @param operator_symbol
     *         the operator symbol from {@link PredefinedSymbols}
     ...
     *         the slot attribute
     * @param operator_symbol
     *         the operator symbol from {@link PredefinedSymbols}. May be null
     ...
     * returns it if found.  If no such slot exists, it returns `null`.
     *
     ...
     * @param after
     *         If non-null, the preference is added <b>after</b> this one.
     */
