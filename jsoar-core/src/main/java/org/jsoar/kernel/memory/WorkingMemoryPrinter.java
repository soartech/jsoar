...
public class WorkingMemoryPrinter {
  ...
  public void print(Agent agent, Printer printer, Symbol idIn, String pattern) {
    ...
    if (idIn == null) {
      ...
      List<Wme> wmes =
          Wmes.filter(
              agent.getAllWmesInRete().iterator(),
      ...
      if (internal) {
        ...
        for (Symbol id : objects.keySet()) {
          if (exact) {
            ...
            // note: this looks like it's doing a strange thing (i.e., it will print the same things
            // multiple times), but that is in fact the csoar behavior
  ...
  /**
   * RPM 4/07 bug 988 This function traverses the ids we are going to print and marks each with its
   * shallowest depth That is, if an id can be reached by multiple paths, this will find the
   * shortest one and save the depth of that path on the id. Thus, when we print, the wmes will be
   * indented properly, making it much easier to read, and avoiding bugs (see bug 988).
   *
   * <p>sml_KernelHelpers.cpp:290:mark_depths_augs_of_id
   */
  ...
  private void print_augs_of_id(
    ...
    int indent =
        (maxdepth - depth) * 2; // set the indent based on how deep we are, RPM 4/07 bug 988
    ...
    if (tree) {
      ...
      for (attr = 0; attr < num_attr; attr++) {
        ...
        if (internal) {
          ...
          // XML format of an <id> followed by a series of <wmes> each of which shares the original
          // ID.
          // <id id="s1"><wme tag="123" attr="foo" attrtype="string" val="123"
          // valtype="string"></wme><wme attr="bar" ...></wme></id>
