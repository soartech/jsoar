...
public class OSupport {
  ...
  /**
   * SBH 4/14/93 For NNPSCM, we must exclude the operator slot from the transitive closure of a
   * state. Do that by passing a boolean argument, "isa_state" to this routine. If it isa_state,
  ...
  private void add_to_os_tc(IdentifierImpl id, boolean isa_state) {
    ...
    for (ListItem<Preference> pit = rhs_prefs_from_instantiation.first;
        pit != null;
        pit = pit.next) {
   ...
   *
   * <p>This routine calculates o-support for each preference for the given instantiation, filling
   * in pref.o_supported (true or false) on each one.
   *
   * <p>The following predicates are used for support calculations. In the following, "lhs has some
   * elt. ..." means the lhs has some id or value at any nesting level.
   *
   * <pre>
   *  lhs_oa_support:
                ...
                if ((w.attr == syms.operator_symbol)
                  ...
                  for (act = inst.prod.getFirstAction(); act != null; act = act.next) {
                    ...
                    if (ma != null) {
                      ...
                      /*o_support_calculation_type == 4 &&*/ reteLoc != null
  ...
  // RPM 5/2010: Removed lots of old, unused code (600-700 lines) intended to provide compile time
  // o-support calculations.
  //             Laird thinks it was originally intended to make the code faster, but that this is
  // less of an issue on modern machines.
