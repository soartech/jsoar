 ...
 * {@link org.jsoar.kernel.io.InputBuilder InputBuilder} but does not add WMEs
 * directly to the input-link. Instead a WME structure is built and the root
 * identifier is returned by {@link #topId()}.
 * 
...
public class WmeBuilder<T> {
	...
	private WmeBuilder(WmeFactory<T> wmeFactory, WmeBuilder<T> parent,
			...
			Map<String, T> wmeMap) {
		final Identifier newId = wmeFactory.getSymbols().createIdentifier(
				Symbols.getFirstLetter(rootAttr));
	 ...
	 * 
	 * <p>
	 * This method returns a new InputBuilder2 rooted at the newly created
