...
/**
 * A builder object for constructing WME structures. Similar to {@link
 * org.jsoar.kernel.io.InputBuilder InputBuilder} but does not add WMEs directly to the input-link.
 * Instead a WME structure is built and the root identifier is returned by {@link #topId()}.
 *
 * @author chris.kawatsu
 * @param <T> the type of WME to build.
 * @see WmeFactory
 */
public class WmeBuilder<T> {
  ...
  /**
   * Construct a new builder object that starts building WMEs at the given identifier name. Use
   * {@link #topId()} to get a reference to the new identifier.<br>
   * <br>
  ...
  public static <U> WmeBuilder<U> create(WmeFactory<U> wmeFactory, String rootName) {
    return new WmeBuilder<U>(
        wmeFactory, null, rootName, new HashMap<String, Identifier>(), new HashMap<String, U>());
  }

  /**
   * Construct a new builder object that starts building WMEs at the given identifier. <br>
   * <br>
   * <i>Note: WmeBuilder objects are immutable.</i>
   *
   * @param wmeFactory The wmeFactory
  ...
  public static <U> WmeBuilder<U> create(WmeFactory<U> wmeFactory, Identifier root) {
    return new WmeBuilder<U>(
        wmeFactory, null, root, new HashMap<String, Identifier>(), new HashMap<String, U>());
  }

  private WmeBuilder(
      WmeFactory<T> wmeFactory,
      WmeBuilder<T> parent,
      String rootAttr,
      Map<String, Identifier> idMap,
      Map<String, T> wmeMap) {
    final Identifier newId =
        wmeFactory.getSymbols().createIdentifier(Symbols.getFirstLetter(rootAttr));
  ...
  /**
   * Construct a new WME from the current id with the given attribute and value. The symbols for the
   * attribute and value are constructed according to the rules of {@link
   * Symbols#create(org.jsoar.kernel.symbols.SymbolFactory, Object)}.
   *
  ...
  public WmeBuilder<T> add(Object attr, Object value) {
    ...
    wmeMap.put(
        null, wmeFactory.addWme(id, Symbols.create(syms, attr), Symbols.create(syms, value)));
    return this;
  }

  /**
   * Construct a new object WME from the current id with the given attribute and a newly generated
   * identifier. The symbols for the attribute are constructed according to the rules of {@link
   * Symbols#create(org.jsoar.kernel.symbols.SymbolFactory, Object)}.
   *
   * <p>This method returns a new InputBuilder2 rooted at the newly created identifier. Use {@link
   * #pop()} to return to the current identifier.
   *
   * @param attr the attribute
  ...
  public WmeBuilder<T> push(Object attr) {
    ...
    final SymbolFactory syms = wmeFactory.getSymbols();
    wmeMap.put(
        null, wmeFactory.addWme(id, Symbols.create(syms, attr), Symbols.create(syms, newId)));
   ...
   *
   * @param name The name of the WME, or <code>null</code> for the most recently created WME
  ...
  /**
   * Pop up one level in the builder stack, i.e. return to the state of the builder before the most
   * recent call to {@link #push(Object)}.
   *
  ...
  /**
   * Jump to start building WMEs at an identifier previously marked with {@link #markId(String)}.
   *
   ...
   * @throws IllegalArgumentException if no identifier with the given name exists.
   */
