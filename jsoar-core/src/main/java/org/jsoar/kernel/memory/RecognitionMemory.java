 ...
 *
 * <p>Recognition Memory (Firer and Chunker) Routines (Does not include the Rete net)
 *
 * <p>Init_firer() and init_chunker() should be called at startup time, to do initialization.
 *
 * <p>Do_preference_phase() runs the entire preference phases. This is called from the top-level
 * control in main.c.
 *
 * <p>Possibly_deallocate_instantiation() checks whether an instantiation can be deallocated yet,
 * and does so if possible. This is used whenever the (implicit) reference count on the
 * instantiation decreases.
 *
...
public class RecognitionMemory {
   ...
   */
  private Instantiation newly_created_instantiations;

  /**
   * during firing, points to the prod. being fired
   *
   * <p>agent.h:574:production_being_fired
   */
  private Production production_being_fired;

  /** agent.h:367:production_firing_count */
  private final LongPropertyProvider production_firing_count =
      new LongPropertyProvider(SoarProperties.PRODUCTION_FIRING_COUNT);
  ...
  /**
   * Preference type of currently executing action, used when instantiating preferences from RHS
   * functions
   */
  ...
  /**
   * As the "firer", this object is in charge of implementing the {@link RhsFunctionContext} that is
   * passed to RHS functions when they are executed.
   *
  ...
  public void initialize() {
    ...
    context
        .getProperties()
        .setProvider(SoarProperties.PRODUCTION_FIRING_COUNT, production_firing_count);
  }
    ...
    for (Condition cond = inst.top_of_instantiated_conditions; cond != null; cond = cond.next) {
      ...
      if (pc != null && bt.trace != null && bt.trace.slot != null) {
        if (this.chunker.chunkThroughEvaluationRules) {
          ...
          var pref = bt.trace.slot.getPreferencesByType(PreferenceType.PROHIBIT);
          while (pref != null) {
            Preference new_pref;
            if (pref.inst.match_goal_level == inst.match_goal_level && pref.isInTempMemory()) {
              bt.addContextDependentPreference(pref);
  ...
  /**
   * Given an instantiation, this routines looks at the instantiated conditions to find its match
   * goal. It fills in {@code inst->match_goal} and {@code inst->match_goal_level}. If there is a
   * match goal, match_goal is set to point to the goal identifier. If no goal was matched,
   * match_goal is set to NIL and match_goal_level is set to ATTRIBUTE_IMPASSE_LEVEL.
   *
   * <p>TODO Make a method of Instantiation?
   *
  ...
  /**
   * Execute_action() executes a given RHS action. For MAKE_ACTION's, it returns the created
   * preference structure, or NIL if an error occurs. For FUNCALL_ACTION's, it returns NIL.
   *
   * <p>Instantiate_rhs_value() returns the (symbol) instantiation of an rhs_value, or NIL if an
   * error occurs. It takes a new_id_level argument indicating what goal_stack_level a new id is to
   * be created at, in case a gensym is needed for the instantiation of a variable. (although I'm
   * not sure this is really needed.)
   *
   * <p>As rhs unbound variables are encountered, they are instantiated with new gensyms. These
   * gensyms are then stored in the rhs_variable_bindings array, so if the same unbound variable is
   * encountered a second time it will be instantiated with the same gensym.
   *
   * <p>recmem.cpp:195:instantiate_rhs_value
   *
   ...
   */
  public SymbolImpl instantiate_rhs_value(
      RhsValue rv, int new_id_level, char new_id_letter, Token tok, WmeImpl w) {
    ...
    final var uv = rv.asUnboundVariable();
    if (uv != null) {
    ...
    final var rl = rv.asReteLocation();
    if (rl != null) {
    ...
    if (!nil_arg_found) {
      ...
      try {
        ...
        context
            .getPrinter()
            .error(
                "Error executing RHS function '%s' with args %s: %s\n",
  ...
  private Preference execute_action(Action a, Token tok, WmeImpl w) {
    ...
    final var fa = a.asFunctionAction();
    if (fa != null) {
    ...
    if (id == null) {
      context
          .getPrinter()
          .error(
              "[%s] RHS makes a preference for %s (not an identifier)\n",
    ...
    if (a.preference_type != PreferenceType.ACCEPTABLE
      ...
      context
          .getPrinter()
          .error(
              "[%s] attribute preference other than +/- for %s ^%s -- ignoring it.",
              production_being_fired, id, attr);
      return null;
    }
  ...
  /**
   * This routine fills in a newly created instantiation structure with various information.
   *
   ...
   *   <li>top-level positive conditions should have bt.wme_, bt.level, and bt.trace filled in, but
   *       bt.wme_ and bt.trace shouldn't have their reference counts incremented yet.
   * </ul>
   *
   ...
   *       </ul>
   *   <li>for each preference_generated, adds that pref to the list of all pref's for the match
   *       goal
   *   <li>fills in backtrace_number;
   *   <li>if "need_to_do_support_calculations" is TRUE, calculates o-support for
   *       preferences_generated;
   * </ul>
   *
   * <p>recmem.cpp:385:fill_in_new_instantiation_stuff
   */
  public void fill_in_new_instantiation_stuff(
      Instantiation inst, boolean need_to_do_support_calculations) {
  ...
  /**
   * This builds the instantiation for a new match, and adds it to newly_created_instantiations. It
   * also calls chunk_instantiation() to do any necessary chunk or justification building.
   *
  ...
  private void create_instantiation(Production prod, Token tok, WmeImpl w) {
    ...
    final var inst = new Instantiation(prod, tok, w);
    this.newly_created_instantiations =
        inst.insertAtHeadOfProdList(this.newly_created_instantiations);
    ...
    ConditionsAndNots cans =
        this.rete.p_node_to_conditions_and_nots(prod.getReteNode(), tok, w, false);
    ...
    boolean trace_it =
        trace.isEnabled(inst.prod.getType().getTraceCategory()) || inst.prod.isTraceFirings();
    if (trace_it) {
    ...
    var need_to_do_support_calculations = false;
    for (var a = prod.getFirstAction(); a != null; a = a.next) {
      Preference pref;
  ...
  /**
   * Returns true if the function create_instantiation should run for this production. Used to delay
   * firing of matches in the inner preference loop.
   *
  ...
  private boolean shouldCreateInstantiation(Production prod, Token tok, WmeImpl w) {
    ...
    for (var a = prod.getFirstAction(); a != null; a = a.next) {
      ...
      if (rsv != null) {
        ...
        var reteLocation = ma.id.asReteLocation();
        if (reteLocation != null) {
          idSym = reteLocation.lookupSymbol(tok, w);
        } else {
  ...
  /**
   * This deallocates the given instantiation. This should only be invoked via the
   * possibly_deallocate_instantiation() macro.
   */
  private void deallocate_instantiation(Instantiation inst) {
    ...
    LinkedList<Condition> cond_stack = new LinkedList<>();
          ...
          if (SoarConstants.DO_TOP_LEVEL_REF_CTS && level <= SoarConstants.TOP_GOAL_LEVEL) {
            ...
            if (trace != null) {
              ...
              if (trace.reference_count == 0) {
                ...
                var has_active_clones = false;
                for (Preference clone = trace.next_clone;
                    (clone != null) && !has_active_clones;
                    clone = clone.next_clone) {
                  if (clone.reference_count != 0) {
                    has_active_clones = true;
                  }
                }
                for (Preference clone = trace.prev_clone;
                    (clone != null) && !has_active_clones;
                ...
                // The clones are hopefully a simple case so we just call deallocate_preference on
                // them.
    ...
    for (Instantiation temp : inst_list) {
      temp.top_of_instantiated_conditions =
          null; //  deallocate_condition_list (thisAgent, inst->top_of_instantiated_conditions);
      temp.bottom_of_instantiated_conditions =
          null; // This is very important to avoid memory leaks!
  ...
  void possibly_deallocate_instantiation(Instantiation inst) {
    if (inst.preferences_generated == null && !inst.in_ms) deallocate_instantiation(inst);
  }

  /**
   * This retracts the given instantiation.
   *
   * <p>recmem.cpp:814:retract_instantiation
   */
  private void retract_instantiation(Instantiation inst) {
    ...
    var retracted_a_preference = false;

    final var trace = context.getTrace();
    final boolean trace_it =
        trace.isEnabled(inst.prod.getType().getTraceCategory()) || inst.prod.isTraceFirings();
    ...
    if (inst.prod.getType()
        == ProductionType.JUSTIFICATION /*&& inst.prod.getReferenceCount() > 1*/) {
  ...
  /**
   * This routine scans through newly_created_instantiations, asserting each preference generated
   * except for o-rejects. It also removes each instantiation from newly_created_instantiations,
   * linking each onto the list of instantiations for that particular production. O-rejects are
   * buffered and handled after everything else.
   *
   * <p>Note that some instantiations on newly_created_instantiations are not in the match set--for
   * the initial instantiations of chunks/justifications, if they don't match WM, we have to assert
   * the o-supported preferences and throw away the rest.
   *
   * <p>recmem.cpp:891:assert_new_preferences
   *
   * <p>bufdeallo Buffered preferences to deallocate after inner elaboration loop.
   */
  private void assert_new_preferences(List<Preference> bufdeallo) {
    ...
    // Note: In CSoar there is some random code commented out at this point. Is it important? Who
    // knows?
  ...
  /**
   * Process_o_rejects_and_deallocate_them() handles the processing of o-supported reject
   * preferences. This routine is called from the firer and passed a list of all the o-rejects
   * generated in the current preference phases (the list is linked via the "next" fields on the
   * preference structures). This routine removes all preferences for matching values from TM, and
   * deallocates the o-reject preferences when done.
   *
   * <p>prefmem.cpp:330:process_o_rejects_and_deallocate_them
   *
   * @param bufdeallo Preferences buffered for deallocation after inner elaboration loop completes.
   */
  private void process_o_rejects_and_deallocate_them(
      List<Preference> o_rejects, List<Preference> bufdeallo) {
    ...
    for (Preference pref : o_rejects) {
      var s = Slot.find_slot(pref.id, pref.attr);
      if (s != null) {
        // remove all pref's in the slot that have the same value
        var p = s.getAllPreferences();
        while (p != null) {
  ...
  /**
   * Add_preference_to_tm() adds a given preference to preference memory (and hence temporary
   * memory).
   *
   * <p>prefmem.cpp:216:add_preference_to_tm
   *
   * <p>TODO I wish this was private, but smem and epmem use it.
   */
  public void add_preference_to_tm(Preference pref) {
    ...
    var s = Slot.make_slot(pref.id, pref.attr, predefinedSyms.operator_symbol);
    ...
    if (wma.wma_enabled() && !s.isa_context_slot) {
      ...
      if (!exists) {
        if (s.wma_val_references == null) {
          s.wma_val_references = new HashMap<>();
        }
   ...
   *
   * <p>prefmem.cpp:282:remove_preference_from_tm
   */
  ...
  public void do_preference_phase(IdentifierImpl top_goal) {
    ...
    if (trace.isEnabled(Category.PHASES)
        && (this.decisionCycle.current_phase.get() == Phase.APPLY)) {
      /* it's always IE for PROPOSE */
      switch (FIRING_TYPE) {
        case PE_PRODS:
          trace
              .startNewLine()
              .print("--- Firing Productions (PE) For State At Depth %d ---", decider.active_level);
          break;
        case IE_PRODS:
          trace
              .startNewLine()
              .print("--- Firing Productions (IE) For State At Depth %d ---", decider.active_level);
          break;
        default:
          // do nothing
          break;
    ...
    for (; ; ) {
      ...
      if (trace.isEnabled(Category.WATERFALL)) {
        trace.print("--- Inner Elaboration Phase, active level: %d", decider.active_level);
      ...
      SoarReteAssertion assertion;
      var assertionsExist = false;
      ...
      if (assertionsExist && (decider.active_level > decider.next_change_level)) {
      ...
      if (this.decisionCycle.current_phase.get() == Phase.APPLY) {
        decider.active_goal =
            this.consistency.highest_active_goal_apply(
                decider.active_goal.goalInfo.lower_goal, true);
      } else if (this.decisionCycle.current_phase.get() == Phase.PROPOSE) {
        // PROPOSE
        decider.active_goal =
            this.consistency.highest_active_goal_propose(
                decider.active_goal.goalInfo.lower_goal, true);
      }
    ...
    for (Preference p : bufdeallo) p.preference_remove_ref(this);
