...
public class Preference implements Formattable {
  ...
  /**
   * next pointer for list of preferences in a slot by type. Use this when iterating over the list
   * head returned by {@link Slot#getPreferencesByType(PreferenceType)}
   */
  ...
  /**
   * next pointer for list of all preference in a slot. Use this when iterating over the list head
   * returned by {@link Slot#getAllPreferences()}
   */
  ...
  /**
   * Make_preference() creates a new preference structure of the given type with the given
   * id/attribute/value/referent. (Referent is only used for binary preferences.) The preference is
   * not yet added to preference memory, however.
   *
   * <p>prefmem.cpp:58:make_preference
   *
   * @param type
   * @param id
   * @param attr
  ...
  /**
   * This routines take a given preference and finds the clone of it whose match goal is at the
   * given goal_stack_level. (This is used to find the proper preference to backtrace through.) If
   * the given preference itself is at the right level, it is returned. If there is no clone at the
   * right level, NIL is returned.
   *
  ...
  /**
   * Possibly_deallocate_preference_and_clones() checks whether a given preference and all its
   * clones have reference_count 0, and deallocates them all if they do. It returns TRUE if they
   * were actually deallocated, FALSE otherwise.
   *
   * <p>prefmem.cpp:141:possibly_deallocate_preference_and_clones
   */
  ...
  /**
   * Remove_preference_from_clones() splices a given preference out of the list of clones. If the
   * preference's reference_count is 0, it also deallocates it and returns TRUE. Otherwise it
   * returns FALSE.
   *
   * <p>prefmem.cpp:176:remove_preference_from_clones
   *
   * @param recMemory
  ...
  public boolean remove_preference_from_clones(RecognitionMemory recMemory) {
    ...
    if (any_clone != null) possibly_deallocate_preference_and_clones(any_clone, recMemory);
    if (pref.reference_count == 0) {
      deallocate_preference(pref, recMemory);
      return true;
    } else {
