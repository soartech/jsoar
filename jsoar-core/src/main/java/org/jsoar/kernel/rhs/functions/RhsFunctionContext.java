...
/**
 * Context interface passed to all RHS functions when they are executed. This interface provides
 * access to the symbol factory as well as methods that allow RHS functions to safely add new WMEs
 * to working memory.
 *
 * @author ray
...
public interface RhsFunctionContext extends WmeFactory<Void> {
  /**
   * Add a WME from the RHS function. The WME is given whatever support and preference type is given
   * to the action containing the RHS function call.
   *
  ...
  /**
   * Returns the production that is currently firing, causing the RHS function to execute.
   *
