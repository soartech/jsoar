...
/**
 * Implementation of <b>deep-copy</b> RHS function in Soar 9.0.0 originally implemented by SoarTech.
 *
 * <p><b>deep-copy</b> takes a single identifier argument and returns a copy of the working memory
 * structure reachable from that identifier. Impasse WMEs (those WMEs in the <code>impasse_wmes
 * </code> list of <code>IdentifierImpl</code> are not copied. The created WMEs have the same
 * support as normal WMEs created by the production instantiation that is firing. That is,
 * <b>deep-copy</b> can be thought of as a macro that generates the RHS actions to copy the working
 * memory reachable from a particular id.
 *
...
public class DeepCopy extends AbstractRhsFunctionHandler {
   ...
   * @return New identifier for id. That is, the identifier in the copy that id maps to in idMap.
   */
  private Identifier copy(
      RhsFunctionContext context, Identifier id, Map<Identifier, Identifier> idMap) {
