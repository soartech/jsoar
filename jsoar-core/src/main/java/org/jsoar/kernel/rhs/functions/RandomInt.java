...
/**
 * {@code (random-int)} RHS function. This is deprecated, use rand-int instead for better csoar
 * compatibility.
 *
 * <p>Has the following usage:
 *
 * <ul>
 *   <li>{@code (random-int)} - No args, returns a random integer in {@code [-Integer.MIN_VALUE,
 *       Integer.MAX_VALUE)}.
 *   <li>{@code (random-int N)} - returns a random integer in {@code [0, N)} if N is positive and a
    ...
    if (arguments.size() == 0) {
      ...
      if (maxSym == null) {
        throw new RhsFunctionException(
            "random-int: Expected integer for first argument, got " + arguments.get(0));
      }
