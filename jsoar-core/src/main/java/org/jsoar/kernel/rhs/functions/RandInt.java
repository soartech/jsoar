 ...
 *
 * <ul>
 *   <li>{@code (rand-int)} - No args, returns a random integer in {@code [-Integer.MIN_VALUE,
 *       Integer.MAX_VALUE)}.
 *   <li>{@code (rand-int N)} - returns a random integer in {@code [0, N]} if N is positive and a
 *       random integer in {@code [N, 0]} if N is negative.
 * </ul>
 *
 * <p>Csoar compatibility notes:
 *
 * <ul>
 *   <li>When no argument is supplied, csoar will generate values in {@code [-2^31, 2^31]} (note the
 *       inclusive endpoint).
 *   <li>Csoar doesn't properly support negative arguments like jsoar does (it seems to wrap them
 *       around to large positive numbers).
 * </ul>
 *
 * @author marinier
    ...
    if (arguments.size() == 0) {
      ...
      if (maxSym == null) {
        throw new RhsFunctionException(
            "rand-int: Expected integer for first argument, got " + arguments.get(0));
      }
