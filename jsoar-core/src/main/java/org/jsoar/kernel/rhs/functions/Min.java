 ...
 *
 * <p>This function takes one or more numeric arguments and returns the minimum value. In the case
 * of mixed int/double arguments, the type of the minimum argument is preserved.
 *
 * <p>For example:
 *
 * <pre>{@code
...
public class Min extends AbstractRhsFunctionHandler {
  ...
  public Symbol execute(RhsFunctionContext context, List<Symbol> arguments)
    ...
    return useDouble
        ? context.getSymbols().createDouble(minDouble)
        : context.getSymbols().createInteger(minInt);
  }
}
