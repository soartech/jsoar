 ...
 *
 * <p>This function takes one or more numeric arguments and returns the maximum value. In the case
 * of mixed int/double arguments, the type of the maximum argument is preserved.
 *
 * <p>For example:
 *
 * <pre>{@code
...
public class Max extends AbstractRhsFunctionHandler {
  ...
  public Symbol execute(RhsFunctionContext context, List<Symbol> arguments)
    ...
    return useDouble
        ? context.getSymbols().createDouble(maxDouble)
        : context.getSymbols().createInteger(maxInt);
  }
}
