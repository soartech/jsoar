...
/**
 * A right hand side function that asks the user for input and returns the result as a string.
 *
 * @author ray
 */
public class AcceptRhsFunction extends AbstractRhsFunctionHandler {
  ...
  public Symbol execute(RhsFunctionContext context, List<Symbol> arguments)
    ...
    final String message =
        arguments.isEmpty() ? "Input requested by agent:" : arguments.get(0).toString();
  ...
  private String askUserForInput(final RhsFunctionContext context, final String message)
    ...
    if (SwingUtilities.isEventDispatchThread()) {
      ...
      SwingUtilities.invokeAndWait(
          new Runnable() {
            ...
            @Override
            public void run() {
              result.set(askUserForInputFromEventThread(context, message));
            }
          });
      return result.get();
    }
