...
public class ActionReorderer {
   ...
   */
  public void reorder_action_list(ByRef<Action> action_list, Marker lhs_tc)
    ...
    if (remaining_actions != null) {
      ...
      if (last_action != null) last_action.next = remaining_actions;
      else first_action = remaining_actions;
  ...
  private boolean legal_to_execute_action(Action a, Marker tc) {
    ...
    if (ma != null) {
      ...
      if (ma.attr.asFunctionCall() != null && (!all_variables_in_rhs_value_bound(ma.attr, tc))) {
        return false;
      }
      if (ma.value.asFunctionCall() != null && (!all_variables_in_rhs_value_bound(ma.value, tc))) {
        return false;
      }
    ...
    return all_variables_in_rhs_value_bound(a.asFunctionAction().getCall(), tc);
  }
