...
public abstract class Action {
  ...
  /**
   * Tests whether two RHS's (i.e., action lists) are the same (except for function calls). This is
   * used for finding duplicate productions.
   *
    ...
    while (a1 != null && a2 != null) {
      ...
      if (ma1.preference_type.isBinary()) {
        ...
        //	to implement the effect of the chunk-stop RL parameter
        {
          boolean stop = true;
          if (rl_chunk_stop) {
            if (ma1.referent.asSymbolValue() != null && ma2.referent.asSymbolValue() != null) {
  ...
  /**
   * TODO This function doesn't belong here. Circular dependency on rete package TODO This function
   * should be polymorphic on Action
   *
   ...
   */
  public static Action copy_action_list_and_substitute_varnames(
      Rete rete, Action actions, Condition cond) {
    Action prev = null;
    Action first = null;
    Action old = actions;
    Action New = null;
    while (old != null) {
      if (old instanceof MakeAction) {
        ...
        newMake.attr =
            RhsValues.copy_rhs_value_and_substitute_varnames(rete, oldMake.attr, cond, 'a');
        char first_letter = newMake.attr.getFirstLetter();
        newMake.value =
            RhsValues.copy_rhs_value_and_substitute_varnames(
                rete, oldMake.value, cond, first_letter);
        if (old.preference_type.isBinary()) {
          newMake.referent =
              RhsValues.copy_rhs_value_and_substitute_varnames(
                  rete, oldMake.referent, cond, first_letter);
        }
  ...
  public static void print_action_list(
      Printer printer, Action actions, int indent, boolean internal) {
    ...
    while (!actions_not_yet_printed.isEmpty()) {
      ...
      while (!actions_for_this_id.isEmpty()) {
        ...
        { // build and print attr/value test for action a
          ...
          gs.append(" ^");
          gs.append(
              String.format("%s %s %c", ma2.attr, ma2.value, ma2.preference_type.getIndicator()));
