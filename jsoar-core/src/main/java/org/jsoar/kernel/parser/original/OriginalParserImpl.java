...
class OriginalParserImpl {
  ...
  private SymbolImpl make_symbol_for_current_lexeme(boolean idLti) throws ParserException {
    switch (currentType()) {
      ...
      case IDENTIFIER:
        {
          if (!idLti) {
            throw new IllegalStateException(
                "Internal error:  ID found in make_symbol_for_current_lexeme");
          } else {
      ...
      default:
        throw new IllegalStateException(
            "bad lexeme type in make_symbol_for_current_lexeme: " + current());
    }
  }
    ...
    try {
      ...
      if (lti_id == 0) {
        throw new ParserException(
            "Internal error: invalid long-term identifier found: " + current().string);
      } else {
        return ltis.smem_lti_soar_make(
            lti_id,
            current().id_letter,
            current().id_number,
      ...
      throw new ParserException(
          "While retrieving long-term identifier '" + current().string + "': " + e.getMessage(), e);
    }
  }
  ...
  /**
   * In attribute paths (and some other places) we need to create dummy variables. But we need to
   * make sure these dummy variables don't accidently have the same names as variables that occur
   * later in the user's production. So, we create "placeholder" variables, whose names have funky
   * characters in them so they couldn't possibly occur in user-written code. When we're all done
   * parsing the production, we go back and replace the placeholder variables with "real" variables
   * (names without funky characters), making sure the real variables don't occur anywhere else in
   * the production.
   */
  ...
  /** Creates and returns a test for equality with a newly generated placeholder variable. */
  private Test make_placeholder_test(char first_letter) {
    ...
    final String namebuf =
        ("<#" + first_letter) + (placeholder_counter[first_letter - 'a']++ + ">");
  ...
  /**
   * When done parsing the production, we go back and substitute "real" variables for all the
   * placeholders. This is done by walking all the LHS conditions and destructively modifying any
   * tests involving placeholders. The placeholder--{@literal >}real mapping is maintained on each
   * placeholder symbol: {@code placeholder->var.current_binding_value} is the corresponding "real"
   * variable, or null if no such "real" variable has been created yet.
   *
   * <p>To use this, first call reset_variable_generator (lhs, rhs) with the lhs and rhs of the
   * production just parsed; then call {@code substitute_for_placeholders_in_condition_list (lhs)}.
   *
  ...
  private Test substitute_for_placeholders_in_test(Test t) {
    ...
    if (eqTest != null) {
      return SymbolImpl.makeEqualityTest(
          substitute_for_placeholders_in_symbol(eqTest.getReferent()));
    }
      ...
      case IDENTIFIER:
        {
          ...
          lexer.getNextLexeme();
          return use_equality_test
              ? SymbolImpl.makeEqualityTest(referent)
              : new RelationalTest(test_type, referent);
        }
      default:
  ...
  /**
   * As low-level routines (e.g., parse_value_test_star) parse, they leave the id (and sometimes
   * attribute) test fields blank (null) in the condition structures they return. The calling
   * routine must fill in the id tests and/or attribute tests. These routines fill in any
   * still-blank {id, attr} tests with copies of a given test. They try to add non-equality portions
  ...
  /**
   * Returns the negation of the given condition list. If the given list is a single positive or
   * negative condition, it just toggles the type. If the given list is a single ncc, it strips off
   * the ncc part and returns the subconditions. Otherwise it makes a new ncc using the given
   * conditions.
   *
  ...
  /**
   * This routine parses {@code <value_test>* }, given as input the id_test and attr_test already
   * read.
   *
  ...
  Condition parse_value_test_star(char first_letter) throws IOException, ParserException {
    final EnumSet<LexemeType> endOfTest =
        EnumSet.of(LexemeType.MINUS, LexemeType.UP_ARROW, LexemeType.R_PAREN);
    ...
    do {
      ...
      if (currentType() == LexemeType.L_PAREN) {
        ...
        if (!Tests.test_includes_equality_test_for_symbol(value_test.value, null)) {
          value_test.value =
              Tests.add_new_test_to_test(value_test.value, make_placeholder_test(first_letter));
        }
      }
  ...
  /**
   * This routine parses the {@code "( [state|impasse] [<id_test>]" } part of {@code
   ...
   * @return a new test
   */
  private Test parse_head_of_conds_for_one_id(char first_letter_if_no_id_given)
    ...
    if (currentType() != LexemeType.MINUS
      ...
      if (!Tests.test_includes_equality_test_for_symbol(id_test, null)) {
        id_test =
            Tests.add_new_test_to_test(id_test, make_placeholder_test(first_letter_if_no_id_given));
      } else {
        ...
        if (sym.asVariable() == null) {
          error(
              String.format(
                  "Warning: Constant %s in id field test.\n    This will never match.", sym));
        }
      }
    } else {
  ...
  /**
   * This routine parses the {@code "<attr_value_tests>* )" } part of {@code <conds_for_one_id>} and
   * returns the resulting conditions. It does not fill in the id tests of the conditions.
   *
   * <pre>{@code
   * <conds_for_one_id> ::= ( [state|impasse] [<id_test>] <attr_value_tests>* )
   * <id_test> ::= <test>
  ...
  /**
   * This routine parses {@code <conds_for_one_id>} and returns the conditions (of null if any error
   * occurs).
   *
   * <pre>{@code
   * <conds_for_one_id> ::= ( [state|impasse] [<id_test>] <attr_value_tests>* )
   ...
   * }</pre>
   *
   * <p>If the argument dest_id_test is non-NULL, then *dest_id_test is set to the resulting
   * complete id test (which includes any goal/impasse test), and in all the conditions the id field
   * is filled in with just the equality portion of id_test.
   *
   * <p>If the argument dest_id_test is NULL, then the complete id_test is included in the
   * conditions.
   *
  ...
  Condition parse_cond_plus() throws IOException, ParserException {
    ...
    } while ((currentType() == LexemeType.MINUS)
        || (currentType() == LexemeType.L_PAREN)
  ...
  /**
   * Parses a {@code <function_call> } after the "(" has already been consumed. At entry, the
   * current lexeme should be the function name.
   *
   * <pre>{@code
   * <function_call> ::= ( <function_name> <rhs_value>* )
   * <function_name> ::= sym_constant | + | -
   ...
   * @return a rhs value
   */
  RhsFunctionCall parse_function_call_after_lparen(boolean is_stand_alone_action)
      throws IOException, ParserException {
  ...
  /**
   * Parses an {@code <rhs_value>}. Returns an rhs_value, or null if any error occurred.
   *
   * <pre>{@code
   * <rhs_value> ::= <constant> | <function_call> | variable
   * <constant> ::= sym_constant | int_constant | float_constant
  ...
  /**
   * Parses a {@code <preference-specifier> }. Returns the appropriate xxx_PREFERENCE_TYPE (see
   * soarkernel.h).
   *
   * <p>Note: in addition to the grammar below, if there is no preference specifier given, then this
   * routine returns ACCEPTABLE_PREFERENCE_TYPE. Also, for {@code <binary-preference>'s}, this
   * routine *does not* read the {@code <rhs_value> } referent. This must be done by the caller
   * routine.
   *
   * <pre>{@code
   * <preference-specifier> ::= <naturally-unary-preference> [,]
  ...
  private PreferenceType parseBinaryOrUnaryPreference(PreferenceType bin, PreferenceType unary)
      throws IOException {
    lexer.getNextLexeme();
    if (!lexer.isEof()
  ...
  private PreferenceType parseIndifferent() throws IOException {
    ...
    if (!lexer.isEof()
      ...
      else return PreferenceType.BINARY_INDIFFERENT;
    }
  ...
  /**
   * Given the id, attribute, and value already read, this routine parses zero or more {@code
   * <preference-specifier>'s }. It builds and returns an action list for these RHS make's. It
   ...
   */
  Action parse_preferences(SymbolImpl id, RhsValue attr, RhsValue value)
      throws IOException, ParserException {
  ...
  /**
   * Given the id, attribute, and value already read, this routine parses zero or more {@code
   * <preference-specifier>'s}. If preferences other than reject and acceptable are specified, it
   * prints a warning message that they are being ignored. It builds an action list for creating an
   * ACCEPTABLE preference. If binary preferences are encountered, a warning message is printed and
   * the production is ignored (returns null). It returns null if any other error occurred. This
   * works in conjunction with the code that supports attribute_preferences_mode == 2. Anywhere that
   * attribute_preferences_mode == 2 is tested, the code now tests for operand2_mode == true.
   *
   * <pre>{@code
   * <value_make> ::= <rhs_value> <preferences>
   * <preferences> ::= [,] | <preference_specifier>+
   ...
   * @return action
   */
  Action parse_preferences_soar8_non_operator(SymbolImpl id, RhsValue attr, RhsValue value)
      throws IOException, ParserException {
    /* --- Note: this routine is set up so if there's not preference type
    ...
    while (true) {
      ...
      if (preference_type.isBinary()) {
        error(
            String.format(
                "In Soar8, binary preference illegal for non-operator. "
                    + "(id = %s\t attr = %s\t value = %s)\n",
                id, attr, value));
      }
  ...
  /**
   * Given the id already read, this routine parses an {@code <attr_value_make>}. It builds and
   * returns an action list for these RHS make's. It returns null if any error occurred.
   *
   * <pre>{@code
   * <attr_value_make> ::= ^ <rhs_value> <value_make>+
   * <value_make> ::= <rhs_value> <preferences>
  ...
  /**
   * Parses a {@code <rhs_action>} and returns an action list. If any error occurrs, null is
   * returned.
   *
  ...
  /**
   * Parses the {@code <rhs>} and sets *actions to the resulting action list. Returns true if
   * successful, false if any error occurred.
   *
  ...
  /**
   * This routine reads a production (everything inside the body of the "sp" command), builds a
   * production, and adds it to the rete.
   *
  ...
  /**
   * As the parser builds the action list for the RHS, it adds each new action onto the front of the
   * list. This results in the order of the actions getting reversed. This has certain problems--for
   * example, if there are several (write) actions on the RHS, reversing their order means the
   * output lines get printed in the wrong order. To avoid this problem, we reverse the list after
   * building it.
   *
  ...
  private static class DefaultLtiSource implements LongTermIdentifierSource {
    ...
    public IdentifierImpl smem_lti_soar_make(
        long lti, char nameLetter, long nameNumber, int level) {
      throw new IllegalStateException("Long term identifiers are not supported by this parser.");
    }
  }
}
