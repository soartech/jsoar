...
public class InputWmes {
  /**
   * Create a new input Wme with the given id, attribute and value. The attribute and value are
   * created according to the rules of {@link Symbols#create(SymbolFactory, Object)}. This method
   * should only be called from inside {@link InputEvent}.
   *
  ...
  /**
   * Convenience form of {@link #add(InputOutput, Identifier, Object, Object)} which creates a new
   * input wme directly on the input-link, i.e. <code>I2</code>.
   *
   * @param io the input/output interface
   * @param attr the attribute
   * @param value the value
  ...
  /**
   * Convenience form of {@link #add(InputOutput, Identifier, Object, Object)} which creates a new
   * input wme under the given input wme.
   *
   * <p>For example, if {@code parent} is {@code (I2 ^contact C2)}, then the call {@code add(parent,
   * "name", "catbird")} will create the wme {@code (C2 ^name catbird)}
   *
   * @param parent the parent WME. Its value <b>must</b> be an {@link Identifier}
   * @param attr the attribute
   * @param value the value
   * @return the new input wme
   * @throws IllegalArgumentException if {@code parent.getValue()} is not an {@link Identifier}
   * @see Symbols#create(SymbolFactory, Object)
   */
  public static InputWme add(InputWme parent, Object attr, Object value) {
    ...
    if (id == null) {
      throw new IllegalArgumentException(
          "value of parent must be an id, got: " + parent.getValue());
    }
    return add(parent.getInputOutput(), id, attr, value);
  }

  /**
   * Update the value of the given input wme with the given value according to the rules of {@link
   * Symbols#create(SymbolFactory, Object)}. This is a convenience form of {@link
   * InputWme#update(org.jsoar.kernel.symbols.Symbol)}.
   *
