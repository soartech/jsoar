 ...
 * @author ray
 */
public class XmlTreeToWme
{
    private static final Log logger = LogFactory.getLog(XmlTreeToWme.class);
     ...
     * @param wmeFactory
     */
    public XmlTreeToWme(SymbolFactory syms, XmlWmeFactory wmeFactory)
    {
        this.syms = syms;
    ...
    public Identifier fromXmlInternal(Element element, Identifier targetId)
    {
        if(targetId == null)
        {
        ...
        for(Node node = element.getFirstChild(); node != null; node = node.getNextSibling())
        {
            ...
            final String linkTo = kid.getAttribute(WmeToXmlTree.LINK);
            ...
            if(linkTo.length() == 0)
            {
                ...
                final String link = kid.getAttribute(WmeToXmlTree.LINK_ID);
                if(link.length() != 0)
                {
