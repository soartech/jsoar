...
/**
 * Interface for a "quick memory" object. Essentially stores key/value pairs
 ...
 * </pre>
 * 
 * <p>QMemory objects are thread-safe. They may be manipulated from any thread.
 * They synchronize on themselves (this). Client code may synchronize on a QMemory
 * object to safely perform larger transactions on the object. For example, without
 * the synchronization below, an agent may see x and y in an inconsistent state:
 * 
 * <pre>
 * {@code
 * QMemory q = DefualtQMemory.create();
 ...
 * </pre>
 * 
 * <p>Credit for this concept, including all the classes in this package
 * goes to Mike Quist.
 * 
 * @author ray
 */
public interface QMemory
{
     ...
     * @return double value on the given path, or 0.0 if not found. Conversions
     *      from int and string are performed if possible.
     */
    ...
    /**
     * @param path path to search
     * @return string value on the given path, or empty string if not found.
     *      Conversions from numeric values are performed
     */
    ...
    /**
     * Returns a new QMemory that views the contents of this memory at a particular
     * path prefix.
     * 
     * <p>For example,
     * <pre>
    ...
    /**
     * Clear the value at the given path, i.e change to a non-leaf. The path
     * remains, but it no longer has a value.
     * 
    ...
    /**
     * Permanently remove the given path and all of its children from this 
     * memory
     * 
