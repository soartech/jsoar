...
/**
 * An {@link AbstractXmlFileToWme} implementation in which the XML tags with types other than string
 * are manually specified. The class does <i>not</i> support arbitrary graph structure using link
 * attributes which is present in {@link SoarTechXmlToWme}. XML tags are specified using their full
 ...
 * }</pre>
 *
 * the values of MessageValue and id would be specified as a floating point or integer using the
 * paths <code>Message.MessageValue</code> and <code>Message.id</code> respectively.<br>
 * <br>
 * Floating point and integer XML paths are added using {@link #addFloatTag(String) addFloatTag} and
 * {@link #addIntTag(String) addIntTag}. The values added as floats and those added as integers are
 * enforced to be mutually exclusive.<br>
 * <br>
 * Using {@link #fromXml(Element) fromXml} the WME structure from the above XML would be:<br>
 * <br>
 *
 * <pre>{@code
 * ^id |1|
 ...
 * }</pre>
 *
 * Note that the root XML tag is ignored, but its attributes are still added. If {@link
 * #xmlToWme(java.io.File, InputOutput) xmlToWme} is used, the message is added directly to the
 * input link and the root tag is not ignored. This results in the following WME structure:<br>
 * <br>
 *
 * <pre>{@code
 * ^io
...
public class ManualTypeXmlToWme extends AbstractXmlFileToWme {
  ...
  /**
   * Add an XML path which should be treated as a float when added to JSoar working memory.
   *
   * @param path - the XML path
   * @throws TagAlreadyAddedException if the path was added as an integer path
   */
  ...
  /**
   * Add an XML path which should be treated as an integer when added to JSoar working memory.
   *
   * @param path - the XML path
   * @throws TagAlreadyAddedException if the path was added as a float path
   */
  ...
  /**
   * Add a WME to the builder. The type of the WME value is assumed to be a <code>String</code>
   * unless the XML path of the attribute has been added as a integer or float type.
   *
   * @param builder - the JSoar builder
   * @param attribute - the attribute of the WME
