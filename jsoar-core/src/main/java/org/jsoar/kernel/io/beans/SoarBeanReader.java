...
/**
 * Implements conversion from Soar working memory (e.g. an output command structure) to Java beans.
 *
 * <p>A single instance of this class can and should be used for multiple deserializations. This
 * will allow for effective property caching across calls.
 *
 ...
 *
 * <ul>
 *   <li>Soar-style attributes, i.e. those with hyphens or stars, are automatically converted to
 *       Java naming conventions. For example, {@code first-name} and {@code first*name} both become
 *       {@code firstName}
 *   <li>A property whose type is a sub-type of {@link Symbol} will just get the raw {@link Symbol}
 *       value of a WME with no conversions. So Soar {@link Identifier}s can be captured in beans.
 * </ul>
 *
 * <p>Limitations:
 *
...
public class SoarBeanReader {
  ...
  /**
   * Read the working memory structure under the given identifier into a JavaBean (mostly) of the
   * given type.
   *
  ...
  private <T> void setProperty(T bean, String name, Symbol value)
      throws IllegalAccessException, InvocationTargetException, NoSuchMethodException,
          SoarBeanException, SecurityException, IllegalArgumentException {
  ...
  private <T> void setField(T bean, String name, Symbol value)
      throws SecurityException, SoarBeanException, IllegalArgumentException,
  ...
  private <T> T trySymbolConversion(Symbol initialValue, T convertedValue, Class<T> symbolType)
      throws SoarBeanException {
    if (convertedValue == null) {
      throw new SoarBeanException(
          "Can't convert '" + initialValue + "' to " + symbolType.getCanonicalName(),
          debugSoarBeanClass,
          debugId);
    }
  ...
  private Object convert(Symbol value, Class<?> targetType) throws SoarBeanException {
    if (targetType.isAssignableFrom(Symbol.class)) {
      ...
      if (s == null) {
        throw new SoarBeanException(
            "Can't convert '" + value + "' to JavaSymbol", debugSoarBeanClass, debugId);
      }
