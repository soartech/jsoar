...
public class DefaultXmlToWme implements XmlToWme {
  ...
  /**
   * Construct an XML to WME converter for I/O, i.e. it uses an instance of {@link InputOutput} to
  ...
  /**
   * Construct a new XML to WME converter using the given factory to construct new symbols and WMEs.
   *
  ...
  private void addAttributes(Element element, Identifier targetId) {
    ...
    for (int i = 0; i < attrsLength; ++i) {
      ...
      wmeFactory.addWme(
          attrsId, syms.createString(attr.getName()), syms.createString(attr.getValue()));
    }
  }
