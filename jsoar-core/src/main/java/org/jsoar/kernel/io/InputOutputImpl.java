 ...
 *
 * <p>User-defined Soar I/O routines should be added at system startup time via calls to
 * add_input_function() and add_output_function(). These calls add things to the system's list of
 * (1) functions to be called every input cycle, and (2) symbol-to-function mappings for output
 * commands. File io.cpp contains the system I/O mechanism itself (i.e., the stuff that calls the
 * input and output functions), plus the text I/O routines.
 *
 * <p>Init_soar_io() does what it says. Do_input_cycle() and do_output_cycle() perform the entire
 * input and output cycles -- these routines are called once per elaboration cycle. (once per
 * Decision cycle in Soar 8). The output module is notified about WM changes via a call to
 * inform_output_module_of_wm_changes().
 *
 * <h3>Output Routines</h3>
 *
 * <p>Inform_output_module_of_wm_changes() and do_output_cycle() are the two top-level entry points
 * to the output routines. The former is called by the working memory manager, and the latter from
 * the top-level phases sequencer.
 *
 * <p>This module maintains information about all the existing output links and the identifiers and
 * wmes that are in the transitive closure of them. On each output link wme, we put a pointer to an
 * output_link structure. Whenever inform_output_module_of_wm_changes() is called, we look for new
 * output links and modifications/removals of old ones, and update the output_link structures
 * accordingly.
 *
 * <p>Transitive closure information is kept as follows: each output_link structure has a list of
 * all the ids in the link's TC. Each id in the system has a list of all the output_link structures
 * that it's in the TC of.
 *
 * <p>After some number of calls to inform_output_module_of_wm_changes(), eventually
 * do_output_cycle() gets called. It scans through the list of output links and calls the necessary
 * output function for each link that has changed in some way (add/modify/remove).
 *
...
public class InputOutputImpl implements InputOutput, WmeFactory<InputWme> {
  ...
  public void init_agent_memory() {
    ...
    outputLinkWme =
        addInputWmeInternal(
            this.io_header, predefinedSyms.output_link_symbol, this.io_header_output);
  ...
  */
  ...
  public void do_input_cycle() {
    ...
    if (prev_top_state == true && decider.top_state == null) {
      ...
      // soar_invoke_callbacks(thisAgent, INPUT_PHASE_CALLBACK, (soar_call_data)
      // TOP_STATE_JUST_REMOVED);
  ...
  public void inform_output_module_of_wm_changes(
    ...
    for (ListItem<WmeImpl> it = wmes_being_removed.first; it != null; it = it.next) {
      final WmeImpl w = it.item;
      if (w.id == io_header) update_for_top_state_wme_removal(w);
   ...
   *
   * <p>We make no attempt to do the TC updating intelligently. Whenever the TC changes, we throw
   * away all the old TC info and recalculate the new TC from scratch. I figure that this part of
   * the system won't get used very frequently and I hope it won't be a time hog.
   *
  ...
  /**
   * This routine is called from the top-level sequencer, and it performs the whole output phases.
   * It scans through the list of existing output links, and takes the appropriate action on each
   * one that's changed.
   *
    ...
    switch (outputLinkStatus) {
      ...
      case NEW_OL_STATUS:
        ...
        context
            .getEvents()
            .fireEvent(
                new OutputEvent(this, OutputMode.ADDED_OUTPUT_COMMAND, iw_list, lastOutputSet));

        // #ifndef NO_TIMING_STUFF
        // stop_timer (thisAgent, &thisAgent->start_kernel_tv,
        // &thisAgent->output_function_cpu_time);
        // start_timer (thisAgent, &thisAgent->start_kernel_tv);
      ...
      case MODIFIED_BUT_SAME_TC_OL_STATUS:
        ...
        context
            .getEvents()
            .fireEvent(
                new OutputEvent(this, OutputMode.MODIFIED_OUTPUT_COMMAND, iw_list, lastOutputSet));
      ...
      case REMOVED_OL_STATUS:
        ...
        context
            .getEvents()
            .fireEvent(
                new OutputEvent(this, OutputMode.REMOVED_OUTPUT_COMMAND, iw_list, lastOutputSet));
