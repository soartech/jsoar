...
/**
 * An output command manager that uses {@link SoarBeanReader} to automatically convert an output
 * command from working memory to a Java object and passes the converted object to a
 * client-registered handler.
 *
 * @author ray
 */
public class SoarBeanOutputManager {
  ...
  /**
   * Construct a new output manager and register it with the given event manager.
   *
  ...
  /**
   * Register a handler for an output command. Only a single handler may be registered for a given
   * name.
   *
   ...
   * @param handler the handler object to call when new output commands are detected.
   * @param beanClass Java bean class
   */
  public <T> void registerHandler(
      String name, SoarBeanOutputHandler<T> handler, Class<T> beanClass) {
    handlers.put(name, new HandlerInfo(handler, beanClass));
  }

  /**
   * Unregister a handler previously registered with {@link #registerHandler(String,
   * SoarBeanOutputHandler, Class)}.
   *
  ...
  private void handleCommand(final OutputEvent outputEvent, final Wme command) {
    ...
    try {
      ...
      final SoarBeanOutputContext context =
          new SoarBeanOutputContext() {
