...
public class XmlToWmeTools {
  ...
  /**
   * Add an XML document to the input link. The XML will be converted to working memory during the
   * next {@link InputEvent} callback. Note that since this may occur in a separate thread, care
   * must be taken to ensure that the XML tree is not modified after this method is called.
   *
   ...
   * @param converter XML converter to use
   * @param root the root of the XML tree to convert, i.e. a Document or Element
   * @param attr the attribute of the root WME that is created. If {@code null}, then the tag name
   *     of the root element is used
   * @throws IllegalArgumentException if root is not a Document or Element object
   */
  public static void addXmlInput(
      final InputOutput io, Node root, final XmlToWme converter, final String attr) {
   ...
   */
  public static void addXmlInput(
      final InputWme parent, Node root, final XmlToWme converter, final String attr) {
   ...
   * @throws IllegalArgumentException if root is not a Document or Element object
   */
  private static void addXmlInput(
      final InputOutput io,
      final Identifier id,
      Node root,
      final XmlToWme converter,
      final String attr) {
    ...
    if (root instanceof Document) {
      ...
      throw new IllegalArgumentException(
          "root must be of type Document or Element, got " + root.getClass());
    }
    ...
    SoarEvents.listenForSingleEvent(
        ...
        new SoarEventListener() {
          ...
          public void onEvent(SoarEvent event) {
            InputWmes.add(
                io, id, attr != null ? attr : element.getTagName(), converter.fromXml(element));
          }
        });
  }
}
