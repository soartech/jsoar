...
public class ProductionFinderTest {
  ...
  private void loadFile(String testName) throws Exception {
    final String path =
        "/"
            + ProductionFinderTest.class.getCanonicalName().replace('.', '/')
            + "_"
            + testName
            + ".soar";
  ...
  public void testLeftHandSide() throws Exception {
    ...
    final List<Production> result1 =
        finder.find("(<c> ^contact.name *)", agent.getProductions().getProductions(null));
    assertEquals(1, result1.size());
    assertEquals("test1", result1.get(0).getName());

    final List<Production> result2 =
        finder.find("(<c> ^contact.threat *yes*)", agent.getProductions().getProductions(null));
    ...
    assertEquals(
        2,
        finder
            .find("(<s> ^contacts <c>)(<c> ^contact)", agent.getProductions().getProductions(null))
            .size());
    assertTrue(
        finder.find("(<s> ^name foo)", agent.getProductions().getProductions(null)).isEmpty());
  }
  ...
  public void testRightHandSide() throws Exception {
    ...
    assertEquals(
        ...
        finder
            .find(
                "(<s> ^contacts <cs>)(<cs> ^contact <c>)",
                agent.getProductions().getProductions(null))
            .size());
    assertTrue(
        finder.find("(<s> ^name foo)", agent.getProductions().getProductions(null)).isEmpty());
  }
