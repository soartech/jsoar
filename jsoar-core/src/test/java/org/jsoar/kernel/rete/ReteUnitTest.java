...
public class ReteUnitTest extends JSoarTest {
  ...
  public void setUp() throws Exception {
    ...
    this.rete =
        new Rete(
            Trace.createStdOutTrace().enableAll(),
            syms,
            episodicMemory,
            semanticMemory,
  ...
  public void testSimpleAddWmeToRete() throws Exception {
    ...
    WmeImpl floatWme =
        new WmeImpl(root, syms.createString("float"), syms.createDouble(3.14), false, 0);
    rete.add_wme_to_rete(floatWme);
    assertFalse(listener.matching.contains(p));

    WmeImpl stringWme =
        new WmeImpl(root, syms.createString("string"), syms.createString("S"), false, 0);
