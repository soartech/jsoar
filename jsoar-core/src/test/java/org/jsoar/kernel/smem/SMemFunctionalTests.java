...
public class SMemFunctionalTests extends FunctionalTestHarness {
  ...
  public void testCueSelection() throws Exception {
    ...
    DefaultSemanticMemory smem =
        (DefaultSemanticMemory) agent.getAdapter(DefaultSemanticMemory.class);
  ...
  public void testSimpleNonCueBasedRetrieval_ActivationRecency() throws Exception {
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationRecency functional test did not halt", halted);
    ...
    assertEquals(
        "testSimpleNonCueBasedRetrieval_ActivationRecency: Invalid Activation Values",
        expected,
        result);
  ...
  @Test
  public void testSimpleNonCueBasedRetrieval_ActivationRecency_WithoutActivateOnQuery()
      throws Exception {
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationRecency_WithoutActivateOnQuery functional test did not halt",
        halted);
    ...
    assertEquals(
        "testSimpleNonCueBasedRetrieval_ActivationRecency_WithoutActivateOnQuery: Invalid Activation Values",
        expected,
        result);
  ...
  public void testSimpleNonCueBasedRetrieval_ActivationFrequency() throws Exception {
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationFrequency functional test did not halt", halted);
    ...
    assertEquals(
        "testSimpleNonCueBasedRetrieval_ActivationFrequency: Invalid Activation Values",
        expected,
        result);
  ...
  private boolean checkActivationValues(
      String activationString, List<Double> lowEndExpectations, List<Double> highEndExpectations) {
    ...
    for (Character c : activationString.toCharArray()) {
      ...
      if (inActivationParse
          && (Character.isDigit(c) || c.equals('.') || c.equals('+') || c.equals('-'))) {
        if (activation.length() != 0 && (c.equals('+') || c.equals('-'))) {
    ...
    if (activationLevels.size() != lowEndExpectations.size()) {
      throw new AssertionError(
          "Low End Expectations is not the same size as parsed Activation Levels!");
    } else if (activationLevels.size() != highEndExpectations.size()) {
      throw new AssertionError(
          "High End Expectations is not the same size as parsed Activation Levels!");
    }
      ...
      if (!(a >= lowEndExpectations.get(i) && a <= highEndExpectations.get(i))) {
        throw new AssertionError(
            "Parsed Activation "
                ...
                + a
                + ") is not within ["
                + lowEndExpectations.get(i)
                + ", "
                + highEndExpectations.get(i)
                + "]");
      }
    }
  ...
  public void testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Stable() throws Exception {
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Stable functional test did not halt",
        halted);
    ...
    // This is the expected output from smem --print modified from CSoar to look like JSoar outputs
    // it (reverse string attributes)
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Stable: Invalid Activation Values",
        checkActivationValues(result, lowEndExpectations, highEndExpectations));
  ...
  public void testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Naive() throws Exception {
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Naive functional test did not halt",
        halted);
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Naive: Invalid Activation Values",
        checkActivationValues(result, lowEndExpectations, highEndExpectations));
  ...
  public void testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Incremental() throws Exception {
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Incremental functional test did not halt",
        halted);
    ...
    assertTrue(
        "testSimpleNonCueBasedRetrieval_ActivationBaseLevel_Incremental: Invalid Activation Values",
        checkActivationValues(result, lowEndExpectations, highEndExpectations));
  ...
  public void dbBackupAndLoadTests() throws Exception {
    ...
    String expectedResultOfPS1 =
        "(S1 ^counter 50 ^epmem E1 ^io I1 ^name Factorization ^operator O1385 ^operator O1385 + ^reward-link R1 ^smem S2 ^superstate nil ^type state ^using-smem true)\n";

    assertEquals("Didn't stop where expected!", expectedResultOfPS1, resultOfPS1);
    ...
    assertEquals(
        "smem --init didn't init smem!",
        "SMem| Semantic memory is empty.",
    ...
    assertEquals("smem --init didn't excise all productions!", 0, resultOfP.length());
    ...
    assertEquals("smem --init didn't reinit WM!", expectedResultOfPS1, resultOfPS1);
    ...
    assertEquals(
        "testFactorization: Test did not get the correct result!",
        resultOfPD2F197,
        expectedResultOfPD2F197);
  ...
  public void readCSoarDB() throws Exception {
    ...
    assertEquals("Unexpected output from CSoar database!", expectedResult, actualResult);
  }
  ...
  public void testSimpleStoreGC() throws Exception {
    ...
    for (String lti : split) {
      if (lti.length() == 3 || lti.length() == 4) {
        if (lti.charAt(0) == '@') {
          ltis.add(lti);
        } else if (lti.charAt(0) == '(' && lti.charAt(1) == '@') {
    ...
    agent.getPrinter().addPersistentWriter(outputWriter = new StringWriter());
  ...
  public void testMultiAgent() throws Exception {
    ...
    while (agents.stream().anyMatch(ThreadedAgent::isRunning))
      ;
