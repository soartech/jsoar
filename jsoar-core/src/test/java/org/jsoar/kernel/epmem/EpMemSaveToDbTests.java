...
public class EpMemSaveToDbTests extends FunctionalTestHarness {
  ...
  public void testPersistentVariablesTable() throws Exception {
        ...
        getConnection()
            .prepareStatement(
                "SELECT * FROM "
                    ...
                    + "persistent_variables WHERE variable_id=?");
    ...
    for (int id = 0; id < expectedVals.size(); id++) {
      ...
      assertTrue(
          "variable_id " + id + " is " + value + ", expected " + expectedVals.get(id),
          value == (long) expectedVals.get(id));
    }
  }
  ...
  public void testSymbolsStringTable() throws Exception {
    ...
    for (int id = 0; id < symbolsString.size(); id++) {
      ...
      if (sym_const == null) {
        assertTrue(
            "id " + id + " is " + sym_const + ", expected " + symbolsString.get(id),
