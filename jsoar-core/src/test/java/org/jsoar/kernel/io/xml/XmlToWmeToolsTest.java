...
public class XmlToWmeToolsTest {
  ...
  public void testAddXmlInputWithDefaultAttribute() throws Exception {
    ...
    XmlToWmeTools.addXmlInput(
        agent.getInputOutput(),
        XmlTools.parse("<xml-input><name value='hi'/></xml-input>"),
        converter);

    agent.runFor(1, RunType.DECISIONS);

    final Wme root =
        Wmes.matcher(agent).attr("xml-input").find(agent.getInputOutput().getInputLink());
    assertNotNull(root);
  ...
  public void testAddXmlInputWithExplicitAttribute() throws Exception {
    ...
    final Wme root =
        Wmes.matcher(agent).attr("another-attr").find(agent.getInputOutput().getInputLink());
    assertNotNull(root);
    assertNotNull(Wmes.matcher(agent).attr("name").value("hi").find(root));
  }
  ...
  public void testAddXmlInputToInputWmeWithExplicitAttribute() throws Exception {
    ...
    final Wme root =
        Wmes.matcher(agent).attr("xml-data").find(agent.getInputOutput().getInputLink());
    assertNotNull(root);
    final Wme xmlData =
        Wmes.matcher(agent).attr("another-attr").find(root.getValue().asIdentifier());
    assertNotNull(xmlData);
    assertNotNull(
        Wmes.matcher(agent).attr("name").value("hi").find(xmlData.getValue().asIdentifier()));
  }
  ...
  public void testAddXmlInputToInputWmeWithDefaultAttribute() throws Exception {
    ...
    final XmlToWme converter = SoarTechXmlToWme.forInput(agent.getInputOutput());
    XmlToWmeTools.addXmlInput(
        loc.value, XmlTools.parse("<xml-input><name value='hi'/></xml-input>"), converter);
