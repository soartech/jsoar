...
public class ReteNetCommandTest {
  ...
  public void testCountTest() throws Exception {
    ...
    assertEquals(
        115136, originalAgent.getProperties().get(SoarProperties.E_CYCLE_COUNT).intValue());
    assertEquals(
        40039, originalAgent.getProperties().get(SoarProperties.PE_CYCLE_COUNT).intValue());
    assertEquals(42, originalAgent.getProductions().getProductions(ProductionType.USER).size());
    assertEquals(15012, originalAgent.getProductions().getProductions(ProductionType.CHUNK).size());
    assertEquals(
        115136, originalAgent.getProperties().get(SoarProperties.E_CYCLE_COUNT).intValue());
    assertEquals(
        40039, originalAgent.getProperties().get(SoarProperties.PE_CYCLE_COUNT).intValue());
    assertEquals(
        120146, originalAgent.getProperties().get(SoarProperties.INNER_E_CYCLE_COUNT).intValue());

    assertEquals(
        revivedAgent.getProductions().getProductions(ProductionType.USER).size(),
        originalAgent.getProductions().getProductions(ProductionType.USER).size());
    assertEquals(
        revivedAgent.getProductions().getProductions(ProductionType.CHUNK).size(),
        originalAgent.getProductions().getProductions(ProductionType.CHUNK).size());
    assertEquals(
        revivedAgent.getProperties().get(SoarProperties.E_CYCLE_COUNT).intValue(),
        originalAgent.getProperties().get(SoarProperties.E_CYCLE_COUNT).intValue());
    assertEquals(
        revivedAgent.getProperties().get(SoarProperties.PE_CYCLE_COUNT).intValue(),
        originalAgent.getProperties().get(SoarProperties.PE_CYCLE_COUNT).intValue());
    assertEquals(
        revivedAgent.getProductions().getProductions(ProductionType.USER).size(),
        originalAgent.getProductions().getProductions(ProductionType.USER).size());
    assertEquals(
    ...
    assertEquals(
        ...
        originalAgent.getProperties().get(SoarProperties.PE_CYCLE_COUNT).intValue());
    assertEquals(
        revivedAgent.getProperties().get(SoarProperties.INNER_E_CYCLE_COUNT).intValue(),
        originalAgent.getProperties().get(SoarProperties.INNER_E_CYCLE_COUNT).intValue());
    assertEquals(
        revivedAgent.getProperties().get(SoarProperties.INNER_E_CYCLE_COUNT).intValue(),
        originalAgent.getProperties().get(SoarProperties.INNER_E_CYCLE_COUNT).intValue());
    assertEquals(
