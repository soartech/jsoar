...
public class InitSoarTests extends FunctionalTestHarness {
  ...
  public void testInitSoar() throws Exception {
    ...
    for (ExecutionTimer t : agent.getAllTimers()) {
      assertTrue(
          "Timer " + t.getName() + "microseconds is not zero", t.getTotalMicroseconds() == 0);
    ...
    for (Production p : agent.getProductions().getProductions(null)) {
      assertTrue(
          "Production " + p.getName() + " firing count is not zero", p.getFiringCount() == 0);
      assertTrue("Production " + p.getName() + " has instantiations", p.instantiations == null);
      assertTrue(
          "Production " + p.getName() + " has non-zero RL update count",
          p.rlRuleInfo == null || p.rlRuleInfo.rl_update_count == 0.0);
    }

    // confirm all stats are reset
    // note that some of these values appear inconsistent (e.g., wme_addition_count=6, while
    // max_wm_size=0)
    ...
    assertTrue(
        "DECISION_PHASES_COUNT is non-zero", props.get(SoarProperties.DECISION_PHASES_COUNT) == 0);
    ...
    assertTrue(
        "INNER_E_CYCLE_COUNT is non-zero", props.get(SoarProperties.INNER_E_CYCLE_COUNT) == 0);
    assertTrue(
        "PRODUCTION_FIRING_COUNT is non-zero",
        props.get(SoarProperties.PRODUCTION_FIRING_COUNT) == 0);
    assertTrue("WME_ADDITION_COUNT is not 13", props.get(SoarProperties.WME_ADDITION_COUNT) == 13);
    assertTrue("WME_REMOVAL_COUNT is non-zero", props.get(SoarProperties.WME_REMOVAL_COUNT) == 0);
    assertTrue(
        "NUM_WM_SIZES_ACCUMULATED is non-zero",
        props.get(SoarProperties.NUM_WM_SIZES_ACCUMULATED) == 0);
    ...
    assertTrue("Wrong number of wmes in rete", agent.getAllWmesInRete().size() == 13);
    ...
    for (Wme w : agent.getAllWmesInRete()) {
      assertTrue(
          ...
          (w.getIdentifier().toString().equals("S1")
                  && w.getAttribute().toString().equals("io")
                  && w.getValue().toString().equals("I1"))
              || (w.getIdentifier().toString().equals("S1")
                  && w.getAttribute().toString().equals("reward-link")
                  && w.getValue().toString().equals("R1"))
              || (w.getIdentifier().toString().equals("S1")
                  && w.getAttribute().toString().equals("epmem")
                  && w.getValue().toString().equals("E1"))
              || (w.getIdentifier().toString().equals("E1")
                  && w.getAttribute().toString().equals("command")
                  && w.getValue().toString().equals("C1"))
              || (w.getIdentifier().toString().equals("E1")
                  && w.getAttribute().toString().equals("present-id")
                  && w.getValue().toString().equals("1"))
              || (w.getIdentifier().toString().equals("E1")
                  && w.getAttribute().toString().equals("result")
                  && w.getValue().toString().equals("R2"))
              || (w.getIdentifier().toString().equals("S1")
                  && w.getAttribute().toString().equals("smem")
                  && w.getValue().toString().equals("S2"))
              || (w.getIdentifier().toString().equals("S2")
                  && w.getAttribute().toString().equals("result")
                  && w.getValue().toString().equals("R3"))
              || (w.getIdentifier().toString().equals("S2")
                  && w.getAttribute().toString().equals("command")
                  && w.getValue().toString().equals("C2"))
              || (w.getIdentifier().toString().equals("S1")
                  && w.getAttribute().toString().equals("superstate")
                  && w.getValue().toString().equals("nil"))
              || (w.getIdentifier().toString().equals("S1")
                  && w.getAttribute().toString().equals("type")
                  && w.getValue().toString().equals("state"))
              || (w.getIdentifier().toString().equals("I1")
                  && w.getAttribute().toString().equals("input-link")
                  && w.getValue().toString().equals("I2"))
              || (w.getIdentifier().toString().equals("I1")
                  && w.getAttribute().toString().equals("output-link")
                  && w.getValue().toString().equals("I3")));
    }
  }
