...
public class MultiplyTest extends JSoarTest {
  ...
  public void testOneIntArg() throws Exception {
    ...
    assertEquals(
        33, multiply.execute(rhsFuncContext, Symbols.asList(syms, 33)).asInteger().getValue());
  }
  ...
  public void testOneFloatArg() throws Exception {
    ...
    assertEquals(
        123.4,
        multiply.execute(rhsFuncContext, Symbols.asList(syms, 123.4)).asDouble().getValue(),
        0.0001);
  }
  ...
  public void testMixedArgs() throws Exception {
    ...
    assertEquals(
        2 * 123.4 * -2,
        multiply.execute(rhsFuncContext, Symbols.asList(syms, 2, 123.4, -2)).asDouble().getValue(),
        0.0001);
  }
  ...
  public void testIntArgs() throws Exception {
    ...
    assertEquals(
        2 * 3 * 4 * -2,
        multiply.execute(rhsFuncContext, Symbols.asList(syms, 2, 3, 4, -2)).asInteger().getValue());
  }
  ...
  public void testLargeIntArgs() throws Exception {
    ...
    assertEquals(
        2000000000000L * 3 * 4 * -2,
        multiply
            .execute(rhsFuncContext, Symbols.asList(syms, 2000000000000L, 3, 4, -2))
            .asInteger()
            .getValue());
  }
