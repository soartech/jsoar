...
public class SoarTclCommandAdapter implements Command {
  ...
  /**
   * Note that the alias support here is very weak -- we only support replacing a single command
   * name with an array of strings That is, we don't support aliasing subcommands
   */
  public SoarTclCommandAdapter(
      SoarCommand inner, String[] alias, SoarTclInterface soarTclInterface) {
    this.inner = inner;
    this.alias = alias;
  ...
  public void cmdProc(Interp interp, TclObject[] args) throws TclException {
    ...
    if (alias.length > 0) {
      ...
      for (int stringArgsIndex = alias.length, argsIndex = 1;
          argsIndex < args.length;
          ++stringArgsIndex, ++argsIndex) {
