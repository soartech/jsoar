...
/**
 * A simple, embedded web application that provides remote debugging for all JSoar agents (those
 * wrapped with {@link ThreadedAgent} in a single JVM.
 *
 * <p>The requirement for {@link ThreadedAgent} is to ensure that the agent can be accessed in a
 * thread-safe way. {@link Agent} provides no such guarantees.
 *
 * @author ray
 */
public class LegilimensServer {
  ...
  private static ThreadedAgent createAgent(String name, final String rules)
      throws InterruptedException, ExecutionException, TimeoutException, IOException {
