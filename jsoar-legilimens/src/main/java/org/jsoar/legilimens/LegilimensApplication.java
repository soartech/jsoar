...
public class LegilimensApplication extends Application {
  ...
  private final SoarEventListener attachListener =
  ...
  private final SoarEventListener detachListener =
      new SoarEventListener() {
        @Override
        public void onEvent(SoarEvent event) {
  ...
  public Restlet createInboundRoot() {
    ...
    router.attach(
        "/agents/{agentName}/productions/{productionName}/{action}", ProductionResource.class);
    ...
    router.attach(
        ...
        new Directory(
            ...
            LocalReference.createClapReference(
                LocalReference.CLAP_THREAD, "/org/jsoar/legilimens/public" + name)));
  }
  ...
  protected void agentAttached(ThreadedAgent agent) {
    logger.info(
        "Attaching to agent '"
            + agent
            + "' (agent's name may not be accurate if it isn't set yet)");
    try {
  ...
  protected void agentDetached(ThreadedAgent agent) {
    ...
    try {
      ...
      logger.error(
          "Failed to detach trace buffer from agent '" + agent + "': " + e.getMessage(), e);
    }
  }
}
