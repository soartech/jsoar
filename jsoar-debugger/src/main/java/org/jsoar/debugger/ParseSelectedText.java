...
public class ParseSelectedText {
  ...
  public static class SelectedProduction extends SelectedObject {
    ...
    public void fillMenu(JSoarDebugger debugger, JMenu parent) {
      ...
      parent.addSeparator();
      parent.add(
          new ExecuteCommandAction(debugger, String.format("production matches %s", m_Name)));
      parent.add(
          new ExecuteCommandAction(
              debugger, String.format("production matches --wmes %s", m_Name)));
      parent.addSeparator();
      parent.add(new ExecuteCommandAction(debugger, String.format("production excise %s", m_Name)));
      parent.addSeparator();
      parent.add(
          new ExecuteCommandAction(debugger, String.format("production break --set %s", m_Name)));
      parent.add(
          new ExecuteCommandAction(debugger, String.format("production break --clear %s", m_Name)));
    }
  }
  ...
  public static class SelectedID extends SelectedObject {
    ...
    public Object retrieveSelection(JSoarDebugger debugger) {
      return debugger
          .getAgent()
          .getSymbols()
          .findIdentifier(m_Name.charAt(0), Integer.parseInt(m_Name.substring(1)));
    }
    ...
    public void fillMenu(JSoarDebugger debugger, JMenu parent) {
      ...
      parent.add(
          new ExecuteCommandAction(debugger, String.format("print --exact (* * %s)", m_Name)));
      parent.add(new ExecuteCommandAction(debugger, String.format("preferences %s", m_Name)));
      parent.add(
          new ExecuteCommandAction(debugger, String.format("preferences --object %s", m_Name)));
    }
  }
  ...
  public static class SelectedWme extends SelectedObject {
    ...
    public Object retrieveSelection(JSoarDebugger debugger) {
      ...
      final Identifier id =
          debugger
              .getAgent()
              .getSymbols()
              .findIdentifier(m_ID.charAt(0), Integer.parseInt(m_ID.substring(1)));
      if (id == null) {
        return null;
      }
    ...
    public void fillMenu(JSoarDebugger debugger, JMenu parent) {
      parent.add(
          new ExecuteCommandAction(debugger, String.format("preferences %s %s", m_ID, m_Att)));
      parent.add(
          new ExecuteCommandAction(
              debugger, String.format("preferences --names %s %s", m_ID, m_Att)));
      ...
      parent.add(
          new ExecuteCommandAction(
              debugger, String.format("print --exact (* %s %s)", m_Att, m_Value)));
      parent.add(
          new ExecuteCommandAction(debugger, String.format("print --exact (* %s *)", m_Att)));
      final JMenu pfMenu = new JMenu("production-find");
      pfMenu.add(new ExecuteCommandAction(debugger, String.format("pf (<v> %s *)", m_Att)));
      pfMenu.add(
          new ExecuteCommandAction(debugger, String.format("pf (<v> %s %s)", m_Att, m_Value)));
      ...
      pfMenu.add(
          new ExecuteCommandAction(
              debugger, String.format("pf --rhs (<v> %s %s)", m_Att, m_Value)));
      pfMenu.add(
          new ExecuteCommandAction(debugger, String.format("pf --rhs (<v> ^* %s)", m_Value)));
      parent.add(pfMenu);
    }
  ...
  protected static final int kPrevToken = 0;
  protected static final int kCurrToken = 1;
  protected static final int kNextToken = 2;
  ...
  public ParseSelectedText(String content, int selectionStart) {
    ...
    if (m_FullText != null && m_FullText.length() > 0) parseTokens();
  }
  ...
  /**
   * Finds the first char from the set of chars to occur in string starting from startPos. SLOWSLOW:
   * This implementation is based on indexOf so it handles end cases in exactly the same way as the
   * original but it's slow. If we ever plan to call this repeatedly it should be rewritten.
   */
  ...
  protected String isID(String token) {
    ...
    if (token.charAt(0) == '@'
        ...
        || token.charAt(0) == '='
        || token.charAt(0) == '+'
        || token.charAt(0) == '-') startId = 1;

    if (token.length() <= startId) return null;

    // ID's start with capital letters
    if (token.charAt(startId) < 'A' || token.charAt(startId) > 'Z') return null;
  ...
  public SelectedObject getParsedObject(JSoarDebugger debugger) {
    ...
    // One could argue that the kernel should handle this but currently it fails to do so, so we'll
    // handle it here.
    if (curr.length() > 2 && curr.charAt(0) == '|' && curr.charAt(curr.length() - 1) == '|')
      curr = curr.substring(1, curr.length() - 1);

    if (isProductionNameQuickTest(curr)) return new SelectedProduction(curr);
    ...
    if (isAttribute(curr)) {
      ...
      if (idPos != -1 && endSpace != -1) {
        ...
        if (foundId != null) return new SelectedWme(foundId, curr, m_Tokens[kNextToken]);
      }
    ...
    if (isProductionNameAskKernel(debugger, curr)) return new SelectedProduction(curr);

    return new SelectedUnknown(curr);
  }

  protected boolean isWhiteSpace(char ch) {
    for (int i = 0; i < kWhiteSpaceChars.length; i++) if (kWhiteSpaceChars[i] == ch) return true;
  ...
  protected void parseTokens() {
    ...
    if (m_SelectionStart == len) return;
    ...
    if (back1 != -1) m_Tokens[kPrevToken] = m_FullText.substring(back2 + 1, back1);
    if (fore2 != -1 && fore1 < fore2) m_Tokens[kNextToken] = m_FullText.substring(fore1 + 1, fore2);
    if (fore1 != -1) m_Tokens[kCurrToken] = m_FullText.substring(back1 + 1, fore1);
