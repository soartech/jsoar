...
public class ProductionEditView extends AbstractAdaptableView implements Disposable {
  ...
  private static final String DEFAULT_CONTENTS =
      "Double-click a production (or right-click) to edit, or just start typing.";
  ...
  private final AbstractAction loadAction =
      new AbstractAction("Load [Ctrl-Return]") {
  ...
  /**
   * Wrapper list that forwards to the production model in the list view. This is the list we use
   * for auto-complete.
   *
   * <p>TODO: There are probably some synchronization issues here.
   */
  private final ForwardingList<Production> productions =
      new ForwardingList<Production>() {
  ...
  public ProductionEditView(JSoarDebugger debugger) {
    ...
    textArea
        ...
        .addActionForKeyStroke(
            KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, KeyEvent.CTRL_DOWN_MASK), loadAction);
    textArea.selectAll();
  ...
  /**
   * Tell the view to start editing the named production. The production's code is loaded into the
   * editor and the view is brought to the front.
   *
  ...
  public void editProduction(final String name) {
    ...
    final CompletionHandler<String> finish =
        result -> {
          textArea.setText(result);
          status.setText(
              result.length() != 0
                  ? "Editing production '" + name + "'"
                  : "No production '" + name + "'");
