...
public class WmeSearchView extends AbstractAdaptableView implements Refreshable, Disposable {
  ...
  private final TableSelectionProvider selectionProvider =
      new TableSelectionProvider(wmeTable) {
  ...
  private JToggleButton synch =
      new JToggleButton(Images.SYNCH, getPreferences().getBoolean("synch", false));
  ...
  private void doSearch() {
    ...
    final CompletionHandler<List<Wme>> done =
        new CompletionHandler<List<Wme>>() {
          ...
          public void finish(List<Wme> result) {
            ...
            description.setText(
                String.format(
                    "<html>&nbsp;WMEs matching pattern <b><code>(%s ^%s %s)</code></b>",
                    id, attr, value));
          }
        };
