...
public class TraceView extends AbstractAdaptableView implements Disposable {
  ...
  private final JTextPane outputWindow =
      new JTextPane() {
        ...
        // ensure that we always wrap the trace text so it fits horizontally within the visible
        // trace window
          ...
          synchronized (outputWriter) // synchronized on outer.this like the flush() method
          {
            ...
            if (SwingUtilities.isEventDispatchThread()) {
              ...
              try {
                ...
                logger.info(
                    "Interrupted while attempting to scroll to the end for text |{}| which may happen on shutdown",
                    input);
  ...
  public TraceView(JSoarDebugger debuggerIn) {
    ...
    outputWindow.setFont(
        new Font("Monospaced", Font.PLAIN, (int) (12 * JSoarDebugger.getFontScale())));
    ...
    highlightImmediately =
        new AtomicBoolean(getPreferences().getBoolean(HIGHLIGHT_IMMEDIATELY, true));
    scrollLock = getPreferences().getBoolean(SCROLL_LOCK, true);
    setDefaultTextStyle();

    reloadSyntax();

    outputWindow.addMouseListener(
        new MouseAdapter() {
    ...
    outputWindow.setText(
        "JSoar "
            ...
            + "Current command interpreter is '"
            + debugger.getAgent().getInterpreter().getName()
            + "'\n"
            + "\n"
    ...
    addAction(
        new CButton("Clear", Images.CLEAR) {
          @Override
  ...
  /**
   * Set the limit on the number of characters saved in the trace. When {@code limit +20%} is
   * reached, the beginning 20% of the trace buffer will be removed.
   *
  ...
  private void updateSelectionFromMouseEvent(MouseEvent e) {
    ...
    final CompletionHandler<Object> finish =
  ...
  private void showPopupMenu(MouseEvent e) {
    ...
    menu.insert(
        new AbstractAction("Limit trace output ...") {
    ...
    scrollLockItem.addActionListener(
        new ActionListener() {
    ...
    immediateHLItem.addActionListener(
        new ActionListener() {
    ...
    delayedHLItem.addActionListener(
        new ActionListener() {
    ...
    menu.insert(
        new AbstractAction("Edit Syntax Highlighting") {
          @Override
          public void actionPerformed(ActionEvent e) {
  ...
  private void askForTraceLimit() {
    final String result =
        JOptionPane.showInputDialog(
            outputWindow, "Trace limit in characters (-1 for no limit)", limit);
    if (result != null) {
      try {
