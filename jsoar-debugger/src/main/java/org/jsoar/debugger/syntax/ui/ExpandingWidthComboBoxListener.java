 ...
 * It will support the following features: - a horizontal scrollbar can be displayed when necessary
 * - the popup can be wider than the combo box - the popup can be displayed above the combo box
 *
 * <p>Class will only work for a JComboBox that uses a BasicComboPop.
 */
public class ExpandingWidthComboBoxListener implements PopupMenuListener {
   ...
   */
  public ExpandingWidthComboBoxListener(boolean popupWider, boolean popupAbove) {
    this(true, popupWider, -1, popupAbove);
  }

  /**
   * Convenience constructor that allows you to display the popup wider than the combo box and to
   * specify the maximum width
   *
   * @param maximumWidth the maximum width of the popup. The popupAbove value is set to "true".
   */
  public ExpandingWidthComboBoxListener(int maximumWidth) {
   ...
   *
   * @param scrollBarRequired display a horizontal scrollbar when the preferred width of popup is
   *     greater than width of scrollPane.
   * @param popupWider display the popup at its preferred with
   * @param maximumWidth limit the popup width to the value specified (minimum size will be the
  ...
  /**
   * Set the maximum width for the popup. This value is only used when setPopupWider( true ) has
   * been specified. A value of -1 indicates that there is no maximum.
   *
   ...
   *
   * @param popupAbove true display popup above the combo box, false display popup below the combo
   *     box.
   */
  ...
  /**
   * Change the width of the popup to be the greater of the width of the combo box or the preferred
   * width of the popup. Normally the popup width is always the same size as the combo box width.
   *
  ...
  /**
   * For some reason the default implementation of the popup removes the horizontal scrollBar from
   * the popup scroll pane which can result in the truncation of the rendered items in the popop.
   * Adding a scrollBar back to the scrollPane will allow horizontal scrolling if necessary.
   *
   * @param scrollBarRequired true add horizontal scrollBar to scrollPane false remove the
