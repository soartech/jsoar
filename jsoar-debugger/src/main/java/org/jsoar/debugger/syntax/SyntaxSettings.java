...
public class SyntaxSettings {
   ...
   */
  public synchronized List<StyleOffset> matchAll(
      String input,
      SyntaxPattern syntax,
      HashMap<String, TextStyle> styles,
    ...
    try {
      ...
      while (m.find()) {
        ...
        // Why are there two loops here (they look identical, the for version should work for the
        // groupCount == 1 case)
        if (groupCount == 1) {
          int start = m.start(1);
          int end = m.end(1);
          ...
          for (int i = 1;
              i < groupCount + 1;
              i++) { // start with match 1, because match 0 is the whole regex not the capture group
            int start = m.start(i);
            int end = m.end(i);
  ...
  /**
   * Generate a TreeSet (which will be iterated through in sorted order) of this syntax across the
   * input string
   *
