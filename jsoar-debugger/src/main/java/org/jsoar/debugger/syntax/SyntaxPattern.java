  ...
  public void expandMacros(JSoarDebugger debugger) {
    ...
    if (regex.contains("%aliases%")) {
      ...
      if (interpreter instanceof DefaultInterpreter) {
        List<String> aliases =
            new ArrayList<>(((DefaultInterpreter) interpreter).getAliasStrings());
        Collections.sort(
            aliases,
        ...
        for (Iterator<String> iterator = aliases.iterator(); iterator.hasNext(); ) {
          ...
          if (iterator.hasNext()) aliasesStr.append("|");
        }
      ...
      expandedRegex =
          expandedRegex.replaceAll("%aliases%", Matcher.quoteReplacement(aliasesStr.toString()));
    }
    if (regex.contains("%commands%")) {
      ...
      if (interpreter instanceof DefaultInterpreter) {
        List<String> commands =
            new ArrayList<>(((DefaultInterpreter) interpreter).getCommandStrings());
        Collections.sort(
            commands,
            new Comparator<String>() {
              @Override
        ...
        for (Iterator<String> iterator = commands.iterator(); iterator.hasNext(); ) {
          ...
          if (iterator.hasNext()) commandsStr.append("|");
        }

      } else if (interpreter instanceof SoarTclInterface) {
        try {
          commandsStr.append(
              interpreter.eval("info commands").replace(" ", "|").replace("?", "\\?"));
          commandsStr.append("|");
      ...
      expandedRegex =
          expandedRegex.replaceAll("%commands%", Matcher.quoteReplacement(commandsStr.toString()));
    }
    if (regex.contains("%rhsfuncs%")) {
      StringBuilder rhsFuncsString = new StringBuilder();
      List<String> rhsFuncs =
          debugger.getAgent().getRhsFunctions().getHandlers().stream()
      ...
      for (Iterator<String> iterator = rhsFuncs.iterator(); iterator.hasNext(); ) {
        ...
        if (iterator.hasNext()) rhsFuncsString.append("|");
      }
      expandedRegex =
          expandedRegex.replaceAll(
              "%rhsfuncs%", Matcher.quoteReplacement(rhsFuncsString.toString()));
    }
  }
