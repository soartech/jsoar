 ...
 * @author ray
 */
public class PreferencesView extends AbstractAdaptableView
    implements SelectionListener, Refreshable, Disposable {
  ...
  private JToggleButton synch =
      new JToggleButton(Images.SYNCH, getPreferences().getBoolean("synch", true));
  ...
  private JToolBar createToolbar() {
    ...
    bar.add(synch);
    bar.add(
        new AbstractDebuggerAction("Print to trace", Images.COPY) {
  ...
  private void finishGetPreferences(Result result) {
    ...
    info.setText(
        String.format(
            "<html><b>&nbsp;Operator preferences for <code>%s</code></b></html>",
            result.getQueryId()));
  ...
  private Result safeGetPreferences(Identifier id) {
    ...
    final PredefinedSymbols predefinedSyms =
        Adaptables.adapt(agent.getAgent(), PredefinedSymbols.class);
