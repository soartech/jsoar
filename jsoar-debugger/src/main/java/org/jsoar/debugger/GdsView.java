...
public class GdsView extends AbstractAdaptableView implements Refreshable, Disposable {
  ...
  private final SoarEventListener removalListener =
  ...
  public void refresh(boolean afterInitSoar) {
    ...
    final CompletionHandler<Model> finish =
        new CompletionHandler<Model>() {
          @Override
          public void finish(Model result) {
            if (result.goal != null) {
              if (result.wmes.isEmpty()) {
                label.setText(
                    String.format(
                        "<html><b>&nbsp;<code>%s</code> has no GDS</b></html>",
                        result.goal.getIdentifier()));
              } else {
                label.setText(
                    String.format(
                        "<html><b>&nbsp;GDS for <code>%s</code>.</b> (%d wmes)</html>",
