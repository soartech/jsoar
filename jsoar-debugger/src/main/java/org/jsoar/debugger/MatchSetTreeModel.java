 ...
 * @author ray
 */
public class MatchSetTreeModel extends DefaultTreeTableModel
{
    ...
    public MatchSetTreeModel(MatchSet info)
    {
    ...
    {
        private final MatchSet info;
        private final List<TreeTableNode> supports;
        
        public InfoNode(MatchSet info)
        {
            super(true);
            
            this.info = info;
            this.supports = new ArrayList<TreeTableNode>(info.getEntries().size());
            for(MatchSetEntry support : info.getEntries())
            {
    ...
    private static class SourceNode extends AbstractTreeTableNode
    {
        ...
        private final MatchSetEntry support;
        private final List<TreeTableNode> wmes;
        
        public SourceNode(TreeTableNode parent, MatchSetEntry support)
        {
            super(true);
            
            this.parent = parent;
            this.support = support;
            this.wmes = new ArrayList<TreeTableNode>(this.support.getWmes().size());
            for(Wme wme : this.support.getWmes())
            {
        ...
        {
            return column == 0 ? support.getProduction() : null;
        }
