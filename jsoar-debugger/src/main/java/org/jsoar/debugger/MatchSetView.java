...
public class MatchSetView extends AbstractAdaptableView implements Refreshable {
  ...
  public MatchSetView(JSoarDebugger debugger) {
    ...
    final JSplitPane split =
        new JSplitPane(
            JSplitPane.VERTICAL_SPLIT, new JScrollPane(entryList), new JScrollPane(wmeTable));
  ...
  private JToolBar createToolbar() {
    ...
    bar.add(
        new AbstractDebuggerAction("Print match set to trace", Images.COPY) {
          ...
          public void actionPerformed(ActionEvent arg0) {
            ...
            a.printMatchSet(
                a.getPrinter(), WmeTraceType.FULL, EnumSet.allOf(MatchSetTraceType.class));
          }
        });
    return bar;
  }
  ...
  public void refresh(boolean afterInitSoar) {
    final Callable<MatchSet> matchCall =
        new Callable<MatchSet>() {
    ...
    final CompletionHandler<MatchSet> finish =
        new CompletionHandler<MatchSet>() {
          @Override
          public void finish(MatchSet result) {
            entryList.setModel(
                SwingTools.addAll(new DefaultListModel<MatchSetEntry>(), result.getEntries()));
  ...
  private static class CellRenderer extends DefaultListCellRenderer {
    ...
    public Component getListCellRendererComponent(
      ...
      setFont(boldFont);
      setText(
          entry.getProduction() != null ? entry.getProduction().getName().toString() : "[dummy]");
      return c;
    }
  }
}
