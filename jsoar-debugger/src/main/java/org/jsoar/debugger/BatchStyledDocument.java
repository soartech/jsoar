	...
	private final Thread highlightingThread = new Thread("Document Highlight Thread"){
		public void run() {
			while(!Thread.interrupted()) {
    			...
    			if(newText == null) {Thread.yield();continue;}
...
//                    elements.add(new ElementSpec(highlighter.getDefaultAttributes(), ElementSpec.ContentType, string.toCharArray(), 0, string.length()));
                    ...
                    for (StyleOffset offset : styles) {
                        ...
                        self.setCharacterAttributesUnsafe(newText.offset + index, start - index, highlighter.getDefaultAttributes(), true);
...
//                        				start - index));
                        //the matched stuff
                        self.setCharacterAttributesUnsafe(newText.offset + index + start-index, end-start, offset.style, true);
//                        elements.add(
//                        		new ElementSpec(
//                        				offset.style, 
//                        				ElementSpec.ContentType, 
...
//                        				end - start));
                    ...
                    if (index < newText.text.length()) {
                    	...
                    	self.setCharacterAttributes(newText.offset + index, newText.text.length() - index, highlighter.getDefaultAttributes(), true);
//                        elements.add(
//                        		new ElementSpec(
//                        				highlighter.getDefaultAttributes(), 
//                        				ElementSpec.ContentType, 
