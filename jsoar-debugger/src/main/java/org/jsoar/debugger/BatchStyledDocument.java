...
public class BatchStyledDocument extends DefaultStyledDocument {
  ...
  private final Thread highlightingThread =
      new Thread("Document Highlight Thread") {
        public void run() {
  ...
  public void takeDelayedBatchUpdate(String text, boolean highlightText) {
    ...
    try {
      ...
      // This function is protected and gets it's own writelocks but they are cheap if you already
      // have them
      ...
      BatchStyledDocument.logger.info(
          "Bad Location offset = {} {} {}", getEndPosition(), getLength(), e);
    } finally {
      writeUnlock();
    }
  ...
  public void takeBatchUpdate(String text, boolean highlightText) {
    ...
    try {
      ...
      BatchStyledDocument.logger.info(
          "Bad Location offset = {} {} {}", offset, getEndPosition(), getLength(), e);
    } finally {
      writeUnlock();
    }
  }
  ...
  private void highlightNewText(NewText newText) {
    ...
    if (!styles.isEmpty()) {
      ...
      for (StyleOffset styleOffset : styles) {
        ...
        self.setCharacterAttributes(
            offset + index + start - index, end - start, styleOffset.style, true);
        writeUnlock();
  ...
  // Exposing write lock functionality (seems to improve the performance of the caret position
  // setting code
