...
public class WorkingMemoryTree extends JComponent {
  ...
  private final CompletionHandler<Void> repaint =
      SwingCompletionHandler.newInstance(
  ...
  private Rectangle2D getCenteredTextBounds(Graphics2D g2d, String text, int x, int y) {
    ...
    bounds.setFrame(
        x, y + (rowHeight / 2 - bounds.getHeight() / 2), bounds.getWidth(), bounds.getHeight());
    return bounds;
  }
  ...
  private void paintRootRow(final RootRow asRoot, Graphics2D g2d) {
    ...
    g2d.fillRect(
        (int) asRoot.bounds.getX(),
        (int) asRoot.bounds.getY(),
        (int) asRoot.bounds.getWidth(),
        (int) asRoot.bounds.getHeight() - 2);

    //        g2d.fillRoundRect((int) asRoot.bounds.getX() + 2, (int) asRoot.bounds.getY() + 2,
    //                          (int) asRoot.bounds.getWidth() - 4, (int) asRoot.bounds.getHeight()
    // - 4,
    //                          6, 6);
    g2d.setColor(rootRowTextColor);
    g2d.drawString(
        text, (int) bounds.getX(), (int) (bounds.getMaxY() - g2d.getFontMetrics().getDescent()));
    ...
    if (!notes.isEmpty()) {
      ...
      final Rectangle2D noteBounds =
          getCenteredTextBounds(g2d, text, 5, (int) asRoot.bounds.getY());
      g2d.drawString(
          notes,
    ...
    if (asRoot.deleteButton.getParent() == null) {
      ...
      validate();
      asRoot.deleteButton.addActionListener(
          new ActionListener() {
            @Override
    ...
    asRoot.deleteButton.setBounds(
        (int) asRoot.bounds.getMaxX() - 30, (int) (asRoot.bounds.getCenterY() - 20 / 2), 20, 20);
  }
  ...
  private void paintRow(Graphics2D g2d, WmeRow row) {
    ...
    //        g2d.drawString(Integer.toString(row.row), getWidth() - 40, (int)
    // row.idBounds.getMaxY());
  }
  ...
  private void paintSelectionIndicator(Graphics2D g2d, WmeRow.Value value) {
    ...
    g2d.drawRoundRect(
        (int) value.bounds.getX() - fillPad,
        top,
        (int) value.bounds.getWidth() + fillPad * 2,
        height,
        4,
        4);
    g2d.setStroke(oldStroke);
  ...
  private void paintExpandedIdIndicator(Graphics2D g2d, WmeRow.Value value) {
    ...
    g2d.fillArc(
        (int) value.bounds.getX(),
        (int) value.bounds.getMaxY(),
        (int) value.bounds.getWidth(),
        (int) (getRowHeight() - value.bounds.getHeight()),
        45,
        90);
  }
  ...
  private Rectangle2D paintSymbol(Graphics2D g2d, Symbol sym, String string, int x, int y) {
    ...
    if (id != null) {
      ...
      g2d.drawRoundRect(
          (int) bounds.getX() - fillPad,
          ((int) bounds.getY() - fillPad),
          (int) bounds.getWidth() + fillPad * 2,
          (int) bounds.getHeight() + fillPad * 2,
          4,
          4);
    }
    ...
    g2d.setColor(textColor);
    g2d.drawString(
        string, (int) bounds.getX(), (int) (bounds.getMaxY() - g2d.getFontMetrics().getDescent()));
