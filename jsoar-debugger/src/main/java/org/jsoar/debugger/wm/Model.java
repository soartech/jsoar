...
class Model {
  ...
  private void expandIdInternalHelper(Identifier id, RootRow root, WmeRow.Value parent) {
    ...
    for (Iterator<Wme> it = id.getWmes(); it.hasNext(); ) {
      ...
      if (newRow == null) {
        newRow =
            parent != null ? parent.addChild(root, wmeId, wmeAttr) : root.addChild(wmeId, wmeAttr);
  ...
  private void expandId(
      final Object id, final WmeRow.Value parent, CompletionHandler<Void> finish) {
  ...
  private CompletionHandler<Void> timeIncrementCompletionHandler(
      final CompletionHandler<Void> inner) {
