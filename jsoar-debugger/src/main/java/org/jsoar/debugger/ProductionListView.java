...
public class ProductionListView extends AbstractAdaptableView
    implements Refreshable, SelectionListener {
    ...
    this.agent.execute(
        () -> {
          ...
          this.agent
              .getAgent()
              .getEvents()
              .addListener(ProductionExcisedEvent.class, eventListener);
        });
  }
  ...
  private final SoarEventListener eventListener =
      new SoarEventListener() {
        ...
        public void onEvent(SoarEvent event) {
          ...
          // we do this through the swing thread so it ends up at the end of the Soar agent queue --
          // otherwise it will execute right away,
          // and we don't want it to execute until after the productions are loaded (e.g., the
          // source command completes)
          SwingUtilities.invokeLater(
              () -> {
                agent.execute(
                    () -> {
                      // by this time, the production sourcing should be complete, so schedule the
                      // refresh on the swing thread and re-register for the events
                      SwingUtilities.invokeLater(() -> refresh(false));
                      agent
                          .getAgent()
                          .getEvents()
                          .addListener(ProductionAddedEvent.class, eventListener);
                      agent
                          .getAgent()
                          .getEvents()
                          .addListener(ProductionExcisedEvent.class, eventListener);
  ...
  private void updateStats() {
    final Callable<Map<ProductionType, Integer>> call =
        new Callable<Map<ProductionType, Integer>>() {
    ...
    final CompletionHandler<Map<ProductionType, Integer>> finish =
        new CompletionHandler<Map<ProductionType, Integer>>() {
          ...
          public void finish(Map<ProductionType, Integer> counts) {
            ...
            for (Map.Entry<ProductionType, Integer> e : counts.entrySet()) {
              b.append(
                  " <b>" + e.getKey().getDisplayString() + ":</b>&nbsp;" + e.getValue() + spaces);
            }
            b.append("</html>");
    ...
    public Component getTableCellRendererComponent(
      ...
      setToolTipText(
          "<html>"
              ...
              + p.getName()
              + "</b><br>"
              + p.getType().getDisplayString()
              + " production<br>"
              + "Fired "
              + fc
              + " time"
              + (fc != 1 ? "s" : "")
              + (p.isBreakpointEnabled() ? "<br>:interrupt" : "")
              + "<p>"
              + p.getDocumentation()
              + "</html>");
      return c;
