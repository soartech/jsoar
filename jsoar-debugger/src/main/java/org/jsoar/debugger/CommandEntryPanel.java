...
public class CommandEntryPanel extends JPanel implements Disposable {
  ...
  private final AbstractAction selectUpAction =
      new AbstractAction() {
        @Override
  ...
  @SuppressWarnings("serial")
  private final AbstractAction selectDownAction =
      new AbstractAction() {
        @Override
        public void actionPerformed(ActionEvent e) {
          if (completionsShowing) {
  ...
  @SuppressWarnings("serial")
  private final AbstractAction completeSelectedAction =
      new AbstractAction() {
        @Override
        public void actionPerformed(ActionEvent e) {
          if (completionsShowing) {
  ...
  public CommandEntryPanel(JSoarDebugger debugger) {
    ...
    // completions.setOpacity(0.8f); // some environments, like Linux Manjaro with KDE, do not
    // support transparency, and it's not necessary here
    ...
    editorComponent.addFocusListener(
        new FocusAdapter() {
          ...
          public void focusLost(FocusEvent e) {
            super.focusLost(e);
            if (e.getOppositeComponent() != completions
                && e.getOppositeComponent() != completionsList) {
              hideCompletions();
            }
          }
        });
    completions.addFocusListener(
        new FocusAdapter() {
          @Override
          public void focusLost(FocusEvent e) {
            super.focusLost(e);
    ...
    editorComponent
        ...
        .put(
            KeyStroke.getKeyStroke(
                KeyEvent.VK_UP, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()),
            "up_complete");
    editorComponent
        .getInputMap()
        .put(
    ...
    editorComponent
        ...
        .put(
            KeyStroke.getKeyStroke(
                KeyEvent.VK_SPACE, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()),
            "show_completions");
    editorComponent
        .getInputMap()
        .put(KeyStroke.getKeyStroke(KeyEvent.VK_ESCAPE, 0), "hide_completions");
    completionsList
        .getInputMap()
        .put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0), "complete_selected");

    completionsList.addMouseListener(
        new MouseAdapter() {
          @Override
      ...
      new Thread(
              () -> {
                ...
                if (lastInputTimestamp + COMPLETION_DELAY <= time) {
                  SwingUtilities.invokeLater(() -> updateCompletions(command, cursorPosition));
                }
              })
          .start();
    } else {
        ...
        .execute(
            () -> {
              ...
              CommandLine commandLine =
                  debugger.getAgent().getInterpreter().findCommand(trimmedCommand);
              if (commandLine == null) {
                commands =
                    debugger
                        .getAgent()
                        .getInterpreter()
                        .getCompletionList(trimmedCommand, cursorPosition);
              } else {
                commands =
                    SoarCommandCompletion.complete(commandLine, trimmedCommand, cursorPosition);
              }
  ...
  private String getHelp(CommandLine commandLine) {
    if (commandLine != null) {
      ...
      CommandLine.Help help =
          new CommandLine.Help(commandLine.getCommandSpec(), commandLine.getColorScheme());
      helpBuilder
          .append("<html>")
          .append("<b>Usage:</b>")
          .append("<br>")
