...
public class WorldPanel extends JPanel {
  ...
  private void drawRobot(Graphics2D g2dIn, Robot robot) {
    ...
    final Rectangle2D bounds =
        g2d.getFont().getStringBounds(robot.name, g2d.getFontRenderContext());
  ...
  private Font prepareFont(Graphics2D g2d, double fontHeight) {
    final AffineTransform fontTransform = AffineTransform.getScaleInstance(1.0, -1.0);
    final Font font =
        g2d.getFont().deriveFont(Font.BOLD, (float) fontHeight).deriveFont(fontTransform);
    g2d.setFont(font);
    return font;
  }
    ...
    for (RadarRange range : robot.ranges) {
      final Arc2D arc =
          new Arc2D.Double(
              ...
              2 * range.range,
              2 * range.range,
              Math.toDegrees(-range.angle) - 1.0,
              2.0,
              Arc2D.PIE);
  ...
  private void drawWaypoint(Graphics2D g2dIn, Waypoint waypoint) {
    ...
    prepareFont(g2d, fontHeight);
    final Rectangle2D bounds =
        g2d.getFont().getStringBounds(waypoint.name, g2d.getFontRenderContext());
    g2d.setColor(Color.BLACK);
    g2d.drawString(
        waypoint.name,
        (float) (p.getX() - bounds.getWidth() / 2.0),
